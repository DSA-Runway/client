...
/**
 * Subtopic 7 of Arrays. Reuses the read/write pointer split from Remove
 * Duplicates, and is kept distinct because the choice between OVERWRITING and
 * SWAPPING has a cost that depends on the data rather than on the input size.
 *
 * SOURCES
 * - LeetCode 283, "Move Zeroes" — the in-place requirement, the no-copy
 *   constraint, and the demand that the relative order of non-zero elements is
 *   preserved.
 * - GeeksforGeeks, "Move all zeroes to end of array" — the temporary-array and
 *   two-pointer formulations.
 *
 * MEASURED ON THIS MACHINE (clang -O2, Python 3.13.4):
 *
 * 1. CORRECTNESS, INCLUDING ORDER. All four in-place variants were checked
 *    against a reference that filters non-zeros and pads with zeros, over
 *    200,000 random arrays — zero failures. Order preservation was part of the
 *    comparison, not assumed.
 *
 * 2. THE WRITE COUNTS CROSS OVER, and the crossing point is derivable. At
 *    n = 1,000, writing z for the fraction of zeros:
 *      zeros%   non-zero   overwrite   ovw+guard   swap    swap+guard
 *      0        1000       1000        0           3000    0
 *      33       673        1000        1000        2019    2019
 *      67       330        1000        990         990     990
 *      90       109        1000        1000        327     327
 *      100      0          1000        1000        0       0
 *    Overwrite always costs exactly n. Swap costs 3x the number of NON-zeros.
 *    So swap wins precisely when non-zeros are under one third of the array,
 *    and the measured crossover sits exactly at 67% zeros as 3f = 1 predicts.
 *
 * 3. THE i != j GUARD IS WORTH 1000 WRITES ON AN ARRAY WITH NO ZEROS, where
 *    both unguarded versions copy every element onto itself and the guarded
 *    ones perform zero writes.
 *
 * 4. THE HEADLINE — RUNTIME IS DOMINATED BY BRANCH PREDICTABILITY, NOT BY
 *    DENSITY OR WRITE COUNT. Three arrays of n = 10,000,000, all with EXACTLY
 *    50% zeros and identical write counts, differing only in arrangement:
 *      arrangement            overwrite   swap+guard
 *      random 50/50           31.529ms    30.679ms
 *      blocked (half, half)    5.691ms     4.936ms
 *      alternating 0,7,0,7     7.406ms     5.090ms
 *    Same size, same density, same writes — a 5.5x spread. The random array
 *    makes `if (a[i] != 0)` a coin flip the processor cannot predict, and every
 *    misprediction discards the speculative work in flight. The blocked and
 *    alternating arrays are perfectly predictable and cost a fifth as much.
 *    This is the first result in the module where the INPUT ARRANGEMENT, with
 *    every other quantity held fixed, is the dominant term.
 *
 * 5. PYTHON, n = 1,000,000 at 50% zeros:
 *      list comprehension : 23.101ms   <- fastest
 *      temp array         : 23.753ms
 *      two-pointer        : 49.180ms
 *      swap               : 51.154ms
 *      sort(key=)         : 63.990ms
 *    The C-level comprehension beats the interpreted two-pointer by 2.1x, the
 *    same pattern seen in every subtopic so far.
 *
 * 6. THE STABLE-SORT TRICK WORKS AND THE OBVIOUS SORT DOES NOT. Verified on
 *    [0,1,0,3,12]: sort(key=lambda x: x == 0) gives [1,3,12,0,0] because
 *    Python's sort is stable, while sorting by value gives [12,3,1,0,0] and
 *    destroys the ordering the problem demands.
 *
 * Scope: partitioning by an arbitrary predicate is the same machinery and shows
 * up in quicksort; that is pointed at rather than developed here.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Brute Force - Remove and Append",
      idea: "Repeatedly delete a zero from its position and push a zero onto the end.",
      steps: [
        "Count how many zeros the array contains.",
        "Repeat that many times.",
        "Find the first zero, remove it by shifting every later element one position left.",
        "Write a zero into the final position.",
        "The relative order of the non-zero elements is preserved because they only ever shift left as a block.",
      ],
      code: [
        {
          language: "cpp",
...
void moveZeroes(vector<int>& nums) {
    ...
    for (int c = 0; c < zeros; c++) {
        ...
        {
          language: "java",
          code: `static void moveZeroes(int[] nums) {
    int n = nums.length;
    int zeros = 0;
...
# temporary-array version — and the gap widens quadratically from there.`,
    ...
    {
      name: "Brute Force - Temporary Array",
      idea: "Collect the non-zeros into fresh storage in order, pad with zeros, then copy back.",
      steps: [
        "Create an empty temporary array.",
        "Walk the input once and append every non-zero element to the temporary array.",
        "Append zeros to the temporary array until it matches the original length.",
        "Copy the temporary array back over the original.",
        "The order is preserved because the non-zeros were appended in the order they were met.",
      ],
      code: [
        {
          language: "cpp",
...
# comprehension runs as compiled C rather than in the interpreter.`,
    ...
    {
      name: "Two Pointers - Overwrite Then Fill",
      idea: "Copy every non-zero forward to the write pointer, then fill the remaining tail with zeros.",
      steps: [
        "Set the write pointer j to zero.",
        "Scan the array with a read pointer i.",
        "Whenever the element at i is non-zero, write it to position j and advance j.",
        "Skip zeros entirely, leaving j where it is.",
        "After the scan, every position from j to the end is stale, so fill it with zeros.",
        "Optionally guard the copy with i != j to avoid writing an element onto itself.",
      ],
      code: [
        {
          language: "cpp",
...
# list comprehension at 23.101ms, which is the usual interpreter tax.`,
    ...
    {
      name: "Two Pointers - Swap in One Pass",
      idea: "Exchange each non-zero with the element at the write pointer, which pushes the zeros backwards as you go.",
      steps: [
        "Set the write pointer j to zero.",
        "Scan the array with a read pointer i.",
        "When the element at i is non-zero, swap it with the element at j and advance j.",
        "The value swapped back to i is necessarily a zero, so zeros accumulate behind the scan.",
        "No second pass is needed, because the tail fills with zeros as a side effect.",
        "Guard the swap with i != j to avoid exchanging an element with itself.",
      ],
      code: [
        {
          language: "cpp",
...
# the write-count difference disappears entirely.`,
    ...
    {
      name: "Library Call",
      idea: "Use the standard routine that already performs an order-preserving partition.",
      steps: [
        "In C++, call remove with the value 0, which compacts every non-zero to the front in order and returns the new logical end.",
        "Fill from that returned iterator to the end with zeros.",
        "Alternatively call stable_partition with a non-zero predicate, which does the same and keeps both groups ordered.",
        "In Python, use a stable sort keyed on whether the element is zero.",
        "Confirm the sort is stable, or the relative order of the non-zeros is not guaranteed.",
      ],
      code: [
        {
          language: "cpp",
...
# Measured 63.990ms at n = 1,000,000 — correct, and the slowest option here.`,
  ...
  examples: [
    {
      ...
      output: "[1, 3, 12, 0, 0]",
      walkthrough: [
        "The write pointer j starts at 0.",
        "i = 0: the value is 0, so it is skipped and j stays at 0.",
        "i = 1: the value is 1, which is non-zero, so it is written to position 0 and j advances to 1.",
        "i = 2: the value is 0, so it is skipped again.",
        "i = 3: the value is 3, written to position 1, and j advances to 2.",
        "i = 4: the value is 12, written to position 2, and j advances to 3.",
        "The scan ends with j = 3, so positions 3 and 4 are filled with zeros, giving [1, 3, 12, 0, 0].",
        "The non-zeros came out as 1, 3, 12 — the order in which they were met, which is what the problem requires.",
      ],
      why: "The statement's own example, and it shows the write pointer standing still on every zero, which is the mechanism that compacts the array.",
    },
    {
      input: "nums = [1, 2, 3] and nums = [0, 0, 0]",
      output: "[1, 2, 3] and [0, 0, 0] — both unchanged",
      walkthrough: [
        "With no zeros, every element is non-zero so i and j advance together and stay equal throughout.",
        "The i != j guard is therefore false every time, and the unguarded version instead writes each element onto itself.",
        "Measured at n = 1,000 with no zeros: the guarded version performs 0 writes and the unguarded one performs 1,000.",
        "With all zeros, the condition nums[i] != 0 is never true, so j never advances and the loop writes nothing.",
        "The fill pass then writes zeros over positions 0 to n-1, which are already zero.",
        "The swap version performs no swaps at all here, finishing with 0 writes.",
      ],
      why: "The two extremes, and together they show why the i != j guard is worth one comparison per element on exactly the inputs most likely to appear.",
    },
    {
      input: "The same 50%-zero array at n = 10,000,000, arranged three different ways",
      output: "31.529ms, 5.691ms and 7.406ms — a 5.5x spread",
      walkthrough: [
        "All three arrays contain exactly five million zeros and five million non-zeros.",
        "All three cause the identical number of element writes, because the write count depends only on the counts, not the arrangement.",
        "Randomly interleaved, the overwrite version measured 31.529ms.",
        "Arranged as five million non-zeros followed by five million zeros, the same code measured 5.691ms.",
        "Arranged as strictly alternating zero and non-zero, it measured 7.406ms.",
        "The only variable is whether the processor can predict the outcome of nums[i] != 0, which is a coin flip in the random case and trivially learnable in the other two.",
      ],
      why: "Size, density, write count and code all held fixed, leaving arrangement as the only explanation for a 5.5x difference — which no complexity analysis of this problem would ever surface.",
    },
    {
      input: "nums = [0, 1, 0, 3, 12] sorted two different ways in Python",
      output: "Stable key sort gives [1,3,12,0,0]; sorting by value gives [12,3,1,0,0]",
      walkthrough: [
        "Calling nums.sort(key=lambda x: x == 0) computes the key False for non-zeros and True for zeros.",
        "False sorts before True, so all the non-zeros come first and all the zeros go to the back.",
        "Python's sort is stable, so elements with equal keys retain their original relative order.",
        "All the non-zeros share the key False, so 1, 3 and 12 stay in that order, giving [1, 3, 12, 0, 0].",
        "Sorting by value in descending order also puts the zeros at the end, giving [12, 3, 1, 0, 0].",
        "That result is wrong, and the zeros being correctly placed is exactly what makes it look right at a glance.",
      ],
      why: "Shows that stability is the entire reason the key-sort trick is legitimate, and that a sort which merely puts zeros last is not a solution to this problem.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why can't I just sort the array?",
      answer:
    ...
    {
      question: "Should I overwrite and fill, or swap in one pass?",
      answer:
    ...
    {
      question: "Is the i != j guard worth an extra comparison?",
      answer:
    ...
    {
      question: "Why is my solution slower on some arrays than others of the same size?",
      answer:
    ...
    {
      question: "Does that mean I should reorder my input before processing it?",
      answer:
    ...
    {
      question: "Why is the two-pointer version slower than a list comprehension in Python?",
      answer:
    ...
    {
      question: "How is this different from Remove Duplicates from Sorted Array?",
      answer:
    ...
    {
      question: "Can I move the zeros by swapping from the end of the array?",
      answer:
    ...
    {
      question: "What is the library way to do this?",
      answer:
  ...
  relatedIds: ["remove-duplicates-from-sorted-array", "linear-search", "sort-an-array-of-0s-1s-and-2s", "rearrange-array-elements-by-sign"],
};
