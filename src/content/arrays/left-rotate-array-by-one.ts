...
/**
 * Subtopic 5 of Arrays. Mechanically the simplest problem in the module, and
 * kept because of what it sets up: the reversal trick, which is pointless at
 * k = 1 and is the whole answer at k = K.
 *
 * SOURCES
 * - GeeksforGeeks, "Array rotation" / "Left rotate an array by one" — the
 *   temporary-variable shift and the reversal algorithm.
 *
 * FIVE APPROACHES, because this subtopic's value is comparative rather than
 * algorithmic. Every one is O(n); they differ in writes, in space, and — as the
 * measurements show — in ways the write count does not predict at all.
 *
 * MEASURED ON THIS MACHINE (clang -O2, Python 3.13.4):
 *
 * 1. WRITE COUNTS, exact and deterministic at n = 1,000:
 *      single temp + shift : 1,001 writes  (1.0n)  <- the minimum
 *      temporary array     : 2,000 writes  (2.0n)
 *      adjacent swaps      : 2,997 writes  (3.0n)
 *      reversal (k = 1)    : 2,997 writes  (3.0n)
 *
 * 2. THE WRITE COUNT DOES NOT PREDICT THE RUNTIME. At n = 10,000,000:
 *      manual shift loop (1.0n writes) : 5.707ms
 *      reversal          (3.0n writes) : 4.685ms   <- 3x the writes, FASTER
 *      memmove + place   (1.0n writes) : 1.249ms   <- 4.4x the manual loop
 *      std::rotate                     : 1.250ms
 *    memmove performs exactly the same element moves as the hand-written loop
 *    and is 4.4x faster, because it moves in bulk instead of one element per
 *    iteration. std::rotate matches it to three decimal places, so libc++ is
 *    clearly dispatching to the same bulk path. Meanwhile clang reports
 *    std::reverse as "vectorized loop (vectorization width: 4, interleaved
 *    count: 2)", which is why the 3n-write reversal beats the 1n-write loop.
 *
 * 3. PYTHON, n = 1,000,000:
 *      manual loop          : 53.303ms
 *      reverse x3           : 19.033ms
 *      slicing a[1:]+a[:1]  : 14.001ms
 *      deque.rotate (+copy) :  7.402ms
 *      a.append(a.pop(0))   :  3.258ms   <- 16x faster than the manual loop
 *    pop(0) is nominally O(n) and still wins by 16x, because it is one C-level
 *    bulk move against a million interpreted iterations.
 *
 * 4. deque.rotate IS GENUINELY O(1), not merely fast. Measured on an existing
 *    deque: n = 1,000 -> 0.0535us, n = 100,000 -> 0.0538us, n = 10,000,000 ->
 *    0.0619us. Ten thousand times the data, essentially the same time. This is
 *    the one place in the module so far where picking a different data structure
 *    changes the complexity class rather than the constant factor.
 *
 * 5. THE CLASSIC BUG, executed rather than described: shifting before saving
 *    a[0] gives [2,3,4,5,2] on [1,2,3,4,5], because a[0] is destroyed by the
 *    very first iteration and the value placed at the end is the wrong one.
 *
 * Scope: rotating by K is the next subtopic and is where the reversal algorithm
 * stops being a curiosity. Rotating right is the mirror image and is noted, not
 * re-taught.
 */
  ...
  approaches: [
    {
      ...
      idea: "Build the rotated array in fresh storage, then copy it back over the original.",
      steps: [
        "Create a temporary array of the same length.",
        "Copy each element from index i + 1 of the original into index i of the temporary array.",
        "Copy the original's first element into the last position of the temporary array.",
        "Copy the temporary array back over the original.",
        "Return, having modified the original in place.",
      ],
      code: [
        {
          language: "cpp",
...
void rotateLeftByOne(vector<int>& arr) {
    ...
    for (int i = 0; i < n; i++) arr[i] = temp[i];
        ...
        {
          language: "java",
          code: `static void rotateLeftByOne(int[] arr) {
    int n = arr.length;
    if (n <= 1) return;
    ...
    System.arraycopy(temp, 0, arr, 0, n);
        ...
        {
          language: "python",
          code: `def rotate_left_by_one(arr):
    n = len(arr)
    if n <= 1:
        return
    ...
    {
      name: "Adjacent Swaps",
      idea: "Bubble the first element to the end by swapping it with each neighbour in turn.",
      steps: [
        "Start at index 0.",
        "Swap the element at the current index with the one immediately after it.",
        "Advance one index, so the original first element travels one step further right.",
        "Repeat until the original first element has reached the last position.",
        "Every other element has shifted one place left as a side effect.",
      ],
      code: [
        {
          language: "cpp",
...
void rotateLeftByOne(vector<int>& arr) {
    ...
    {
      name: "Optimal - Save One, Shift, Place",
      idea: "Hold the first element in one variable, slide everything left, then drop it into the vacated last slot.",
      steps: [
        "Return immediately if the array has fewer than two elements.",
        "Save the first element in a temporary variable.",
        "For each index from 0 to n - 2, copy the element from the next index into the current one.",
        "Stop the loop at n - 2, because reading index n would run past the end.",
        "Write the saved value into the last position.",
      ],
      code: [
        {
          language: "cpp",
...
# arr.append(arr.pop(0)) does the same job in 3.258ms.`,
    ...
    {
      name: "Reversal Algorithm",
      idea: "Reverse the first k elements, reverse the rest, then reverse the whole array.",
      steps: [
        "Reverse the first k elements, which for k = 1 is a single element and therefore does nothing.",
        "Reverse the remaining elements from index k to the end.",
        "Reverse the entire array.",
        "The result is the array rotated left by k positions.",
        "For k = 1 this is deliberate overkill; for a general k it is the whole solution.",
      ],
      code: [
        {
          language: "cpp",
...
static void rotateLeftByK(int[] arr, int k) {
    ...
    reverse(arr, 0, n - 1);
        ...
        {
          language: "python",
          code: `def rotate_left_by_k(arr, k):
    n = len(arr)
    if n <= 1:
        return
    k %= n
...
def rotate_left_by_one(arr):
    ...
    {
      name: "Library Call or the Right Data Structure",
      idea: "Use the standard rotate routine, or store the data in a structure where rotation costs nothing.",
      steps: [
        "In C++, call rotate with the new first element as the middle argument.",
        "In Python, use a deque and call rotate with a negative argument to rotate left.",
        "Recognise that a deque's rotate is a pointer adjustment rather than a data move.",
        "Account for the O(n) cost of converting a list into a deque if the data does not already live in one.",
        "Prefer this whenever rotation is a frequent operation rather than a one-off.",
      ],
      code: [
        {
          language: "cpp",
...
# n=10,000,000 — flat in n, because no elements move at all.`,
  ...
  examples: [
    {
      ...
      output: "[2, 3, 4, 5, 1]",
      walkthrough: [
        "Save the first element, so temp holds 1.",
        "i = 0: copy arr[1] into arr[0], giving [2, 2, 3, 4, 5].",
        "i = 1: copy arr[2] into arr[1], giving [2, 3, 3, 4, 5].",
        "i = 2: copy arr[3] into arr[2], giving [2, 3, 4, 4, 5].",
        "i = 3: copy arr[4] into arr[3], giving [2, 3, 4, 5, 5], and the loop stops here because reading arr[5] would run past the end.",
        "Write temp into the last slot, giving [2, 3, 4, 5, 1].",
        "Six writes for five elements, which is the n + 1 the analysis predicts.",
      ],
      why: "The full trace shows the duplicated value travelling along the array as a normal intermediate state rather than a bug, which is the part that looks alarming the first time it is watched.",
    },
    {
      input: "arr = [1, 2, 3, 4, 5], with the save omitted",
      output: "[2, 3, 4, 5, 2] — wrong, and plausible-looking",
      walkthrough: [
        "The shift runs first, so i = 0 copies arr[1] into arr[0] and the value 1 is destroyed immediately.",
        "The remaining iterations proceed normally, leaving [2, 3, 4, 5, 5].",
        "Now the code reads arr[0] to fill the last slot, but arr[0] no longer holds 1 — it holds 2.",
        "The final array is [2, 3, 4, 5, 2], executed and verified on this machine.",
        "The original first element has vanished and the second element appears twice.",
        "Nothing about the output signals an error: it is the right length, and it looks like a rotation.",
      ],
      why: "The single most common way to write this wrong, and the reason the ordering of two adjacent lines is worth stating explicitly rather than assuming.",
    },
    {
      input: "arr = [7] and arr = []",
      output: "[7] and [] — both unchanged",
      walkthrough: [
        "A single-element array rotated by one position is itself, since the only element wraps back to where it started.",
        "The guard returns immediately, and even without it the loop bound n - 1 evaluates to 0 so the body never runs.",
        "The final write arr[n-1] = temp would then write arr[0] back to itself, which is harmless but pointless.",
        "For an empty array the guard is genuinely necessary: n - 1 is -1, and arr[-1] is an out-of-bounds write in C++ and Java.",
        "In Python arr[-1] would silently target the last element, so on an empty list it raises IndexError instead.",
        "Both cases are handled by the single n <= 1 check at the top.",
      ],
      why: "The empty case is the one that actually needs the guard, and it fails differently in each language — silently corrupting memory in C++ and raising in Python.",
    },
    {
      input: "The same rotation performed by four approaches at n = 10,000,000",
      output: "Runtimes that do not follow the write counts",
      walkthrough: [
        "The hand-written shift loop performs 1.0n writes, the theoretical minimum, and takes 5.707ms.",
        "The reversal algorithm performs 3.0n writes, three times as many, and takes 4.685ms — faster despite tripling the work.",
        "Compiled with -Rpass=loop-vectorize, clang reports std::reverse as vectorized at width 4 with interleave 2, so it moves several elements per instruction.",
        "memmove performs exactly the same 1.0n writes as the hand-written loop and takes 1.249ms, which is 4.4x faster.",
        "std::rotate takes 1.250ms, matching memmove to three decimal places and indicating it uses the same bulk path.",
        "So the fastest and the slowest versions here perform an identical number of element writes.",
      ],
      why: "The cleanest demonstration in the module that operation counts rank algorithms rather than runtimes — two versions with exactly the same write count differ by 4.4x.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why must I save arr[0] before the shift instead of after?",
      answer:
    ...
    {
      question: "Why does the loop stop at n - 2 rather than n - 1?",
      answer:
    ...
    {
      question: "The mid-rotation array shows a duplicated value. Is something wrong?",
      answer:
    ...
    {
      question: "Which approach has the fewest writes?",
      answer:
    ...
    {
      question: "Then why is the version with the fewest writes not the fastest?",
      answer:
    ...
    {
      question: "Why teach the reversal algorithm for a one-step rotation?",
      answer:
    ...
    {
      question: "What is the fastest way to do this in Python?",
      answer:
    ...
    {
      question: "Is deque.rotate really O(1), or just fast?",
      answer:
    ...
    {
      question: "How do I rotate right by one instead?",
      answer:
  ...
  relatedIds: ["left-rotate-array-by-k-places", "check-if-array-is-sorted-and-rotated", "move-zeros-to-end", "remove-duplicates-from-sorted-array"],
};
