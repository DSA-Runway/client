...
/**
 * Subtopic 17 of Arrays (Medium #3). The same question as subtopic 16 with the
 * sign restriction removed — and that one word removal deletes an entire
 * approach, which is why this is a separate subtopic rather than a footnote.
 *
 * It also carries a genuinely counterintuitive measured result: the prefix-map
 * solution runs 8.6x FASTER on arrays containing negatives than on all-positive
 * ones, because negatives make the prefix walk revisit values and keep the map
 * small enough to live in cache.
 *
 * SOURCES
 * - GeeksforGeeks, "Longest sub-array having sum k" — the general (signed)
 *   formulation and the prefix-sum hashing solution.
 *
 * MEASURED ON THIS MACHINE (Apple M2, arm64, clang -O2, Python 3.13.4):
 *
 * 1. THREE FAILURE MODES, ALL QUANTIFIED. Over every array of length 0..6 drawn
 *    from {-2,-1,0,1,2} against every k in -3..3 — 136,717 (array, k) pairs:
 *      correct prefix map          :      0 failures
 *      forgetting the {0:-1} seed  : 39,528 failures (28.9%)
 *      storing the LATEST index    : 39,123 failures (28.6%)
 *      subtopic 16's sliding window: 65,418 failures (47.8%)
 *    Smallest counterexamples, all tiny:
 *      no seed   : a=[-2]      k=-2 -> 0, correct 1
 *      overwrite : a=[0,-2]    k=-2 -> 1, correct 2
 *      window    : a=[-2,-1]   k=-3 -> 0, correct 2
 *
 * 2. THE OVERWRITE BUG IS NOW LIVE. In subtopic 16 it was measured at 0 failures
 *    over 7,651 all-positive pairs, because positive values make prefix sums
 *    strictly increasing so no sum ever repeats. Here it fails 28.6%. Same code,
 *    same tests, different input class.
 *
 * 3. WITH NEGATIVES THE PREFIX SUMS COLLAPSE. Distinct prefix sums at
 *    n = 1,000,000:
 *      positive 1..9      : 1,000,001  (100.0% of n)
 *      non-negative 0..9  :   899,932  ( 90.0%)
 *      mixed -9..9        :     5,948  (  0.6%)
 *      mixed -1..1        :     1,271  (  0.1%)
 *    A signed prefix sum is a random walk, whose range grows like sqrt(n) rather
 *    than n, so it revisits values constantly.
 *
 * 4. WHICH MAKES THE SAME CODE FASTER ON THE HARDER INPUT. Identical algorithm,
 *    identical n = 10,000,000, only the values differing:
 *      positive 1..9 : 10,000,001 distinct sums -> 318.77ms
 *      mixed -9..9   :     21,290 distinct sums ->  39.11ms
 *      mixed -1..1   :      2,781 distinct sums ->  37.22ms
 *    8.6x faster with negatives present. The map is cache-resident instead of
 *    thrashing, and that dominates everything else.
 *
 * 5. AGAINST BRUTE FORCE, n = 100,000 with values -9..9: 2,975.32ms against
 *    0.42ms — 7,127x.
 *
 * FEWER APPROACHES THAN SUBTOPIC 16, deliberately. Removing the sign guarantee
 * removes the sliding window and the binary-search-on-prefixes variant, because
 * both depend on monotonicity. That reduction IS the lesson.
 *
 * Scope: counting subarrays with a given sum uses the same prefix map with
 * counts rather than first-indices, and is its own subtopic.
 */
  ...
  approaches: [
    {
      ...
      idea: "Try every starting point, extend to every endpoint with a running sum, and keep the longest match.",
      steps: [
        "Take each index in turn as the start of a subarray.",
        "Reset a running sum to zero.",
        "Extend the endpoint one element at a time, adding each to the running sum.",
        "Whenever the running sum equals k, record the length if it beats the best so far.",
        "Do not stop early — with negatives, a longer subarray from the same start may also reach k.",
      ],
      code: [
        {
          language: "cpp",
...
int longestSubarray(const vector<int>& a, long long k) {
    ...
    return best;
          ...
          annotations: {
            ...
            13: "Measured 2,975.32ms at n = 100,000 against 0.42ms for the prefix map — 7,127x.",
          },
        },
        {
          language: "java",
...
# prefix map it makes no assumption about direction at all.`,
    ...
    {
      name: "Guarded Sliding Window",
      idea: "Check once whether any value is negative, and dispatch to the fast window only when it is safe to do so.",
      steps: [
        "Scan the array once looking for a negative value.",
        "If none is found, run the sliding window from the previous subtopic, which is faster and uses no extra space.",
        "If any negative is present, fall back to the prefix map.",
        "Never run the window without that check, because it cannot detect its own precondition being violated.",
      ],
      code: [
        {
          language: "cpp",
...
# risking the second.`,
    ...
    {
      name: "Optimal - Prefix Sums with a Hash Map",
      idea: "Remember the earliest index at which each running total occurred, and look up the total a qualifying subarray would have started from.",
      steps: [
        "Seed a map with prefix sum 0 at index -1, representing the empty prefix before the array begins.",
        "Walk the array keeping a running total.",
        "At each index, look up the running total minus k in the map.",
        "If it is present, a subarray ending here sums to k, and its length is the current index minus the stored one.",
        "Insert the current running total only if it is not already present, so the earliest index is preserved.",
      ],
      code: [
        {
          language: "cpp",
...
# because the prefix walk revisits values and the map stays cache-resident.`,
  ...
  examples: [
    {
      input: "a = [10, 5, 2, 7, 1, -10], k = 15",
      output: "6",
      walkthrough: [
        "The map starts holding prefix 0 at index -1.",
        "After index 0 the running total is 10, and 10 - 15 = -5 is not in the map, so 10 is recorded at index 0.",
        "After index 1 the total is 15, and 15 - 15 = 0 IS in the map at index -1, giving a length of 0 - (-1) = 1... which is the subarray [10,5] of length 2.",
        "Working through the rest, the totals reach 17, 24, 25 and finally 15 again at index 5.",
        "At index 5 the lookup for 0 returns index -1 again, giving a length of 5 - (-1) = 6.",
        "That is the whole array, whose sum is 15 because the trailing -10 cancels an earlier 10.",
        "Note the total passed through 15 at index 1 and returned to it at index 5 — only possible because a negative pulled it back down.",
      ],
      why: "Shows a running total leaving the target and returning to it, which cannot happen with non-negative values and is exactly what defeats the sliding window.",
    },
    {
      input: "a = [-2], k = -2, with the map seed omitted",
      output: "0 — the correct answer is 1",
      walkthrough: [
        "The running total after the single element is -2, which equals k.",
        "The algorithm looks up total minus k, which is -2 - (-2) = 0.",
        "Without the seed the map is empty at that moment, so the lookup misses.",
        "The single element is then recorded, the loop ends, and 0 is returned.",
        "With the seed present, prefix 0 sits at index -1, the lookup succeeds, and the length is 0 - (-1) = 1.",
        "Measured over 136,717 pairs, omitting the seed produced 39,528 wrong answers — 28.9%.",
      ],
      why: "A one-element array is enough to expose it, and the failure mode is that every subarray starting at index 0 becomes invisible — which is a large fraction of all answers.",
    },
    {
      input: "a = [0, -2], k = -2, with the map storing the LATEST index",
      output: "1 — the correct answer is 2",
      walkthrough: [
        "The map starts with prefix 0 at index -1.",
        "After index 0 the running total is still 0, since the element is 0.",
        "The correct version leaves prefix 0 pointing at index -1; the buggy version overwrites it to point at index 0.",
        "After index 1 the total is -2, and the lookup for -2 - (-2) = 0 succeeds.",
        "The correct version reads index -1 and returns a length of 1 - (-1) = 2, the whole array.",
        "The buggy version reads index 0 and returns a length of 1 - 0 = 1, finding only the trailing element.",
        "Measured, that overwrite produced 39,123 wrong answers over the same 136,717 pairs — 28.6%.",
      ],
      why: "The same bug measured zero failures across 7,651 all-positive pairs one subtopic ago, because positive prefix sums never repeat — so this two-element array is the smallest thing that could have caught it there and could not exist there.",
    },
    {
      input: "The identical algorithm on 10,000,000 positive values versus 10,000,000 mixed values",
      output: "318.77ms on positives, 39.11ms on mixed — the harder input is faster",
      walkthrough: [
        "With values drawn from 1 to 9 every prefix sum is larger than the last, so all 10,000,001 of them are distinct.",
        "The map therefore holds ten million entries, and almost every lookup is a cache miss.",
        "With values drawn from -9 to 9 the running total is a random walk that keeps returning to values it has already visited.",
        "Only 21,290 distinct prefix sums occur across the entire ten million elements — about 0.2 percent as many.",
        "That map fits comfortably in cache, so lookups are fast, and the same code finishes in 39.11ms.",
        "Narrowing the values to -1..1 shrinks the map further to 2,781 entries and the time to 37.22ms.",
      ],
      why: "The same code running 8.6x faster on the input that removed your best algorithm — a clean separation between a problem being algorithmically harder and being computationally slower.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why can't I use the sliding window from the previous subtopic?",
      answer:
    ...
    {
      question: "Why does the map need the entry {0: -1}?",
      answer:
    ...
    {
      question: "Why must I keep the earliest index rather than the most recent?",
      answer:
    ...
    {
      question: "I wrote that overwriting version for the previous subtopic and every test passed. How?",
      answer:
    ...
    {
      question: "Why is the prefix map faster on arrays with negatives? That seems backwards.",
      answer:
    ...
    {
      question: "How much memory does the map actually need?",
      answer:
    ...
    {
      question: "Should I ever still use the sliding window?",
      answer:
    ...
    {
      question: "Can I break out of the loop when I find a match?",
      answer:
    ...
    {
      question: "How does this extend to counting subarrays instead of finding the longest?",
      answer:
  ...
  relatedIds: ["longest-subarray-with-given-sum-k-positives", "count-subarrays-with-given-sum", "largest-subarray-with-sum-0", "two-sum"],
};
