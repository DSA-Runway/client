...
/**
 * Subtopic 13 of Arrays. Boyer-Moore voting — the first algorithm in the module
 * that looks like a trick until the cancellation argument makes it obvious, and
 * the first whose correctness depends entirely on a PRECONDITION it cannot check
 * for itself.
 *
 * SOURCES
 * - LeetCode 169, "Majority Element" — the statement and the guarantee that a
 *   majority always exists.
 * - GeeksforGeeks, "Majority Element" — the brute force, sorting, hashing and
 *   Boyer-Moore formulations.
 *
 * MEASURED ON THIS MACHINE (Apple M2, arm64, clang -O2, Python 3.13.4):
 *
 * 1. BOYER-MOORE IS EXACTLY CORRECT WHEN A MAJORITY EXISTS. Checked over every
 *    array of length 1..12 drawn from 3 symbols that actually has a majority —
 *    196,230 arrays — with 0 failures.
 *
 * 2. AND IT HAS NO ERROR PATH WHEN ONE DOES NOT. Over the 600,930 arrays in that
 *    same space with NO majority, it returned an element every time and never
 *    signalled anything. (That the returned element is not a majority is
 *    definitional, not a discovery — the finding is that there is no failure
 *    mode to observe.) Concretely: [1,2,3] returns 3 which appears once of
 *    three; [1,1,2,2] returns 1 which appears twice of four. Both look like
 *    ordinary answers.
 *
 * 3. VERIFICATION IS ALMOST FREE. Adding a second counting pass to confirm the
 *    candidate measured 17.04ms against 16.15ms at n = 10,000,000 — 5.5%. There
 *    is no performance argument for omitting it when the guarantee is not
 *    written into the problem.
 *
 * 4. BOYER-MOORE WINS OUTRIGHT IN C++, unlike the hash map in Two Sum.
 *    n = 10,000,000:
 *      sort              : 96.11ms
 *      hash map          : 61.88ms
 *      bit counting (32) : 31.50ms
 *      Boyer-Moore       : 16.15ms   <- 3.8x the hash, 6.0x the sort
 *      Boyer + verify    : 17.04ms
 *    Note the bit-counting variant makes 32 full passes and still beats the hash
 *    map by 2x, which is the Two Sum lesson again: sequential scans are cheap
 *    and hashing is not.
 *
 * 5. THE BRUTE FORCE IS NOT REALLY QUADRATIC HERE, which is why it measured
 *    0.01ms at n = 100,000. It tests a[0] first, and the majority occupies MORE
 *    THAN HALF the array, so each candidate is the answer with probability
 *    p > 1/2. The number of candidates tried is geometric with mean 1/p < 2.
 *    Simulated over 20,000 trials: mean 1.998 at n = 1,001 and 2.009 at
 *    n = 1,000,001, against the theoretical 2.000, with a maximum of 17.
 *    Expected work is about 2n. The O(n^2) worst case is real but needs an
 *    adversarial arrangement with every non-majority element placed first.
 *
 * 6. PYTHON, n = 1,000,000: Counter 60.8ms, Boyer-Moore 66.3ms, Boyer+verify
 *    82.0ms, sorted 124.4ms, hand-written dict count 137.3ms. Counter edges out
 *    the interpreted voting loop because its counting runs in C — and the
 *    hand-written dict version is the slowest thing here by more than 2x.
 *
 * Scope: Majority Element II (elements appearing more than n/3 times) extends
 * the voting idea to two candidates and is a Hard subtopic.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Brute Force - Count Each Candidate",
      idea: "For each element, count how many times it occurs, and return the first one exceeding half the length.",
      steps: [
        "Take each element in turn as a candidate.",
        "Scan the whole array counting how many times that candidate appears.",
        "If the count exceeds half the array length, return the candidate.",
        "Otherwise move to the next candidate.",
        "Because the majority occupies over half the array, a shuffled input usually finds it within the first two candidates.",
      ],
      code: [
        {
          language: "cpp",
...
# Simulated over 20,000 trials at n = 1,000,001: mean 2.009, worst seen 17.`,
    ...
    {
      name: "Sorting",
      idea: "Sort the array; the element at the middle index must be the majority.",
      steps: [
        "Sort the array in ascending order.",
        "Observe that the majority now forms one contiguous block longer than half the array.",
        "A block that long must cover the middle index regardless of where it begins.",
...
int majorityElement(vector<int> nums) {     // by value: caller's data survives
    ...
    return nums[nums.size() / 2];
          ...
          annotations: {
            ...
            8: "No search is needed — the middle index is provably inside the majority's block.",
          },
        },
        {
          language: "java",
...
# Measured 124.4ms at n = 1,000,000 — the second slowest option in Python.`,
    ...
    },
    {
      ...
      idea: "Tally occurrences in a map and return the first key whose count exceeds half the length.",
      steps: [
        "Create an empty map from value to running count.",
        "Walk the array, incrementing the count for each element.",
        "After each increment, check whether that count now exceeds half the array length.",
        "Return that element as soon as it does, without finishing the scan.",
      ],
      code: [
        {
          language: "cpp",
...
#       if freq[x] > len(nums) // 2: return x`,
    ...
    {
      name: "Bit Counting",
      idea: "Decide each bit of the answer independently by counting how many elements have it set.",
      steps: [
        "Consider each of the 32 bit positions in turn.",
        "Count how many elements have that bit set.",
        "If more than half of them do, the majority element must have that bit set, since it alone accounts for over half the array.",
        "Set that bit in the result.",
        "After all 32 positions the result is the majority element.",
      ],
      code: [
        {
          language: "cpp",
...
int majorityElement(const vector<int>& nums) {
    ...
    return result;
          ...
          annotations: {
            8: "Each bit is decided independently, because the majority alone contributes more than half the array's votes for it.",
...
# explicit width and sign handling that the C++ and Java versions get free.`,
    ...
    },
    {
      ...
      idea: "Carry one candidate and one counter, letting disagreements cancel out, and confirm the survivor.",
      steps: [
        "Set the counter to zero and leave the candidate undefined.",
        "Visit each element once.",
        "If the counter is zero, adopt this element as the candidate and set the counter to one.",
        "Otherwise increase the counter if the element equals the candidate, and decrease it if it does not.",
        "After the scan, the candidate is the majority if one exists.",
        "Make a second pass counting the candidate to confirm it, unless a majority is guaranteed by the problem.",
      ],
      code: [
        {
          language: "cpp",
...
# Verification adds 15.7ms, and nums.count() runs in C so it stays cheap.`,
  ...
  examples: [
    {
      ...
      output: "2",
      walkthrough: [
        "Start with the counter at zero, so the first 2 becomes the candidate with count 1.",
        "The second 2 matches, taking the count to 2.",
        "A 1 disagrees, dropping the count to 1 — one vote for 2 and one for 1 have cancelled.",
        "Another 1 disagrees, dropping the count to 0, so all votes so far have annihilated each other.",
        "The next 1 arrives with the count at zero, so 1 becomes the new candidate with count 1.",
        "A 2 disagrees, cancelling back to 0, and the final 2 arrives at zero to become the candidate again with count 1.",
        "The scan ends with 2 as the candidate, and counting confirms it appears 4 times in 7 — a genuine majority.",
      ],
      why: "The candidate changes twice during this scan, which is the behaviour that makes the algorithm look unreliable until the cancellation argument explains why the final survivor is still guaranteed correct.",
    },
    {
      input: "nums = [1, 2, 3] and nums = [1, 1, 2, 2], with no verification",
      output: "Returns 3 and 1 — neither is a majority, and nothing signals it",
      walkthrough: [
        "On [1, 2, 3] every element disagrees with the one before, so the count returns to zero at each step.",
        "The last element seen becomes the final candidate, so the algorithm returns 3.",
        "3 appears once in an array of three, where a majority needs more than one occurrence.",
        "On [1, 1, 2, 2] the two 1s build a count of 2 and the two 2s cancel it back to 0, leaving 1 as the candidate.",
        "1 appears twice in an array of four, and a majority needs more than two.",
        "In both cases an ordinary-looking element of the array is returned with no exception and no sentinel — there is no failure mode to detect.",
      ],
      why: "The reason the verification pass exists. The algorithm's contract is conditional, and when the condition is broken it does not fail loudly — it produces a plausible wrong answer.",
    },
    {
      input: "The same 10,000,000-element array through every approach",
      output: "Boyer-Moore 16.15ms, bit counting 31.50ms, hash 61.88ms, sort 96.11ms",
      walkthrough: [
        "Boyer-Moore performs one sequential pass holding two integers, and finishes in 16.15ms.",
        "The bit-counting variant performs thirty-two complete passes over the same array and finishes in 31.50ms.",
        "That means thirty-two sequential scans cost only twice what one hashing pass costs.",
        "The hash map needs 61.88ms for its single pass, because each element is hashed, probed and possibly allocated for.",
        "Sorting takes 96.11ms to derive a complete ordering from which only one position is read.",
        "Adding the verification pass to Boyer-Moore brings it to 17.04ms, an increase of 5.5%.",
      ],
      why: "Unlike Two Sum, where the O(n) hash map lost to an O(n log n) sort, here the asymptotically best approach is also the fastest — and the bit-counting result shows why: sequential scanning is close to free.",
    },
    {
      input: "Brute force at n = 100,000 on shuffled input",
      output: "0.01ms — linear in practice, not quadratic",
      walkthrough: [
        "Brute force tests the element at index 0 first and counts its occurrences.",
        "The majority occupies more than half the array, so a shuffled a[0] is the majority with probability above one half.",
        "The number of candidates tried before finding it is therefore geometric with expected value 1/p, which is under 2.",
        "Simulated over 20,000 trials at n = 1,000,001: a mean of 2.009 candidates against the theoretical 2.000, with a worst case of 17.",
        "The expected total work is about 2n, which is linear, and explains the measured 0.01ms.",
        "The O(n^2) worst case is real but requires every non-majority element to be placed before the first majority one, which shuffled data essentially never produces.",
      ],
      why: "Explains a measurement that otherwise looks impossible, and warns that an algorithm can be quadratic in the worst case while looking linear on every random benchmark you run.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why does Boyer-Moore work? It looks like it just guesses.",
      answer:
    ...
    {
      question: "The candidate changes partway through my trace. Is that a bug?",
      answer:
    ...
    {
      question: "What happens if there is no majority element?",
      answer:
    ...
    {
      question: "Isn't the verification pass expensive?",
      answer:
    ...
    {
      question: "Why does sorting and taking the middle element work?",
      answer:
    ...
    {
      question: "Should I use the hash map instead? It's also O(n).",
      answer:
    ...
    {
      question: "Why did my brute force run so fast? It's supposed to be O(n^2).",
      answer:
    ...
    {
      question: "How does the bit-counting approach work, and is it worth using?",
      answer:
    ...
    {
      question: "How does this extend to elements appearing more than n/3 times?",
      answer:
  ...
  relatedIds: ["majority-element-ii", "two-sum", "maximum-consecutive-ones", "largest-element"],
};
