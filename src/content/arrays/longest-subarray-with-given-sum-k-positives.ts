...
/**
 * Subtopic 16 of Arrays (Medium #2). The first sliding-window problem, and the
 * first whose optimal solution is correct only under a stated precondition on
 * the VALUES rather than on their arrangement.
 *
 * The parenthesised "(positives)" in the title is the entire subtopic. Subtopic
 * 17 is the same question without it, and it needs a different algorithm.
 *
 * SOURCES
 * - GeeksforGeeks, "Longest sub-array having sum k" — the brute force, prefix-sum
 *   hashing and two-pointer formulations.
 *
 * MEASURED ON THIS MACHINE (Apple M2, arm64, clang -O2, Python 3.13.4):
 *
 * 1. THE WINDOW IS EXACT ON POSITIVES. Every array of length 0..7 over the values
 *    {1,2,3} against every k in 0..9 — 32,800 (array, k) pairs — with 0 failures
 *    against a brute-force reference.
 *
 * 2. ZEROS ARE SAFE, which is more than the usual statement of the precondition
 *    claims. Every array of length 0..7 over {0,1,2} against every k in 0..7 —
 *    26,240 pairs — also 0 failures. The shrink rule fires only while the sum
 *    EXCEEDS k, and a zero can never push it over, so a zero never triggers a
 *    wrong shrink. The real requirement is NON-NEGATIVE, not strictly positive.
 *
 * 3. NEGATIVES BREAK IT, AND BADLY. Over values {-1,1,2}, 6,558 pairs produced
 *    2,007 wrong answers — 30.6%. The smallest counterexamples are two elements
 *    long:
 *      a = [1,-1], k = 0  -> window says 0, correct 2
 *      a = [-1,1], k = 1  -> window says 0, correct 1
 *    The shrink rule assumes removing an element LOWERS the sum. Removing a
 *    negative raises it, so the pointer moves the wrong way and never recovers.
 *
 * 4. THE PREFIX-MAP BUG IS INVISIBLE ON EXACTLY THIS INPUT CLASS. Storing the
 *    LATEST index for each prefix sum instead of the earliest gives the shortest
 *    subarray rather than the longest. Measured: 0 failures over 7,651 pairs on
 *    positives, and 1,174 failures — 15.3% — over the same count with negatives.
 *    On positives the prefix sums are STRICTLY INCREASING, so no sum ever repeats
 *    and there is nothing to overwrite. The bug is dormant here and fires the
 *    moment the code is reused for subtopic 17.
 *
 * 5. THE WINDOW BEATS THE PREFIX MAP BY 8x, CONSISTENTLY. C++ wall clock:
 *      n = 100,000    brute 2,945.15ms   prefix 3.55ms    window 0.44ms
 *      n = 1,000,000  (skipped)          prefix 36.43ms   window 4.53ms
 *      n = 10,000,000 (skipped)          prefix 362.22ms  window 45.29ms
 *    At n = 100,000 the window is 6,693x the brute force. Against the prefix map
 *    it is 8x at every size, and uses O(1) space against O(n).
 *
 * 6. PYTHON narrows the gap, as usual, because both are interpreted loops:
 *    n = 2,000,000 — prefix hash 295.5ms, window 201.9ms, so 1.5x.
 *
 * 7. THE BINARY-SEARCH MIDDLE RUNG WORKS ON NON-NEGATIVES TOO. I expected zeros
 *    to break it, since they make the prefix array non-strictly-increasing —
 *    they do not. bisect_left returns the LEFTMOST matching index, which is
 *    exactly what "longest" wants. Verified 0 failures over 32,800 positive
 *    pairs and 8,744 pairs containing zeros.
 *
 * Scope: subtopic 17 removes the sign restriction and the sliding window with
 * it. Counting subarrays rather than measuring the longest is its own problem.
 */
  ...
  approaches: [
    {
      ...
      idea: "Try every starting point, extend to every endpoint, and keep a running sum so each subarray costs one addition.",
      steps: [
        "Take each index in turn as the start of a subarray.",
        "Reset a running sum to zero.",
        "Extend the end one element at a time, adding each to the running sum.",
        "Whenever the running sum equals k, record the length if it beats the best so far.",
        "Continue to the end of the array, then move the start forward.",
      ],
      code: [
        {
          language: "cpp",
...
int longestSubarray(const vector<int>& a, long long k) {
    ...
    return best;
        ...
        {
          language: "java",
          code: `static int longestSubarray(int[] a, long k) {
    int best = 0;
...
# sliding window — a factor of 6,693.`,
    ...
    {
      name: "Prefix Sums with Binary Search",
      idea: "With non-negative values the prefix sums never decrease, so the matching earlier prefix can be found by binary search rather than by scanning.",
      steps: [
        "Build the array of prefix sums, where entry i is the sum of everything before index i.",
        "Note that non-negative values make this array non-decreasing.",
        "For each endpoint, compute the prefix value that a qualifying subarray would need to start from.",
        "Binary search the prefix array for that value, taking the leftmost match.",
        "If it is present, the distance between the two positions is the subarray's length.",
      ],
      code: [
        {
          language: "cpp",
...
# prefix sums that zeros create.`,
    ...
    {
      name: "Prefix Sums with a Hash Map",
      idea: "Record where each prefix sum first occurred, and look up the prefix a qualifying subarray would have started from.",
      steps: [
        "Keep a running prefix sum and a map from prefix value to the earliest index where it occurred.",
        "Seed the map with prefix 0 at index -1, so subarrays starting at index 0 are found.",
        "At each index, check whether the current prefix minus k is already in the map.",
        "If it is, the subarray between that index and the current one sums to k, so record its length.",
        "Insert the current prefix ONLY if it is not already present, preserving the earliest index.",
      ],
      code: [
        {
          language: "cpp",
...
int longestSubarray(const vector<int>& a, long long k) {
    ...
    return best;
          ...
          annotations: {
            ...
            18: "That bug is invisible on positive input, where prefix sums strictly increase and no sum ever repeats.",
          },
        },
        {
          language: "java",
...
# Measured 295.5ms at n = 2,000,000 against 201.9ms for the sliding window.`,
    ...
    },
    {
      ...
      idea: "Extend the window from the right, pull it in from the left whenever the sum overshoots, and record every time it lands exactly on k.",
      steps: [
        "Start both pointers at the beginning with a running sum of zero.",
        "Extend the right pointer by one element, adding it to the sum.",
        "While the sum exceeds k, remove the element at the left pointer and advance it.",
        "If the sum now equals k, record the window's length if it is the best so far.",
        "Repeat until the right pointer reaches the end.",
        "Both pointers only ever move forward, so each element is added once and removed at most once.",
      ],
      code: [
        {
          language: "cpp",
...
int longestSubarray(const vector<int>& a, long long k) {
    ...
    return best;
        ...
        {
          language: "java",
          code: `// PRECONDITION: every element is non-negative.
static int longestSubarray(int[] a, long k) {
    int left = 0, best = 0;
    long sum = 0;
...
# 2,007 of 6,558 pairs — 30.6% — with [1,-1] k=0 the smallest failure.`,
  ...
  examples: [
    {
      ...
      output: "3",
      walkthrough: [
        "right = 0 puts 2 in the window, sum 2, which is under k so nothing shrinks.",
        "right = 1 adds 3 for sum 5, still under k.",
        "right = 2 adds 5 for sum 10, which equals k, so the window [2,3,5] of length 3 is recorded.",
        "right = 3 adds 1 for sum 11, which exceeds k, so the left pointer removes 2, leaving [3,5,1] with sum 9.",
        "right = 4 adds 9 for sum 18, so the window shrinks twice — removing 3 to reach 15, then 5 to reach 10.",
        "The window is now [1,9] with sum 10, which equals k and gives length 2 — shorter than the 3 already recorded.",
        "The answer is 3, and the left pointer moved forward a total of three times across the whole scan.",
      ],
      why: "Shows the window landing on k twice with different lengths, which is why the algorithm records rather than returns on the first hit.",
    },
    {
      input: "a = [1, -1], k = 0 — the precondition violated",
      output: "0 — the correct answer is 2",
      walkthrough: [
        "right = 0 puts 1 in the window, giving sum 1.",
        "The sum 1 exceeds k = 0, so the shrink loop runs and removes the 1, leaving an empty window with sum 0.",
        "The sum now equals k, but the window is empty, so the recorded length is 0.",
        "right = 1 adds -1, giving sum -1, which does not exceed k so nothing shrinks and nothing matches.",
        "The scan ends with best = 0, yet the whole array [1,-1] sums to exactly 0 and has length 2.",
        "The element the answer needed was thrown away at the first step, and the window never reconsiders discarded elements.",
      ],
      why: "The smallest possible demonstration that the failure is structural rather than an edge case — the algorithm discards the answer on its first move and has no mechanism to recover it.",
    },
    {
      input: "a = [0, 0, 3, 0], k = 3",
      output: "4",
      walkthrough: [
        "right = 0 and right = 1 add zeros, keeping the sum at 0, which never exceeds k so nothing shrinks.",
        "right = 2 adds 3, bringing the sum to exactly 3, so the window [0,0,3] of length 3 is recorded.",
        "right = 3 adds another 0, leaving the sum at 3 — still equal to k, and now the window has length 4.",
        "The answer is 4, and the leading zeros were never discarded because they never pushed the sum above k.",
        "This is why zeros are safe: the shrink loop is guarded by sum > k, and a zero cannot cause that.",
        "Verified over 26,240 arrays drawn from {0,1,2}: zero failures.",
      ],
      why: "Establishes that the precondition is non-negativity rather than strict positivity, which is a stronger statement than the problem title makes and one the measurement supports.",
    },
    {
      input: "a = [-1, 1, 1], k = 1, through the prefix map storing the LATEST index",
      output: "1 — the correct answer is 3",
      walkthrough: [
        "The prefix sums are -1, 0 and 1 after each element, with prefix 0 seeded at index -1.",
        "At index 1 the running prefix is 0, which already exists in the map from the seed.",
        "The buggy version overwrites it, so prefix 0 now maps to index 1 instead of index -1.",
        "At index 2 the prefix is 1, and the lookup for prefix 0 returns index 1, giving a length of just 1.",
        "Had the earliest index been kept, the lookup would have returned -1 and given the full length of 3.",
        "Measured over 7,651 pairs containing negatives, that overwrite produced 1,174 wrong answers — 15.3%.",
        "Over 7,651 all-positive pairs it produced zero, because positive values make prefix sums strictly increase so none ever repeats.",
      ],
      why: "A bug that is provably unable to fire on this subtopic's input and fires on one input in seven in the next subtopic — the clearest case in the module of a test suite that cannot detect a real defect.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why does the sliding window need positive numbers?",
      answer:
    ...
    {
      question: "Do zeros break it too?",
      answer:
    ...
    {
      question: "Can you show me the smallest case where negatives break it?",
      answer:
    ...
    {
      question: "Then why not always use the prefix map, since it handles everything?",
      answer:
    ...
    {
      question: "Why must the prefix map keep the earliest index?",
      answer:
    ...
    {
      question: "I wrote the overwriting version and all my tests passed. How?",
      answer:
    ...
    {
      question: "Why check for a match after the shrink loop rather than inside it?",
      answer:
    ...
    {
      question: "Why is this O(n) when there is a loop inside a loop?",
      answer:
    ...
    {
      question: "What is the binary-search version for, if the window is faster?",
      answer:
  ...
  relatedIds: ["longest-subarray-with-sum-k", "count-subarrays-with-given-sum", "two-sum", "maximum-consecutive-ones"],
};
