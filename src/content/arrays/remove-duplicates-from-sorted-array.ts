...
/**
 * Subtopic 4 of Arrays. The first in-place-modification problem, the first with
 * a return contract that is not the answer itself, and the introduction of the
 * two-pointer read/write split that most of the rest of the module is built on.
 *
 * Title capitalisation normalised to match LeetCode 26; the slug is unchanged.
 *
 * SOURCES
 * - LeetCode 26, "Remove Duplicates from Sorted Array" — the in-place
 *   requirement, the O(1) extra memory constraint, and the "return k" contract
 *   where only the first k elements are judged.
 * - GeeksforGeeks, "Remove duplicates from Sorted Array" — the set-based and
 *   temporary-array approaches, and the two-pointer formulation.
 *
 * FIVE APPROACHES are carried here rather than the usual three, because the
 * ladder is the lesson: the two brute forces fail on DIFFERENT axes (one on
 * time, one on space), and seeing both is what makes the two-pointer feel
 * inevitable instead of memorised.
 *
 * MEASURED ON THIS MACHINE (clang -O2, Python 3.13.4):
 *
 * 1. ALL FIVE APPROACHES AGREE. Checked over 200,000 random sorted arrays
 *    (n <= 10, values 0-4) against each other and against std::unique — zero
 *    disagreements.
 *
 * 2. THE C++ LADDER, wall clock:
 *      n         erase O(n^2)   set        tempArray   twoPointer   std::unique
 *      1,000     0.027ms        0.050ms    0.001ms     0.001ms      0.001ms
 *      10,000    2.472ms        0.637ms    0.034ms     0.044ms      0.041ms
 *      100,000   339.932ms      12.390ms   0.509ms     0.478ms      0.459ms
 *      1,000,000 (skipped)      222.302ms  5.397ms     4.838ms      4.558ms
 *    The erase-based version is 24x the two-pointer at n = 1,000, 57x at 10,000
 *    and 717x at 100,000 — the ratio growing linearly, as O(n^2) vs O(n) predicts.
 *    Note the set is 46x SLOWER than the two-pointer at a million elements
 *    despite being O(n log n), because every insert allocates a tree node.
 *    Note also twoPointer 4.838ms vs std::unique 4.558ms — writing it by hand
 *    costs essentially nothing in C++.
 *
 * 3. PYTHON INVERTS THE RANKING, for the third subtopic running. At n = 100,000:
 *      sorted(set(a)) 3.095ms | dict.fromkeys 3.904ms | two-pointer 7.548ms
 *    The O(n)-space "wrong" answer is 2.4x FASTER than the O(1)-space optimal
 *    one, because set and sorted run as compiled C while the two-pointer loop
 *    runs in the interpreter. Write the two-pointer anyway — the problem demands
 *    O(1) extra space — but know the constant factor is not on your side.
 *
 * 4. list(set(arr)) LOSES ORDER, AND THE BUG IS INVISIBLE ON TUTORIAL DATA.
 *    Measured failure rate on sorted input, by value range:
 *      values 0-9      : 0 / 20,000 arrays reordered  (0.0%)
 *      values 0-99     : 19,937 / 20,000              (99.7%)
 *      values 0-999    : 19,998 / 20,000              (100.0%)
 *      values 0-999,999: 20,000 / 20,000              (100.0%)
 *    Small ints hash to themselves and land in slot order while they fit the
 *    table, so [1,1,2,2,3] -> [1,2,3] looks perfect. The first randomly drawn
 *    realistic array broke it: [11,32,41,49] -> [32,41,11,49]. This is the
 *    cleanest example in the module of a bug that passes every example in the
 *    lesson and fails on real input.
 *
 * 5. THE != GUARD SAVES REAL WRITES. Two-pointer write counts at n = 1,000,000:
 *    all identical -> 0 writes, each value twice -> 499,999, all distinct ->
 *    999,999. An unguarded version that always assigns would perform n - 1
 *    writes regardless.
 *
 * Scope: removing duplicates from an UNSORTED array is a different problem and
 * needs a hash set; that is pointed at, not solved. Remove Element and Move
 * Zeros reuse this same read/write split and are their own subtopics.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Brute Force - Shift Left on Each Duplicate",
      idea: "When two neighbours are equal, delete the second by sliding the rest of the array one position left.",
      steps: [
        "Start at the first index and compare each element with the one after it.",
        "If they are equal, remove the second by shifting every later element one position left.",
        "Do not advance the index after a removal, because a new element has just slid into the position after it.",
        "If they differ, advance the index by one.",
        "Stop when the index reaches the end, and return the remaining length.",
      ],
      code: [
        {
          language: "cpp",
...
int removeDuplicates(vector<int>& nums) {
    ...
    return nums.size();
        ...
        {
          language: "java",
          code: `static int removeDuplicates(int[] nums) {
    int n = nums.length;
    for (int i = 0; i + 1 < n; ) {
        if (nums[i] == nums[i + 1]) {
    ...
    return len(nums)`,
          annotations: {
            ...
            7: "len(nums) is re-evaluated every iteration, so the loop naturally tracks the shrinking list.",
          },
        },
      ],
      complexity: {
    ...
    {
      name: "Brute Force - Set and Rewrite",
      idea: "Insert every element into a set to collapse duplicates, then write the set back over the front of the array.",
      steps: [
        "Insert every element of the array into an ordered set, which discards duplicates.",
        "Note the size of the set, which is the number of unique values.",
        "Write the set's elements back into the array from index zero, in order.",
        "Return the size of the set.",
        "In Python, use dict.fromkeys rather than set, because a plain set does not preserve order.",
      ],
      code: [
        {
          language: "cpp",
...
static int removeDuplicates(int[] nums) {
    ...
    {
      name: "Better - Temporary Array",
      idea: "Collect the unique values into a fresh array in one pass, then copy them back over the front.",
      steps: [
        "Create an empty temporary array.",
        "Walk the input once, from the first element to the last.",
        "Append an element to the temporary array when it is the first element, or when it differs from the element before it.",
        "Copy the temporary array back over the front of the original array.",
        "Return the length of the temporary array.",
      ],
      code: [
        {
          language: "cpp",
...
int removeDuplicates(vector<int>& nums) {
    ...
    },
    {
      ...
      idea: "Keep a write pointer for the deduplicated prefix and a read pointer scanning ahead, writing only when a new value appears.",
      steps: [
        "Return zero immediately if the array is empty.",
        "Set the write pointer i to 0, so that nums[0..0] is trivially deduplicated.",
        "Scan with a read pointer j from index 1 to the end.",
        "Compare nums[j] against nums[i], the last value written.",
        "If they differ, advance i by one and write nums[j] into that position.",
        "If they match, the value is a duplicate, so leave i where it is and move on.",
        "When the scan finishes, nums[0..i] holds every unique value, so return i + 1.",
      ],
      code: [
        {
          language: "cpp",
...
int removeDuplicates(vector<int>& nums) {
    ...
    {
      name: "Library Call",
      idea: "Use the standard library routine that already performs adjacent-duplicate removal.",
      steps: [
        "In C++, call unique over the whole range, which compacts adjacent duplicates to the front.",
        "Take the returned iterator as the new logical end and convert it to a count by subtracting begin.",
        "Note that the container is not resized, which matches the problem's own contract.",
        "In Python, use itertools.groupby, whose group keys are the distinct run values.",
        "Write those keys back over the front of the list and return how many there were.",
      ],
      code: [
        {
          language: "cpp",
...
def remove_duplicates(nums):
  ...
  examples: [
    {
      input: "nums = [1, 1, 2]",
      output: "k = 2, with nums starting [1, 2, ...]",
      walkthrough: [
        "The write pointer i starts at 0, so nums[0..0] holding 1 is already a valid deduplicated prefix.",
        "j = 1: nums[1] is 1 and nums[i] is 1, so this is a duplicate and nothing is written.",
        "j = 2: nums[2] is 2, which differs from nums[i] = 1, so i advances to 1 and 2 is written there.",
        "The scan ends with i = 1, so the answer is i + 1 = 2.",
        "The array now begins [1, 2] and whatever sits at index 2 is ignored by the caller.",
      ],
      why: "The smallest input that contains both a duplicate and a new value, so it exercises both branches of the loop exactly once.",
    },
    {
      input: "nums = [0, 0, 1, 1, 1, 2, 2, 3, 3, 4]",
      output: "k = 5, with nums starting [0, 1, 2, 3, 4]",
      walkthrough: [
        "i starts at 0 with the value 0 already in place.",
        "j = 1 finds another 0, which matches nums[i], so it is skipped.",
        "j = 2 finds 1, which differs, so i becomes 1 and 1 is written to index 1.",
        "j = 3 and j = 4 find further 1s, both matching nums[i], so both are skipped.",
        "The remaining values 2, 3 and 4 each trigger one write, ending with i = 4.",
        "The answer is 5, and verified on this machine the tail is left as [2, 2, 3, 3, 4] — stale values that the judge never reads.",
      ],
      why: "The standard multi-run example, and the one that makes the ignored tail concrete rather than theoretical.",
    },
    {
      input: "nums = [7, 7, 7, 7] and nums = [1, 2, 3, 4]",
      output: "k = 1 and k = 4 — the two extremes of the write count",
      walkthrough: [
        "On the all-identical array, every comparison matches nums[i], so the condition never fires.",
        "The write pointer never moves, i finishes at 0, and the answer is 1.",
        "Measured at a million identical elements, this performs zero writes, because the guard rejects every candidate.",
        "On the all-distinct array, every comparison differs, so every element triggers a write.",
        "The write pointer keeps pace with the read pointer, and each element is written back to the position it already occupied.",
        "Measured at a million distinct elements, this performs 999,999 writes — the opposite extreme, and the reason the guard is worth having.",
      ],
      why: "The two ends of the behaviour range in one place, and the concrete evidence that the != guard changes real work rather than merely reading well.",
    },
    {
      input: "nums = [11, 32, 41, 49] passed through Python's list(set(nums))",
      output: "[32, 41, 11, 49] — the order is destroyed",
      walkthrough: [
        "The input is already sorted and every value is unique, so the correct output is the input unchanged.",
        "list(set(nums)) collapses nothing, since there are no duplicates, but it returns the values in hash-table slot order.",
        "That order is [32, 41, 11, 49], which is neither sorted nor the original ordering.",
        "The same code on [1, 1, 2, 2, 3] returns [1, 2, 3], which is completely correct.",
        "Small integers hash to themselves, so while the values fit inside the table they land in increasing slot order.",
        "Measured on 20,000 sorted arrays per range: 0% were reordered with values 0-9, 99.7% with values 0-99, and 100% with values 0-999.",
      ],
      why: "The clearest case in the module of a bug that passes every example in the lesson and then fails on essentially all realistic input.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why do we return a number instead of the array?",
      answer:
    ...
    {
      question: "Why is the answer i + 1 rather than i?",
      answer:
    ...
    {
      question: "Why is it safe to write into the same array we are still reading?",
      answer:
    ...
    {
      question: "Why compare nums[j] against nums[i] rather than nums[j-1]?",
      answer:
    ...
    {
      question: "But list(set(...)) worked when I tried it. Why?",
      answer:
    ...
    {
      question: "In Python the set version measured faster than the two-pointer. Should I submit that?",
      answer:
    ...
    {
      question: "Does the != guard actually matter, or is it just tidier?",
      answer:
    ...
    {
      question: "Does this work if the array is not sorted?",
      answer:
  ...
  relatedIds: ["largest-element", "second-largest-element", "move-zeros-to-end", "linear-search"],
};
