...
/**
 * Subtopic 3 of Arrays. The first problem where the array is treated as a RING
 * rather than a line, and the first where the answer is a property of the whole
 * array rather than a value drawn from it.
 *
 * Renamed from "Check if the Array is Sorted II", which named a sequence position
 * rather than a problem. This is LeetCode 1752, and its own title says what is
 * actually being asked.
 *
 * SOURCES
 * - LeetCode 1752, "Check if Array Is Sorted and Rotated" — the problem
 *   statement, the rotation definition B[i] == A[(i + x) % n], and the three
 *   worked examples reproduced here.
 * - GeeksforGeeks, "Check if an array is sorted and rotated" — the break-counting
 *   formulation.
 *
 * MEASURED ON THIS MACHINE (clang -O2, Python 3.13.4):
 *
 * 1. THE ONE-BREAK RULE IS PROVEN, NOT ASSUMED. The circular break count was
 *    checked against a definitional brute force that literally tries all n
 *    rotations:
 *      - Python : all 5,460 arrays of length 1-6 over values 1-4 -> 0 mismatches.
 *      - C++    : all 9,840 arrays of length 1-8 over values 1-3 -> 0 mismatches.
 *      - C++    : 200,000 random arrays, n <= 12, values 1-100, one third of them
 *                 genuinely sorted-and-rotated                    -> 0 mismatches.
 *    Three independent corpora, 215,300 arrays, zero disagreements.
 *
 * 2. BOTH CANDIDATE BUGS WERE MEASURED, NOT GUESSED, over the 5,460-array corpus:
 *      - Using >= instead of > : 916 wrong answers (16.8%). Smallest
 *        counterexample [1, 1], where it returns false and the answer is true.
 *      - Omitting the wrap-around comparison: 1,046 wrong answers (19.2%).
 *        Smallest counterexample [1, 3, 2], where it returns true and the answer
 *        is false.
 *    Both fail on roughly one small array in six, so unlike many silent bugs
 *    these DO get caught by even casual testing — which is worth saying plainly
 *    rather than implying every bug hides.
 *
 * 3. BRUTE FORCE IS FINE AT THE STATED CONSTRAINT AND NOWHERE ELSE. Measured on
 *    a valid rotation: n = 100 -> 3.2us vs 0.08us (38x); n = 1,000 -> 257.1us vs
 *    0.79us (325x); n = 10,000 -> 25,510.9us vs 7.67us (3,327x). The ratio grows
 *    linearly with n exactly as O(n^2) against O(n) predicts. At the problem's
 *    own limit of n <= 100 the brute force passes comfortably, which is why this
 *    subtopic argues for the linear solution on reasoning rather than on urgency.
 *
 * Scope: searching a rotated sorted array, and finding the rotation index by
 * binary search, are Binary Search material. This container establishes the ring
 * view and the break count, and points there rather than going further.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Brute Force - Try Every Rotation",
      idea: "Generate each of the n rotations and check whether any one of them is sorted.",
      steps: [
        "For each possible rotation amount x from 0 to n - 1, consider the array that rotation produces.",
        "Read that rotated array from its first element to its last.",
        "Check whether every element is less than or equal to the next.",
        "If any rotation passes that check, the array is sorted and rotated, so return true.",
        "If every rotation fails, return false.",
      ],
      code: [
        {
          language: "cpp",
...
bool check(const vector<int>& nums) {
    ...
    return false;
          ...
          annotations: {
            ...
            12: "Measured 3.2us at n = 100 and 25,510.9us at n = 10,000 — the 100x input cost about 8,000x the time.",
          },
        },
        {
          language: "java",
          code: `static boolean check(int[] nums) {
    int n = nums.length;
    for (int x = 0; x < n; x++) {
        boolean sorted = true;
        for (int i = 0; i + 1 < n; i++) {
    ...
    {
      name: "Count Breaks Around the Ring",
      idea: "Count the positions where order falls, treating the last element as adjacent to the first.",
      steps: [
        "Set a counter of breaks to zero.",
        "Visit every index i from 0 to n - 1.",
        "Compare the element at i against the element at (i + 1) % n, so the last index wraps to the first.",
        "If the current element is strictly greater than its circular neighbour, increment the break counter.",
        "After the full circle, return whether the counter is at most one.",
      ],
      code: [
        {
          language: "cpp",
...
def check_pairwise(nums):
    ...
    {
      name: "Early Exit on the Second Break",
      idea: "Stop the moment a second break appears, since no further evidence can change the answer.",
      steps: [
        "Set a counter of breaks to zero.",
        "Visit every index i from 0 to n - 1, comparing against the circular neighbour as before.",
        "On each break, increment the counter.",
        "As soon as the counter reaches two, return false immediately without examining the rest.",
        "If the circle completes with at most one break, return true.",
      ],
      code: [
        {
          language: "cpp",
...
bool check(const vector<int>& nums) {
    ...
    return true;
          ...
          annotations: {
            ...
            11: "Reached whenever zero or one break was found, which covers sorted, rotated and all-equal alike.",
          },
        },
        {
          language: "java",
  ...
  examples: [
    {
      ...
      output: "true",
      walkthrough: [
        "i = 0: 3 against 4 climbs, so no break.",
        "i = 1: 4 against 5 climbs, so no break.",
        "i = 2: 5 against 1 falls, which is the first break — this is the rotation point.",
        "i = 3: 1 against 2 climbs, so no break.",
        "i = 4: the last element wraps to the first, comparing 2 against 3, which climbs and is not a break.",
        "One break in total, so the array is a rotation of a sorted array and the answer is true.",
        "Cutting immediately after the break and reading round gives 1, 2, 3, 4, 5 — the original sorted array, rotated by 2.",
      ],
      why: "The statement's own first example, and it shows the break sitting exactly at the rotation point while the wrap-around comparison passes cleanly.",
    },
    {
      input: "nums = [2, 1, 3, 4]",
      output: "false",
      walkthrough: [
        "i = 0: 2 against 1 falls, which is the first break.",
        "i = 1: 1 against 3 climbs, so no break.",
        "i = 2: 3 against 4 climbs, so no break.",
        "i = 3: the last element wraps to the first, comparing 4 against 2, which falls — the second break.",
        "Two breaks means the circle falls twice, and a sorted array read round a circle can fall only once.",
        "The answer is false, and no rotation of any sorted array produces this input.",
      ],
      why: "The statement's negative example, and the wrap-around comparison is what supplies the decisive second break — a scan stopping at n-2 would find only one and answer true.",
    },
    {
      input: "nums = [1, 2, 3]",
      output: "true",
      walkthrough: [
        "i = 0: 1 against 2 climbs, so no break.",
        "i = 1: 2 against 3 climbs, so no break.",
        "i = 2: the last element wraps to the first, comparing 3 against 1, which falls — one break.",
        "A fully sorted array therefore has exactly one break, located at the wrap.",
        "One break is within the allowance, so the answer is true, which matches rotation by zero.",
        "This is why the sorted case needs no special handling: the rule already covers it.",
      ],
      why: "Shows that an unrotated array still registers a break at the wrap, which is the detail that convinces people the allowance of one break is principled rather than a fudge.",
    },
    {
      input: "nums = [1, 1] and nums = [1, 3, 2], run against the two buggy variants",
      output: "The buggy versions answer false and true respectively — both wrong",
      walkthrough: [
        "On [1, 1] the correct answer is true, since equal neighbours are non-decreasing and rotation by zero works.",
        "A version using >= instead of > counts 1 >= 1 as a break at both positions, totals two, and returns false.",
        "Measured over all 5,460 arrays of length 1 to 6 with values 1 to 4, that mistake produced 916 wrong answers, or 16.8%.",
        "On [1, 3, 2] the correct answer is false, because no rotation of a sorted array produces it.",
        "A version that scans only to n - 2 finds the single break at 3 against 2, never compares 2 back to 1, and returns true.",
        "That mistake produced 1,046 wrong answers over the same corpus, or 19.2%.",
      ],
      why: "These are the two smallest inputs that expose the two ways this algorithm is normally written wrong, and both failure rates were measured rather than estimated.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why is [1, 2, 3] true when it was never rotated?",
      answer:
    ...
    {
      question: "Why must I compare the last element back to the first?",
      answer:
    ...
    {
      question: "Why is the test > and not >=?",
      answer:
    ...
    {
      question: "Why is at most one break the right rule, rather than something I just have to remember?",
      answer:
    ...
    {
      question: "How do you know the break-counting rule is actually equivalent to the definition?",
      answer:
    ...
    {
      question: "Is zero breaks also valid, or does it have to be exactly one?",
      answer:
    ...
    {
      question: "Should I use the early-exit version?",
      answer:
    ...
    {
      question: "The constraints say n is at most 100, so why not just try every rotation?",
      answer:
    ...
    {
      question: "How does this connect to searching a rotated sorted array?",
      answer:
  ...
  relatedIds: ["largest-element", "second-largest-element", "left-rotate-array-by-k-places"],
};
