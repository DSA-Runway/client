...
/**
 * Subtopic 1 of Arrays — the first problem after Basics, and the one that
 * establishes the pattern the next twenty problems are built on.
 *
 * Rewritten from the original template to match the depth of the Basics module.
 *
 * SOURCES
 * - GeeksforGeeks, "Find the largest three distinct elements in an array" and
 *   "Program to find largest element in an Array" — the sort-based and
 *   single-traversal approaches, and the O(n) vs O(n log n) framing.
 *
 * MEASURED ON THIS MACHINE (clang -O2, Python 3.13.4), and it both CONFIRMED and
 * SHARPENED the usual "sorting is about 20x worse" claim:
 *
 * 1. COMPARISONS, counted with an instrumented comparator at n = 1,000,000:
 *    the scan performs 999,999 comparisons, std::sort performs 22,224,069.
 *    Ratio 22.2x. So "roughly 20x" is right — but only for comparisons.
 *
 *    The ratio is not a constant, it grows with n. Measured 11.2x at n = 1e3,
 *    19.0x at n = 1e5, 22.2x at n = 1e6. std::sort uses about 1.115 * n log2(n)
 *    comparisons, so the ratio is almost exactly 1.1 * log2(n) — predicted
 *    11.1 / 18.5 / 22.2 against measured 11.2 / 19.0 / 22.2. Teaching the
 *    formula is more useful than teaching one number.
 *
 * 2. WALL CLOCK is worse than the comparison count suggests: 0.573ms vs
 *    32.866ms at n = 1e6, a 57x gap, because sorting also MOVES elements while
 *    the scan only reads them. Quoting 20x understates the real cost.
 *
 * 3. THE LIBRARY-VS-LOOP CONSTANT RUNS OPPOSITE IN C++ AND PYTHON, which is the
 *    finding that earns its place here:
 *      - C++  : max_element 0.638ms vs hand loop 0.575ms — effectively identical,
 *               both compile to the same machine code. Writing it out costs nothing.
 *      - Python: max() 10.179ms vs hand loop 40.530ms — the BUILTIN IS 4x FASTER,
 *               measured at 4.0x / 4.2x / 4.0x across n = 1e3 / 1e5 / 1e6.
 *               Same O(n), because the loop pays interpreter overhead per element
 *               while max() runs as compiled C.
 *    So "write the loop yourself to show you understand it" is free advice in
 *    C++ and a 4x tax in Python. No single-language source can state this.
 *
 * 4. EMPTY INPUT FAILS THREE DIFFERENT WAYS. Verified: max_element on an empty
 *    vector returns end() and dereferencing it is undefined behaviour with no
 *    diagnostic; Python raises ValueError("max() iterable argument is empty").
 *    Java's Arrays.stream(arr).max() returns an empty OptionalInt whose
 *    getAsInt() throws NoSuchElementException — documented, NOT measured here,
 *    as no JRE is installed on this machine.
 *
 * 5. SORTING MUTATES THE CALLER. Verified in C++: passing vector<int>& and
 *    sorting left the caller's vector reordered to [1,2,3]. Python's arr.sort()
 *    mutates and returns None; sorted(arr) copies. This is the first place the
 *    pass-by-reference lesson from Basics has real consequences.
 *
 * Scope: returning the INDEX of the maximum, and the second largest, are their
 * own subtopics. Both are pointed at rather than solved here.
 */
  ...
  approaches: [
    {
      ...
      idea: "Sort the array in ascending order, then read the last element.",
      steps: [
        "Sort the array in ascending order.",
        "The largest element is now at index n - 1.",
        "Read and return that element.",
        "If the caller's array must survive, sort a copy instead of the original.",
      ],
      code: [
        {
          language: "cpp",
...
# biggest = arr.sort()   ->  biggest is None, because sort() returns nothing`,
    ...
    },
    {
      ...
      idea: "Carry the best value seen so far, updating it whenever an element beats it.",
      steps: [
        "Set the candidate to arr[0], so it starts as a real element of the array.",
        "Start the scan at index 1, since comparing arr[0] against itself can change nothing.",
        "Compare the current element against the candidate.",
        "If the element is strictly greater, it becomes the new candidate.",
        "Advance to the next index and repeat until the array is exhausted.",
        "Return the candidate, which no element exceeded and which is itself an element of the array.",
      ],
      code: [
        {
          language: "cpp",
...
int largestElement(const vector<int>& arr) {
    ...
    return maxi;
        ...
        {
          language: "java",
          code: `static int largestElement(int[] arr) {
...
def largest_element_no_copy(arr):
    ...
    },
    {
      ...
      idea: "Call the language's built-in maximum, and know what it costs relative to writing the loop.",
      steps: [
        "Confirm the array is non-empty, because every library version fails on empty input.",
        "Call the language's maximum routine over the whole array.",
        "In C++ dereference the returned iterator, having checked it is not end().",
        "In Java unwrap the OptionalInt, which throws if the stream was empty.",
        "In Python call max directly, which raises ValueError on an empty iterable.",
      ],
      code: [
        {
          language: "cpp",
...
def largest_element_safe(arr):
  ...
  examples: [
    {
      ...
      output: "6",
      walkthrough: [
        "Seed the candidate with arr[0], so it starts at 3.",
        "i = 1: arr[1] is 3, which is not strictly greater than 3, so the candidate stays 3.",
        "i = 2: arr[2] is 6, which beats 3, so the candidate becomes 6.",
        "i = 3: arr[3] is 1, which does not beat 6, so the candidate stays 6.",
        "The scan is finished and the candidate holds 6.",
        "Three comparisons were made for four elements, which is the n - 1 the analysis predicts.",
      ],
      why: "The duplicate of the seed value at index 1 is the case that shows > and >= return the same answer, differing only in which occurrence they latch onto.",
    },
    {
      input: "arr = [-7, -2, -9]",
      output: "-2",
      walkthrough: [
        "Seed the candidate with arr[0], so it starts at -7.",
        "i = 1: arr[1] is -2, which is greater than -7, so the candidate becomes -2.",
        "i = 2: arr[2] is -9, which does not beat -2, so the candidate stays -2.",
        "Return -2, which is an actual element of the array.",
        "Now run the same input against a solution that seeds max = 0.",
        "No element is greater than 0, so that version returns 0 — a value that never appears in the input.",
      ],
      why: "The all-negative array is the single test case that separates a correct seed from the max = 0 bug, and it is exactly the case that casual testing skips.",
    },
    {
      input: "arr = [5]",
      output: "5",
      walkthrough: [
        "Seed the candidate with arr[0], so it holds 5.",
        "The loop starts at i = 1 and the condition 1 < 1 is immediately false.",
        "The body never runs and zero comparisons are performed.",
        "Return 5, which was correct the moment it was seeded.",
        "This is the for-loop rule from Basics doing real work: a loop whose condition starts false runs zero times, so no special case is needed.",
      ],
      why: "Proves the single-element case needs no guard at all, which is the payoff of seeding from arr[0] rather than looping from index 0.",
    },
    {
      input: "An empty array, passed to each language's library call",
      output: "Three different failures — one of them silent",
      walkthrough: [
        "Python evaluates max([]) and raises ValueError with the message 'max() iterable argument is empty'.",
        "Java evaluates Arrays.stream(arr).max() to an empty OptionalInt, and getAsInt() then throws NoSuchElementException.",
        "C++ evaluates max_element(begin, end) to end() itself, which is verified behaviour on an empty vector.",
        "Dereferencing that iterator is undefined behaviour, and in practice it usually does not crash.",
        "It reads whatever memory sits past the end of the container and returns it as a plausible-looking integer.",
        "So two languages hand you a named exception and the third hands you a wrong answer that looks right.",
      ],
      why: "The empty case is where the three languages diverge most sharply, and the C++ silent-success path is the one that reaches production undetected.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why do we start the loop at i = 1 instead of i = 0?",
      answer:
    ...
    {
      question: "Why not just initialise max = 0?",
      answer:
    ...
    {
      question: "Sorting is only O(n log n). Is one pass really worth the trouble?",
      answer:
    ...
    {
      question: "Does it matter whether I use > or >=?",
      answer:
    ...
    {
      question: "Should I just call max() or max_element() instead of writing the loop?",
      answer:
    ...
    {
      question: "What should happen if the array is empty?",
      answer:
    ...
    {
      question: "What if the largest element appears more than once?",
      answer:
    ...
    {
      question: "Can this be done with recursion?",
      answer:
    ...
    {
      question: "Is there any way to beat O(n)?",
      answer:
  ...
  relatedIds: ["second-largest-element", "maximum-consecutive-ones", "kadanes-algorithm"],
};
