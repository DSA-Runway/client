...
<!-- @approach -->
### Brute Force - General-Purpose Sort

<!-- @idea -->
Ignore that there are only three values and hand the array to a comparison sort.

<!-- @steps -->
1. Call the language's sort routine on the whole array.
2. Accept O(n log n) comparisons for a problem that has a linear solution.
3. Note that this is the baseline the specialised approaches must beat.
...
<!-- @idea -->
Count how many of each value there are, then overwrite the array with that many of each in order.

<!-- @steps -->
1. Create three counters, one per possible value.
2. Walk the array once, incrementing the counter for each element.
3. Walk the array again, writing that many 0s, then that many 1s, then that many 2s.
4. Each cell is written exactly once, giving n writes in total.
...
- 9: Both passes are strictly sequential, which is why the per-element cost measured FLAT at ~1.23ns from a thousand to four million elements.
...
<!-- @idea -->
Maintain three pointers so that the array is always sorted except for a shrinking unknown middle region.

<!-- @steps -->
1. Set low and mid to the start and high to the last index.
2. While mid has not passed high, inspect the element at mid.
3. If it is 0, swap it with the element at low, then advance both low and mid.
4. If it is 1, it is already in the correct region, so just advance mid.
5. If it is 2, swap it with the element at high and decrement high — but do NOT advance mid, because the value swapped in has not been examined.
6. Stop when mid passes high, at which point the unknown region is empty.
...
- 15: Two swaps per element in the worst case, giving 2.00n writes — measured 1,997,484 at n = 1,000,000.
...
- 12: No mid++ on this branch — the omission is deliberate and is the correctness of the algorithm.
...
<!-- @idea -->
Partition the 0s to the front, then partition the 1s to the front of what remains.

<!-- @steps -->
1. Partition the whole array so that every 0 comes before every non-0.
2. Note where that partition ended.
3. Partition the remainder so that every 1 comes before every 2.
4. The array is now sorted, since the three groups are in order.
...
<!-- @why -->
The opening move is a 2-swap, which is exactly the case where advancing mid would be wrong — so the trace exercises the algorithm's one subtle rule on its first step.

<!-- @walkthrough -->
1. Start with low = 0, mid = 0 and high = 5.
2. nums[0] is 2, so swap it with nums[5] and drop high to 4, giving [0,0,2,1,1,2] — and mid stays at 0, because the incoming 0 has not been examined.
3. nums[0] is now 0, so swap it with nums[low], which is itself, and advance both low and mid to 1.
4. nums[1] is 0, so again a self-swap, and low and mid advance to 2.
5. nums[2] is 2, so swap it with nums[4] and drop high to 3, giving [0,0,1,1,2,2] — mid stays at 2.
6. nums[2] is now 1, so mid advances to 3; nums[3] is 1, so mid advances to 4.
7. mid is now 4 and high is 3, so mid has passed high and the loop ends with the array sorted.
8. Note the very first step: had mid advanced after that swap, the 0 that arrived at index 0 would never have been examined.

<!-- @example -->

<!-- @input -->
nums = [1, 2, 0] with mid advanced after the 2-swap

<!-- @output -->
[1, 0, 2] — not sorted

<!-- @why -->
Three elements are enough to expose it, and the failure rate of 63% means almost any random test would catch it — which makes it a bug of understanding rather than one that hides.

<!-- @walkthrough -->
1. low = 0, mid = 0, high = 2. nums[0] is 1, so mid advances to 1.
2. nums[1] is 2, so it is swapped with nums[2], giving [1,0,2], and high drops to 1.
3. The correct algorithm leaves mid at 1, re-examines the 0 that just arrived, and moves it to the front.
4. The buggy version advances mid to 2, which is now greater than high, so the loop ends immediately.
5. The result is [1,0,2], which is not sorted, and the 0 was never looked at.
6. Measured over all 29,524 arrays of length up to 9, that single extra increment produced 18,640 wrong answers — 63.14%.

<!-- @example -->

<!-- @input -->
nums = [1, 0] with the loop written as while (mid < high)

<!-- @output -->
[1, 0] — completely unchanged

<!-- @why -->
The smallest possible input for this bug, and it shows the boundary case precisely: when mid meets high there is still exactly one unclassified element sitting there.

<!-- @walkthrough -->
1. low = 0, mid = 0 and high = 1, so mid < high is true and the loop body runs once.
2. nums[0] is 1, so mid advances to 1.
3. Now mid equals high, so the condition mid < high is false and the loop exits.
4. The element at index 1 was never examined, and the array is returned unsorted.
5. With mid <= high the loop runs once more, sees the 0, swaps it to the front, and returns [0,1].
6. Measured, that off-by-one produced 9,330 wrong answers out of 29,524 — 31.60%.

<!-- @example -->

<!-- @input -->
The same uniform array at sizes spanning the L1 cache boundary

<!-- @output -->
DNF 0.769ns per element at n = 16,384 and 3.330ns at n = 65,536

<!-- @why -->
The crossover is located precisely rather than asserted, and counting's size-independence is what proves the explanation is about memory rather than about instruction count.

<!-- @walkthrough -->
1. At n = 16,384 the array occupies 64 KB, which fits inside this machine's 128 KB L1 data cache.
2. The Dutch National Flag algorithm costs 0.769 nanoseconds per element there, comfortably beating counting's 1.248.
3. At n = 65,536 the array occupies 256 KB and no longer fits, and the same code costs 3.330 nanoseconds per element — a 4.3x jump.
4. Counting sort, measured across the same range, stays flat at roughly 1.23 nanoseconds per element from a thousand elements to four million.
5. That flat line is the evidence: counting makes two sequential passes that the prefetcher streams regardless of size.
6. The Dutch National Flag walks from both ends at once, so its working set is the whole array and it is only fast while that array is L1-resident.
...
<!-- @doubt -->
### Why doesn't mid advance when the element is a 2?
...
<!-- @doubt -->
### Why is the loop condition mid <= high and not mid < high?
...
<!-- @doubt -->
### How do I remember the three rules?
...
<!-- @doubt -->
### Isn't one pass always better than two?
...
<!-- @doubt -->
### Why does the Dutch National Flag get slower on large arrays?
...
<!-- @doubt -->
### So which one should I actually write?
...
<!-- @doubt -->
### Can I use this for more than three values?
...
<!-- @doubt -->
### Does the three-way partition show up anywhere else?
...
<!-- @doubt -->
### Why does std::sort beat the Dutch National Flag at scale?
