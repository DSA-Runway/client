...
<!-- @answer -->
The loop form carries over completely, including the lexicographic property, with the duplicate-skip line from Combination Sum II added inside the loop. The mask forms do not: they have no notion of a level, so `i > start && a[i] == a[i-1]` has nothing to attach to, and deduplicating counting or gray order needs a different mechanism entirely — typically canonicalising each subset and filtering, which is the approach Combination Sum II measured at up to 529.90x worse. Unranking also breaks, because with duplicates the number of *distinct* subsets is no longer 2^n and the subtree sizes are no longer 2^(n-1-i). So of the four approaches here, exactly one survives the addition of duplicates, which is a reasonable argument for it being the default.
