...
/**
 * Subtopic 4 of Basics — deliberately placed after data-types and
 * variables-and-constants, because reading input requires somewhere to put it.
 *
 * The lesson that actually matters here is the input buffer. Every one of the
 * three languages has a signature I/O bug (cin >> then getline, nextInt then
 * nextLine, forgetting int() on input) and all three are the same underlying
 * misunderstanding about what is left in the buffer after a read.
 */
  ...
  approaches: [
    {
      ...
      idea: "Send values to standard output, with control over spacing and line breaks.",
      steps: [
        "Choose the printing construct for your language.",
        "Pass the value or values you want displayed.",
        "Insert separators explicitly where you need spaces between values.",
        "Decide whether a newline should follow, and add or suppress it accordingly.",
        "Print only what is asked for — extra text becomes part of your output.",
      ],
      code: [
        {
          language: "cpp",
...
print(f"{a} + {b} = {a + b}")   # 5 + 9 = 14`,
    ...
    },
    {
      ...
      idea: "Pull one value from standard input into a variable of the right type.",
      steps: [
        "Declare a variable of the type you expect to read.",
        "Call the language's read operation, which pauses until input is available.",
        "The read consumes the characters that make up the value and stops at whitespace.",
        "Convert the text to a number if the language does not do it for you.",
        "The newline the user typed remains in the buffer for the next read.",
      ],
      code: [
        {
          language: "cpp",
...
String fullLine = sc.nextLine();  // now reads the whole line`,
        ...
        {
          language: "python",
          code: `n = int(input())         # input() returns a string — int() converts it
price = float(input())
...
# print(n + n)   ->  "55" instead of 10, because n is a string`,
    ...
    {
      name: "Reading Multiple Values and Arrays",
      idea: "The standard DSA input shape: a count, then that many values.",
      steps: [
        "Read the count n first.",
        "Create a container sized to hold n values.",
        "Loop n times, reading one value per iteration into the container.",
        "In Python, split the line into pieces and convert them all at once instead of looping.",
        "The values may arrive on one line or many — whitespace-separated reads do not care.",
      ],
      code: [
        {
          language: "cpp",
...
arr = [int(input()) for _ in range(n)]`,
  ...
  examples: [
    {
      input: "Two numbers on one line: 5 9",
      output: "14",
      walkthrough: [
        "The program reads the first value, consuming the characters 5 and stopping at the space.",
        "It reads the second value, skipping the space and consuming 9, stopping at the newline.",
        "Both values are now stored as integers in their variables.",
        "The sum 5 + 9 is computed, producing 14.",
        "14 is written to standard output followed by a newline.",
      ],
      why: "The complete input-process-output cycle in its smallest useful form.",
    },
    {
      input: "A count followed by that many values:\n4\n3 7 1 9",
      output: "The four values stored in an array",
      walkthrough: [
        "The first read consumes 4 and stops at the newline, so n is 4.",
        "A container sized 4 is created.",
        "The loop reads 3, then 7, then 1, then 9 — each read skipping the whitespace before it.",
        "The leftover newline after 4 causes no trouble here, because numeric reads skip whitespace automatically.",
        "The array now holds all four values and the input is fully consumed.",
      ],
      why: "This is the input format of nearly every DSA problem, so the pattern is worth having automatic.",
    },
    {
      input: "Read an int, then attempt to read a full line:\n5\nhello world",
      output: "The line comes back empty instead of 'hello world'",
      walkthrough: [
        "The numeric read consumes 5 and stops, leaving the newline sitting in the buffer.",
        "The line read begins at the cursor, which is parked directly on that newline.",
        "It sees an end-of-line immediately and concludes the line is finished.",
        "It returns an empty string without ever reaching the text on the next line.",
        "Discarding the leftover newline first — cin.ignore() in C++, an extra sc.nextLine() in Java — makes the following read behave as expected.",
      ],
      why: "The single most reported I/O bug in both C++ and Java, and it is invisible until you picture the buffer cursor.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why does Python treat my input as a string when I typed a number?",
      answer:
    ...
    {
      question: "Why does my nextLine() in Java get skipped after nextInt()?",
      answer:
    ...
    {
      question: "Why does cin stop reading at the space when I enter a full name?",
      answer:
    ...
    {
      question: "Should I use endl or a newline character?",
      answer:
    ...
    {
      question: "Scanner or BufferedReader in Java?",
      answer:
    ...
    {
      question: "How do I read a whole array in one go?",
      answer:
    ...
    {
      question: "My program works on my machine but fails on the online judge. Why?",
      answer:
    ...
    {
      question: "My solution is correct but times out on large input. Is I/O the problem?",
      answer:
  ...
  relatedIds: ["data-types", "variables-and-constants", "type-conversion-and-casting", "arithmetic-operators"],
};
