...
<!-- @idea -->
Let the language promote operands automatically when the conversion cannot lose anything.

<!-- @steps -->
1. Write an expression whose operands have different types.
2. The language finds the wider of the two types.
3. The narrower operand is promoted to that wider type.
4. The operation runs with both operands now at the same type.
5. The result carries the wider type, which may need a wider variable to hold it.
...
<!-- @annotations -->
- 7: Python promotes int to float automatically, the same way C++ and Java do.
- 12: Deliberate refusal. Guessing between "55" and 10 would be wrong half the time.

<!-- @approach -->
### Explicit Casting and Where to Put It

<!-- @idea -->
Force a conversion the language will not do for you — and place it before the operation, not after.

<!-- @steps -->
1. Identify the operation that is running at the wrong type.
2. Determine which operand needs converting so the operation itself changes behaviour.
3. Apply the cast to that operand, not to the finished result.
4. The other operand is promoted automatically to match.
5. Confirm the operation now runs at the intended type by checking a case where the two would differ.
...
<!-- @idea -->
Parse text into a number, or render a number as text — the conversion every program doing input and output needs.

<!-- @steps -->
1. Confirm the direction: text into a number, or a number into text.
2. Call the parsing function for the target numeric type.
3. Handle the failure case, since text that is not a valid number cannot be parsed.
4. For the reverse direction, call the language's string conversion on the number.
5. To parse a decimal string into an integer, convert to a floating-point value first, then narrow.
...
<!-- @input -->
sum = 7, count = 2; compute (double)(sum / count) and (double) sum / count

<!-- @output -->
3.0 and 3.5

<!-- @why -->
The single most common 'but I already cast it' bug. Seeing both forms side by side is what makes the ordering rule stick.

<!-- @walkthrough -->
1. In the first form, the parentheses force sum / count to be evaluated first.
2. Both are integers, so integer division runs and produces 3, discarding the half.
3. The cast then converts the integer 3 into the double 3.0 — faithfully, but too late.
4. In the second form, the cast applies to sum alone, making it 7.0 before any division happens.
5. count is promoted to 2.0 to match, the division runs in floating point, and 3.5 survives.
6. The same characters in a different order produce different answers, because conversion happens at a point in time.

<!-- @example -->

<!-- @input -->
Cast 5.9 and -5.9 to int, and compare against floor division

<!-- @output -->
5 and -5, where floor division would give 5 and -6

<!-- @why -->
Truncation and flooring are routinely confused, and they only disagree on negatives — exactly where such bugs are hardest to notice.

<!-- @walkthrough -->
1. Casting 5.9 to an integer discards the fractional part, leaving 5.
2. Casting -5.9 discards the fraction too, leaving -5 — moving the value toward zero.
3. Floor division instead rounds down the number line, so -5.9 becomes -6.
4. For positive values both approaches agree, which is why the difference goes unnoticed.
5. For negative values they differ by one, and that is enough to break an index calculation.

<!-- @example -->

<!-- @input -->
Cast the int 300 to a byte in Java

<!-- @output -->
44

<!-- @why -->
Shows that narrowing wraps rather than clamping, and that writing the cast means you took responsibility for the range check.

<!-- @walkthrough -->
1. 300 in binary is 100101100, which needs nine bits.
2. A byte holds only eight bits, so the value does not fit.
3. The narrowing cast keeps the low eight bits and discards everything above them.
4. The remaining bits, 00101100, are read as the value 44.
5. No error or warning is produced — the cast was your explicit instruction that this was acceptable.
...
<!-- @doubt -->
### I cast my division to double and still got a whole number. Why?
...
<!-- @doubt -->
### Why does int(-5.9) give -5 when I expected -6?
...
<!-- @doubt -->
### Why do I need a cast to store a double in an int, but not the reverse?
...
<!-- @doubt -->
### Should I use static_cast or a C-style cast in C++?
...
<!-- @doubt -->
### Python has no declared types. Why does it need conversion?
...
<!-- @doubt -->
### My long turned into a wrong number when I stored it in a float. How?
