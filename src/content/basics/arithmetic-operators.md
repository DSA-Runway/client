...
<!-- @idea -->
Apply +, -, *, / and % to two values, and see where division stops behaving like maths class.

<!-- @steps -->
1. Choose two operands and the operator to apply.
2. For addition, subtraction and multiplication, the result is what ordinary arithmetic gives.
3. For division, check the operand types first — two integers produce an integer result.
4. If a fractional result is needed, make at least one operand a floating-point value.
5. For modulo, the result is what remains after removing as many whole multiples as possible.
...
<!-- @idea -->
See exactly where the three languages diverge, and how to write index arithmetic that survives it.

<!-- @steps -->
1. Compute the exact mathematical quotient, which will be a fractional value.
2. Apply the language's rounding rule: C++ and Java truncate toward zero, Python floors toward negative infinity.
3. Derive the remainder from the identity that quotient times divisor plus remainder equals the original value.
4. Note the sign: the remainder follows the dividend in C++ and Java, and the divisor in Python.
5. When wrapping an index backwards, add the array size and take the modulo again in C++ and Java.

<!-- @code cpp -->
```cpp
cout << -7 / 2 << endl;   // -3   truncated toward zero
...
- 8: A negative index. Used directly to access an array, this reads out of bounds.
- 9: Adding n makes it positive, and the second modulo brings it back into range.
...
<!-- @idea -->
Shorthand for the most common update of all — changing a variable based on its current value.

<!-- @steps -->
1. Identify a variable that should be updated using its own current value.
2. For adding or subtracting one, use the increment or decrement operator where the language has it.
3. Choose prefix when the new value should be used in the surrounding expression.
4. Choose postfix when the old value should be used before the change takes effect.
5. For any other update, use a compound assignment such as plus-equals.
...
<!-- @input -->
Compute 7 / 2 and 7 % 2 with both operands as integers

<!-- @output -->
Quotient 3, remainder 1

<!-- @why -->
The most reported beginner surprise in every language, and the reason averages computed with / come out wrong.

<!-- @walkthrough -->
1. The exact mathematical result of 7 divided by 2 is 3.5.
2. Both operands are integers, so the language performs integer division and the result must be an integer.
3. The fractional part is discarded, leaving 3. No rounding occurs — 3.9 would also become 3.
4. The remainder is what is left after removing three whole 2s from 7, which is 1.
5. Checking the identity: 3 times 2 plus 1 equals 7, so the pair is consistent.

<!-- @example -->

<!-- @input -->
Compute -7 divided by 2 and -7 modulo 2 in C++ and in Python

<!-- @output -->
C++: -3 and -1. Python: -4 and 1.

<!-- @why -->
This is the divergence that silently breaks index wrapping when code is translated between languages.

<!-- @walkthrough -->
1. The exact quotient is -3.5 in both languages.
2. C++ truncates toward zero, discarding the fraction and landing on -3.
3. Its remainder must satisfy -3 times 2 plus r equals -7, which forces r to be -1.
4. Python floors toward negative infinity, rounding -3.5 down to -4.
5. Its remainder must satisfy -4 times 2 plus r equals -7, which forces r to be 1.
6. Both are internally consistent — the languages simply chose different rounding rules.

<!-- @example -->

<!-- @input -->
int i = 5; int j = i++; then reset and int k = ++i;

<!-- @output -->
j is 5, k is 6, and i ends at 6 in both cases

<!-- @why -->
Shows that prefix and postfix are identical as standalone statements and differ only when the expression's value is consumed.

<!-- @walkthrough -->
1. In the postfix form, the current value 5 is handed to j first.
2. Only afterwards is i incremented to 6.
3. In the prefix form, i is incremented to 6 first.
4. The new value 6 is then handed to k.
5. The variable itself finishes at 6 either way — only the value the expression produced differs.
...
<!-- @doubt -->
### Why is 7 / 2 equal to 3 instead of 3.5?
...
<!-- @doubt -->
### Why does my average come out wrong?
...
<!-- @doubt -->
### Why does -7 % 2 give 1 in Python but -1 in C++?
...
<!-- @doubt -->
### How do I wrap an array index backwards safely?
...
<!-- @doubt -->
### How do I raise a number to a power in C++ or Java?
...
<!-- @doubt -->
### Why can't I use % with a double in C++?
...
<!-- @doubt -->
### Why does modulo matter so much in DSA?
