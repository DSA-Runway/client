...
/**
 * Subtopic 18 of Basics (Medium #2). Picks up where
 * functions-declaration-and-calling stopped.
 *
 * Scope boundary: whether a function can MODIFY what it was handed is the next
 * subtopic (Pass by Value vs Pass by Reference). This one is about the interface
 * — how values get in, and how results get out.
 *
 * The single highest-value item is Python's mutable default argument trap.
 * Defaults are evaluated ONCE at definition time, so a mutable default is shared
 * across every call. It produces silently accumulating state, it is one of the
 * most-reported Python bugs in existence, and the mechanism is genuinely
 * non-obvious rather than just a rule to memorise.
 *
 * Second-highest: Java has NO default arguments at all. Students moving between
 * languages assume they exist; Java uses overloading instead, which is subtopic
 * 20 and gets a forward pointer rather than duplication here.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Passing Arguments to Parameters",
      idea: "Supply values at the call site that fill the function's parameters in order.",
      steps: [
        "Read the function's parameter list to see how many values it expects and in what order.",
        "Supply that many arguments at the call site.",
        "Each argument binds to the parameter in the matching position, left to right.",
        "The parameters exist as local variables for the duration of the call.",
        "Confirm the order is right, since a swap of same-typed arguments compiles and runs silently.",
      ],
      code: [
        {
          language: "cpp",
...
int main() {
    ...
    return 0;
          ...
          annotations: {
            ...
            14: "The danger of same-typed parameters: the compiler cannot tell these apart.",
          },
        },
        {
          language: "java",
...
print(divide(denominator=12, numerator=3))   # 0.25 — same result`,
    ...
    },
    {
      ...
      idea: "Make parameters optional, and name them at the call site where the language allows it.",
      steps: [
        "Identify parameters that have a sensible common value.",
        "Give those parameters a default, placing them to the right of all required parameters.",
        "In C++, put the default in the declaration only, never repeated in the definition.",
        "In Python, never use a mutable value as a default — use None and create the object inside the function.",
        "At the call site, omit defaulted arguments, or in Python name any argument to pass it out of order.",
      ],
      code: [
        {
          language: "cpp",
...
// greet("Ana", punct='.');   // not valid C++`,
          annotations: {
            ...
            26: "To vary only the third argument you must supply the second as well.",
          },
        },
        {
          language: "java",
          code: `public class Defaults {
...
# {'mode': 'fast', 'debug': True}`,
    ...
    {
      name: "Returning One Value or Several",
      idea: "Hand results back to the caller, packaging them when there is more than one.",
      steps: [
        "Decide what the function produces and declare the matching return type.",
        "Return that value, remembering the function ends immediately at that point.",
        "When several values are needed, package them into a tuple, pair, struct, or record.",
        "Prefer a named type over an anonymous pair when the values mean different things.",
        "Unpack the result at the call site into separate variables.",
      ],
      code: [
        {
          language: "cpp",
...
def average(arr):
  ...
  examples: [
    {
      input: "divide(numerator, denominator) called as divide(12, 3) and divide(3, 12)",
      output: "4 and 0.25 — both valid, one almost certainly wrong",
      walkthrough: [
        "In the first call, 12 binds to numerator and 3 binds to denominator by position.",
        "The division computes 12 divided by 3, giving 4.",
        "In the second call, 3 binds to numerator and 12 to denominator, since only order matters.",
        "The division computes 3 divided by 12, giving 0.25.",
        "Both calls compile and run without any warning, because both arguments have the same type.",
        "Python's keyword form removes the risk by naming each value at the call site.",
      ],
      why: "Positional matching is invisible when the types agree, which makes argument order a genuine correctness concern rather than a style question.",
    },
    {
      input: "def add_item(item, lst=[]): lst.append(item); return lst  — called three times",
      output: "[1], then [1, 2], then [1, 2, 3] — the list is shared",
      walkthrough: [
        "The def statement executes once, and while it does, the empty list default is created — a single list object.",
        "The first call omits lst, so the parameter binds to that object, appends 1, and returns [1].",
        "The second call omits lst again and binds to the very same object, which still contains 1.",
        "Appending 2 mutates that shared list, so it returns [1, 2] rather than [2].",
        "The third call does the same again, giving [1, 2, 3].",
        "Nothing is reset between calls because nothing creates a new list — the default was evaluated at definition time, not call time.",
      ],
      why: "One of the most-reported Python bugs, and the mechanism is genuinely surprising: the trap is in when the default is evaluated, not in what it contains.",
    },
    {
      input: "The same function written with lst=None and the list created inside",
      output: "[1], then [2], then [3] — correct",
      walkthrough: [
        "The default None is evaluated once at definition time, exactly as before.",
        "None is immutable, so there is nothing about it that can accumulate state.",
        "Each call that omits the argument binds lst to None and then reaches the guard.",
        "The guard creates a brand new empty list, which exists only for this call.",
        "Appending affects that fresh list, so each call returns exactly one item.",
        "The pattern works because the mutable object is created at call time rather than definition time.",
      ],
      why: "The standard fix, and pairing it with the broken version shows it is not superstition — it changes when the object is created.",
    },
    {
      input: "A function returning both the minimum and maximum of [3, 7, 1]",
      output: "1 and 7, packaged differently in each language",
      walkthrough: [
        "A function returns exactly one value, so two results must be packaged into one object.",
        "Python builds a tuple from the comma-separated values and the caller unpacks it into two names.",
        "C++ returns a pair, which structured bindings unpack into two names in a single statement.",
        "Java returns an array when both values share a type, though the caller must remember which index is which.",
        "For values of different types or meanings, a struct in C++ or a record in Java gives the fields names.",
        "Naming matters because s.average documents itself where result[1] relies on the caller remembering.",
      ],
      why: "Multiple return values look like a language feature in Python and are really just packaging, which is what makes the C++ and Java equivalents easy to recognise.",
    },
  ],
  ...
  commonDoubts: [
    ...
    {
      question: "Why does my Python function remember values from previous calls?",
      answer:
    ...
    {
      question: "Does Java have default arguments?",
      answer:
    ...
    {
      question: "Why must default arguments be the rightmost parameters?",
      answer:
    ...
    {
      question: "Should I return a pair or define a struct?",
      answer:
    ...
    {
      question: "How do I avoid mixing up arguments of the same type?",
      answer:
    ...
    {
      question: "What happens if I pass the wrong number of arguments?",
      answer:
