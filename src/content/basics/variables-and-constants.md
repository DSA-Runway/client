...
<!-- @idea -->
Bring a name into existence and give it its first value — ideally in one line.

<!-- @steps -->
1. Decide what kind of value you need to store, which fixes the type.
2. Choose a name that describes the meaning of the value.
3. In C++ and Java, write the type before the name; in Python, write the name alone.
4. Assign the initial value with = on the same line as the declaration.
5. Read or update the value later by using the name.
...
<!-- @code python -->
```python
student_count = 30           # no type written — inferred from the value
average_score = 76.5
top_grade     = "A"
is_passing    = True
...
<!-- @approach -->
### Updating and Swapping Values

<!-- @idea -->
Assignment replaces what a variable holds — and swapping two values is the classic exercise in getting that right.

<!-- @steps -->
1. Assign a new value to an existing variable; the previous value is overwritten and lost.
2. To swap two variables, first copy one of them into a temporary variable.
3. Copy the second variable into the first.
4. Copy the temporary variable into the second.
5. Both values are now exchanged, and the temporary can be discarded.
...
<!-- @code python -->
```python
a, b = 5, 9      # assign several names in one statement
...
<!-- @idea -->
Lock a value that should never change, so the compiler enforces what you intended.

<!-- @steps -->
1. Identify a value that is fixed for the whole program — a limit, a size, a conversion factor.
2. Declare it with the language's constant keyword, or the naming convention where none exists.
3. Give it its value at the point of declaration; a constant cannot be assigned later.
4. Name it in UPPER_SNAKE_CASE so readers recognise it as fixed.
5. Use the name everywhere the value is needed, so changing it later means editing one line.
...
<!-- @input -->
int score = 0; then score = 10; then score = score + 5;

<!-- @output -->
15

<!-- @why -->
Shows that = is not equality but an instruction: compute the right side, then store it on the left.

<!-- @walkthrough -->
1. Declaration and initialisation reserve a box named score and put 0 in it.
2. Assignment replaces the contents with 10, and the 0 is gone with no record of it.
3. The third line reads the current value 10, adds 5 to produce 15, then writes 15 back into the same box.
4. The right side is always evaluated first, which is why score can appear on both sides.
5. score now holds 15.

<!-- @example -->

<!-- @input -->
a = 5, b = 9; swap them using a temporary variable

<!-- @output -->
a = 9, b = 5

<!-- @why -->
The temporary variable exists for exactly one reason, and seeing the failure case without it is what makes that reason stick.

<!-- @walkthrough -->
1. temp = a copies 5 into temp, so 5 now exists in two places.
2. a = b overwrites a with 9. The original 5 would be lost here if temp had not saved it.
3. b = temp writes the saved 5 into b.
4. a holds 9 and b holds 5 — the values are exchanged.
5. Doing it without temp fails: a = b then b = a leaves both holding 9.

<!-- @example -->

<!-- @input -->
Declare MAX = 100 as a constant, then try to assign MAX = 200

<!-- @output -->
C++ and Java: compile error. Python: succeeds, MAX becomes 200.

<!-- @why -->
Makes the enforcement gap explicit, so a student moving between languages does not assume a guarantee Python never offered.

<!-- @walkthrough -->
1. In C++, const marks the variable read-only and the compiler rejects any assignment to it.
2. In Java, final does the same and the error appears before the program is built.
3. In Python, MAX_STUDENTS is an ordinary variable with a name written in capitals.
4. The interpreter has no concept of a constant here, so the assignment simply happens.
5. The capitals communicated intent to a human reader, and nothing more.
...
<!-- @doubt -->
### What is the difference between declaration, initialisation, and assignment?
...
<!-- @doubt -->
### Why does C++ print a random number for a variable I didn't initialise?
...
<!-- @doubt -->
### Is a Python variable a box or a label?
...
<!-- @doubt -->
### Why bother with constants? I can just not change the value.
...
<!-- @doubt -->
### How do I choose a good variable name?
...
<!-- @doubt -->
### Can I change a variable's type after declaring it?
