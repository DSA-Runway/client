...
/**
 * Subtopic 24 of Basics (Medium #8). Closes the chain:
 * Count Digits -> Reverse a Number -> Palindrome Number.
 *
 * SOURCES
 * - GeeksforGeeks, "Check if a number is Palindrome" — the reverse-and-compare
 *   baseline, the string two-pointer alternative, and the optimised approach
 *   that examines only half the digits.
 *
 * MEASURED ON THIS MACHINE (g++ -O2), not assumed:
 *
 * 1. The half-reversal approach was checked EXHAUSTIVELY over 0..1,999,999
 *    against a string reference, plus targeted boundary values including
 *    1000000001, 2147483647, 1000021, 100, 10, 0 and 9. Zero mismatches.
 *
 * 2. THE TRAILING-ZERO GUARD IS LOAD-BEARING. Removing it produces 1,431 wrong
 *    answers in the first million alone, starting at 10 — every number ending in
 *    zero is falsely reported as a palindrome. This is the single easiest way to
 *    get the half-reversal wrong and it is not obvious from reading the code.
 *
 * 3. A finding worth stating carefully: scanning the FULL non-negative int range
 *    0..INT_MAX with -fwrapv, naive full reversal produced zero false positives
 *    and zero false negatives. So on 32-bit ints with wrapping semantics it
 *    happens to be correct everywhere. It still relies on signed overflow, which
 *    is undefined behaviour in C++, so it is not something to depend on — but
 *    claiming "overflow makes it wrong" would have been an overstatement, and
 *    the container says what was actually observed.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Full Reversal and Compare",
      idea: "Reverse the entire number using the previous subtopic's loop and test whether it equals the original.",
      steps: [
        "Return false immediately for negative input, since the minus sign cannot mirror.",
        "Keep a copy of the original value, because the loop destroys the input.",
        "Reverse the number using the accumulator loop: read the last digit, shift the accumulator left, append, and divide.",
        "Compare the reversed value with the saved original.",
...
print(is_palindrome(12345678987654321))   # True`,
    ...
    {
      name: "Reverse Only Half the Digits",
      idea: "Build the reversed second half until it meets the remaining first half, so no accumulator ever holds more than half the digits.",
      steps: [
        "Return false for negative input.",
        "Return false for any positive number whose last digit is zero, since its mirror would need a leading zero.",
        "Initialise the reversed half to zero.",
        "While the remaining number is greater than the reversed half, move one digit across: append it to the reversed half and remove it from the number.",
        "Stop when the two meet, which happens after roughly half the digits.",
        "Return true if they are equal, or if the number equals the reversed half with its last digit dropped, which handles an odd digit count.",
      ],
      code: [
        {
          language: "cpp",
          code: `bool isPalindromeHalf(int x) {
...
cout << isPalindromeHalf(2147483647) << endl;   // 0`,
          annotations: {
            ...
            17: "The rev / 10 branch discards the middle digit, which matches itself by definition.",
          },
        },
        {
          language: "java",
          code: `static boolean isPalindromeHalf(int x) {
    if (x < 0) return false;
...
# halved iteration count rather than for safety.`,
    ...
    {
      name: "String Two-Pointer Comparison",
      idea: "Convert to text and compare characters inward from both ends, stopping at the first mismatch.",
      steps: [
        "Return false for negative input, or strip the sign if the problem defines it that way.",
        "Convert the number to its string representation.",
        "Place one index at the first character and another at the last.",
        "While the two indices have not met, compare the characters they point at.",
        "Return false on the first mismatch, since one difference is enough.",
        "Move the left index right and the right index left, and return true if they meet without a mismatch.",
      ],
      code: [
        {
          language: "cpp",
...
// same middle character, which never needs comparing.`,
          annotations: {
            ...
            22: "left < right rather than left <= right, so the middle character of an odd-length string is skipped.",
          },
        },
        {
          language: "java",
          code: `static boolean isPalindromeStr(int x) {
    if (x < 0) return false;
...
# building a reversed copy is not affordable.`,
  ...
  examples: [
    {
      input: "N = 1221, using the half-reversal approach",
      output: "true, after two iterations rather than four",
      walkthrough: [
        "The number is positive and does not end in zero, so both guards pass.",
        "The reversed half starts at 0, and 1221 is greater than 0 so the loop runs.",
        "Move the last digit across: the reversed half becomes 1 and the number becomes 122.",
        "122 is still greater than 1, so move again: the reversed half becomes 12 and the number becomes 12.",
        "Now 12 is not greater than 12, so the loop stops — the two halves have met.",
        "The number equals the reversed half, so the answer is true after touching only two of the four digits.",
      ],
      why: "Shows the halves meeting in the middle, which is the whole idea. Half the digits were never examined and did not need to be.",
    },
    {
      input: "N = 12321, an odd digit count",
      output: "true, via the rev / 10 branch",
      walkthrough: [
        "The loop moves 1 across, leaving the number at 1232 and the reversed half at 1.",
        "It moves 2 across, leaving 123 and 12.",
        "It moves 3 across, leaving 12 and 123.",
        "Now 12 is not greater than 123, so the loop stops with the reversed half having overshot by one digit.",
        "The direct comparison fails, since 12 is not 123.",
        "Dividing the reversed half by 10 drops the 3, which is the middle digit and matches itself by definition.",
        "12 equals 12, so the answer is true.",
      ],
      why: "The odd-length case is where the second comparison earns its place, and seeing the overshoot explains why rev / 10 is the right correction.",
    },
    {
      input: "N = 10, with the trailing-zero guard removed",
      output: "Incorrectly reports true",
      walkthrough: [
        "Without the guard, the loop begins with the number at 10 and the reversed half at 0.",
        "The last digit is 0, so appending it leaves the reversed half at 0 while the number becomes 1.",
        "1 is still greater than 0, so the loop runs again: the reversed half becomes 1 and the number becomes 0.",
        "0 is not greater than 1, so the loop stops with the number at 0 and the reversed half at 1.",
        "The direct comparison fails, since 0 is not 1.",
        "But the odd-length branch computes 1 divided by 10, which is 0, and 0 equals 0 — so it reports a palindrome.",
        "Measured across the first million values, removing the guard produces 1,431 such wrong answers, the first at 10.",
      ],
      why: "The guard looks like an optimisation and is actually part of the algorithm. This trace shows exactly which branch it protects.",
    },
    {
      input: "Full reversal on the entire non-negative int range, with wrapping arithmetic",
      output: "Zero false positives and zero false negatives across all 2,147,483,648 values",
      walkthrough: [
        "A genuine palindrome that fits in the type has a reverse equal to itself, so it can never overflow.",
        "Overflow is therefore only possible while reversing a non-palindrome.",
        "The risk would be a wrapped result landing back on the original value and reporting a false positive.",
        "Every non-negative int was scanned against a string reference with wrapping arithmetic enabled.",
        "No such collision exists — the naive version agreed with the reference on every single value.",
        "That does not make it safe: signed overflow is undefined behaviour in C++, so a compiler may optimise on the assumption it cannot happen.",
      ],
      why: "The usual claim is that overflow makes this approach wrong. Measuring it shows the real situation is narrower — not observably wrong on 32-bit ints, but resting on behaviour the language does not define.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why is a negative number not a palindrome?",
      answer:
    ...
    {
      question: "Why does any number ending in zero fail?",
      answer:
    ...
    {
      question: "Why does reversing only half the number work?",
      answer:
    ...
    {
      question: "Why is the comparison x == rev / 10 needed?",
      answer:
    ...
    {
      question: "Can I skip the trailing-zero guard?",
      answer:
    ...
    {
      question: "Does the full-reversal approach actually break on overflow?",
      answer:
    ...
    {
      question: "Is the half-reversal faster in complexity terms?",
      answer:
    ...
    {
      question: "Should I just convert to a string and compare?",
      answer:
