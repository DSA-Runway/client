...
/**
 * Subtopic 11 of Basics — the first repetition construct, and the workhorse of
 * every array algorithm that follows.
 *
 * Rewritten from the original template to match the depth of subtopics 1-10.
 *
 * The divergence that gets real weight: Python has no three-part for loop at
 * all. `for i in range(n)` iterates a sequence of values, so there is no
 * condition and no update expression, and reassigning the counter inside the
 * body does nothing. GFG's Python coverage omits this comparison entirely, which
 * is exactly the gap a three-language platform cannot afford.
 *
 * Scope: nested loops are subtopic 17, for-each is 14, break/continue is 15.
 * Each is pointed at rather than duplicated.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Counting Up",
      idea: "The default form — start low, increment, stop when the condition fails.",
      steps: [
        "Run the initialisation once, creating the counter with its starting value.",
        "Check the condition; if it is false the loop ends immediately without running the body.",
        "Run the body once.",
        "Run the update, changing the counter.",
        "Return to the condition check and repeat until it fails.",
        "Continue after the loop, with the counter holding the first value that failed the check.",
      ],
      code: [
        {
          language: "cpp",
...
print()   # prints: 100 100 100 100 100 — still exactly 5 iterations`,
    ...
    {
      name: "Counting Down and Custom Steps",
      idea: "Change the start and the update to walk backwards, or to move by more than one.",
      steps: [
        "Set the initialisation to the value the loop should start from.",
        "Write the condition so it stays true across the intended range and fails just past it.",
        "Write the update to move the counter in the required direction and size.",
        "For a backwards walk over an array, start at n - 1 and continue while the counter is at least 0.",
        "For a multiplicative step, note that Python's range cannot express it and a while loop is required.",
      ],
      code: [
        {
          language: "cpp",
...
for (int i = 0, j = n - 1; i < j; i++, j--) {
    swap(arr[i], arr[j]);
          ...
          annotations: {
            ...
            22: "A multiplying update runs about log n times rather than n — the basis of binary search's cost.",
            26: "Comma-separated initialisation and update. This exact header appears in dozens of array problems.",
          },
        },
        {
          language: "java",
...
for (int i = 0, j = n - 1; i < j; i++, j--) {
    int tmp = arr[i]; arr[i] = arr[j]; arr[j] = tmp;
        ...
        {
          language: "python",
          code: `# Counting down — the third argument is the step
...
while i < j:
    ...
    {
      name: "Iterating a Collection by Index",
      idea: "The workhorse of array algorithms — visit every element using its position.",
      steps: [
        "Determine the number of elements in the collection.",
        "Start the counter at 0, which is the first valid index.",
        "Continue while the counter is strictly less than the length.",
        "Use the counter to read or write the element at that position inside the body.",
        "Increment by one so every position is visited exactly once.",
      ],
      code: [
        {
          language: "cpp",
...
for (int value : arr) {
    cout << value << " ";
          ...
          annotations: {
            ...
            19: "In C++ this may not even crash. It reads whatever memory sits past the array, which is worse.",
          },
        },
        {
          language: "java",
...
for i, value in enumerate(arr):
  ...
  examples: [
    {
      input: "for (int i = 0; i < 3; i++) { print(i); }",
      output: "0 1 2, and i holds 3 after the loop",
      walkthrough: [
        "Initialisation runs once, setting i to 0.",
        "The condition 0 < 3 is checked and passes, so the body runs and prints 0. The update then makes i equal to 1.",
        ...
        "The condition 3 < 3 fails, so the loop exits without running the body again.",
        "The body ran three times and never saw the value 3, yet i holds 3 — because reaching 3 is what ended the loop.",
      ],
      why: "The clearest demonstration that the update runs after the body, which is the source of the most common confusion about the final counter value.",
    },
    {
      input: "for (int i = 5; i < 3; i++) { print(i); }",
      output: "nothing at all",
      walkthrough: [
        "Initialisation runs once, setting i to 5.",
        "The condition 5 < 3 is checked and is immediately false.",
        "The loop exits without ever entering the body.",
        "No output is produced and the update never runs.",
        "This is correct behaviour, not an error — and it is why looping over an empty collection needs no special case.",
      ],
      why: "Proves the condition is checked before the first iteration, which is what makes zero-iteration loops safe rather than surprising.",
    },
    {
      input: "An array of 4 elements iterated with i <= n instead of i < n",
      output: "Reads index 4, which does not exist",
      walkthrough: [
        "The array has 4 elements, so its valid indices are 0, 1, 2 and 3.",
        "The loop runs correctly for i equal to 0, 1, 2 and 3, reading each real element.",
        "The condition 4 <= 4 still passes, so the body runs once more with i equal to 4.",
        "Index 4 is one past the last element and was never allocated as part of the array.",
        "Java throws ArrayIndexOutOfBoundsException and Python raises IndexError, both naming the bad index.",
        "C++ performs the read anyway, returning whatever memory happens to sit there — often without crashing, which makes it the hardest of the three to notice.",
      ],
      why: "The single most common loop bug in array code, and the reason the i < n habit is worth forming deliberately rather than by trial and error.",
    },
    {
      input: "for i in range(5): i = 100; print(i)",
      output: "100 printed five times — the loop still runs exactly five iterations",
      walkthrough: [
        "range(5) produces the sequence 0, 1, 2, 3, 4 before the loop begins.",
        "Each iteration takes the next value from that sequence and binds it to the name i.",
        "Assigning 100 to i replaces what the name refers to for the rest of this iteration only.",
        "The sequence itself is untouched, so the next iteration binds i to the next value as if nothing happened.",
        "Five values were produced, so the body runs exactly five times regardless of what the body does to i.",
        "The equivalent C++ or Java code would set the counter to 100, fail the condition, and exit after one iteration.",
      ],
      why: "The clearest proof that Python's for is not a counter loop, and it is the divergence most likely to mislead someone porting code between the languages.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why does i end up as 3 when the loop only printed up to 2?",
      answer:
    ...
    {
      question: "Why is it i < n and not i <= n?",
      answer:
    ...
    {
      question: "Can a for loop run zero times?",
      answer:
    ...
    {
      question: "Why doesn't changing i inside a Python for loop affect the iteration?",
      answer:
    ...
    {
      question: "How do I loop by 2, or by doubling?",
      answer:
    ...
    {
      question: "What is enumerate for?",
      answer:
    ...
    {
      question: "Does the loop variable still exist after the loop ends?",
      answer:
    ...
    {
      question: "Is i++ different from ++i in the loop header?",
      answer:
