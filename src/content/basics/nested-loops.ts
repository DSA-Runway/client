...
/**
 * Subtopic 17 of Basics — the first Medium one, and the bridge from Basics into
 * the Pattern Printing topic, every subtopic of which is a nested loop.
 *
 * Two things get deliberate emphasis:
 *
 * 1. The inner loop RESTARTS COMPLETELY on every outer iteration. Almost every
 *    nested-loop bug is a failure of that mental model — most often an
 *    accumulator declared outside the outer loop when it should be reset inside.
 *
 * 2. Cost is POLYNOMIAL, not exponential. GFG's own loop-analysis page says
 *    nested loops grow "exponentially as the number of nested loops increases",
 *    which is wrong and worth correcting explicitly: k nested loops is O(n^k),
 *    while exponential means O(2^n) — a different and far worse growth class. A
 *    DSA platform that blurs those two teaches a misconception students carry
...
`.trim(),
  ...
  approaches: [
    {
      name: "Running a Loop Inside a Loop",
      idea: "Repeat an entire loop once for each iteration of an outer loop.",
      steps: [
        "Write the outer loop with its own counter, controlling how many times the whole inner sequence repeats.",
        "Write the inner loop with a different counter, controlling one full sweep.",
        "Place any state that should be per-outer-iteration inside the outer loop but outside the inner one.",
        "The inner loop runs to completion, then the outer update fires and the inner loop restarts from its initial value.",
        "Total body executions are the outer count multiplied by the inner count.",
      ],
      code: [
        {
          language: "cpp",
...
for i in range(1, n + 1):
    ...
    {
      name: "Traversing a 2D Structure",
      idea: "Visit every cell of a grid using one loop for rows and one for columns.",
      steps: [
        "Determine the number of rows and the number of columns.",
        "Loop over the rows in the outer loop.",
        "Loop over that row's columns in the inner loop.",
        "Access the element using both counters as its coordinates.",
        "Reset any per-row accumulator inside the outer loop, before the inner loop begins.",
        "Prefer rows in the outer loop, since that order matches how the data sits in memory.",
      ],
      code: [
        {
          language: "cpp",
...
print(sum(sum(row) for row in grid))   # 45`,
    ...
    {
      name: "Generating Pairs with Triangular Loops",
      idea: "Compare every element against every other, without doing each pair twice.",
      steps: [
        "Loop over the array with the outer counter to select the first element of each pair.",
        "Start the inner counter at one past the outer counter, not at zero.",
        "Loop the inner counter to the end to select the second element.",
        "Each unordered pair is now produced exactly once, and no element is paired with itself.",
        "Note that the total is n times n minus one, divided by two — about half the full nesting, but still quadratic.",
      ],
      code: [
        {
          language: "cpp",
          code: `vector<int> arr = {3, 7, 1, 9};
int n = arr.size();
...
System.out.println(found);   // true`,
          annotations: {
            20: "No flag needed in the outer condition, because the labelled break leaves both loops directly.",
          },
        },
        {
          language: "python",
...
print(has_pair_fast(arr, 10))   # True — one pass, no nesting`,
  ...
  examples: [
    {
      input: "for i in 0..2: for j in 0..3: print(i, j)",
      output: "12 lines: 00 01 02 03, then 10 11 12 13, then 20 21 22 23",
      walkthrough: [
        "The outer loop sets i to 0 and enters its body.",
        "The inner loop runs completely, taking j through 0, 1, 2 and 3, printing four pairs.",
        "The inner loop's condition fails and it ends, so control returns to the outer loop.",
        "The outer update sets i to 1, and the inner loop starts again from j equal to 0.",
        "It is a fresh four-count, not a continuation — j was re-initialised by the inner loop's own header.",
        "This repeats for i equal to 2, giving three full sweeps of four, which is twelve body executions.",
      ],
      why: "Establishes the restart, which is the mental model every other nested-loop behaviour follows from.",
    },
    {
      input: "n = 4; for i in 1..4: for j in 1..i: print '*'",
      output: "A right triangle: 1 star, then 2, then 3, then 4",
      walkthrough: [
        "With i at 1, the inner loop runs while j is at most 1, so it prints one star.",
        "The newline after the inner loop ends the row.",
        "With i at 2, the inner bound is now 2, so the inner loop prints two stars.",
        "With i at 3 and then 4, the rows grow to three and four stars.",
        "The inner loop's bound depends on the outer counter, so each sweep is a different length.",
        "Total stars are 1 plus 2 plus 3 plus 4, which is 10 — not 16, because the loop is triangular.",
      ],
      why: "Rows mapping to the outer loop and columns to the inner is the exact structure of all 22 Pattern Printing subtopics.",
    },
    {
      input: "Summing each row of a 3x3 grid, with the accumulator declared before the outer loop",
      output: "6, then 21, then 45 — running totals instead of row totals",
      walkthrough: [
        "The accumulator is created once, before any looping begins.",
        "The first row adds 1, 2 and 3, so it holds 6 and the correct row total is printed.",
        "The outer loop advances, but the accumulator is not reset because it lives outside.",
        "The second row adds 4, 5 and 6 on top of the existing 6, giving 21 rather than 15.",
        "The third row adds its 24 on top of that, giving 45 — which is the grand total, not the row total.",
        "Moving the declaration inside the outer loop creates a fresh accumulator per row and fixes it.",
      ],
      why: "The most common nested-loop bug, and it follows directly from the restart model: the inner loop starts over, so what it builds should start over too.",
    },
    {
      input: "An array of 4 elements: full nesting versus starting the inner loop at i + 1",
      output: "16 iterations versus 6",
      walkthrough: [
        "With both loops running the full range, every ordered pair is produced, giving 4 times 4 which is 16.",
        "Four of those pair an element with itself, which is almost never wanted.",
        "The remaining twelve consist of each unordered pair appearing twice, once in each order.",
        "Starting the inner loop at i plus 1 skips everything at or before the diagonal.",
        "The inner loop now runs 3 times, then 2, then 1, then 0, totalling 6.",
        "That is n times n minus one over two — roughly half the work, and still O of n squared.",
      ],
      why: "Shows a real and worthwhile optimisation that does not change the complexity class, which is a distinction students need before complexity analysis.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "How many times does the inner loop actually run?",
      answer:
    ...
    {
      question: "Why are my row sums coming out as running totals?",
      answer:
    ...
    {
      question: "Are nested loops exponential?",
      answer:
    ...
    {
      question: "Why start the inner loop at i + 1?",
      answer:
    ...
    {
      question: "Does the triangular version make my algorithm faster?",
      answer:
    ...
    {
      question: "How do I break out of both loops at once?",
      answer:
    ...
    {
      question: "When is a nested loop the wrong solution?",
      answer:
    ...
    {
      question: "Does it matter whether rows or columns go in the outer loop?",
      answer:
