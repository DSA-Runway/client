...
/**
 * Subtopic 15 of Basics. Four earlier subtopics point forward to this one.
 *
 * The centrepiece is the payoff of the framing established in while-loop: a for
 * loop BUNDLES its update into the header, a while loop SCATTERS it into the
 * body. That single difference decides whether `continue` is safe or fatal —
 * in a for loop continue passes through the update, in a while loop it jumps
 * over it and freezes the counter. Neither GFG page covers this, and it is the
 * most valuable thing in the subtopic.
 *
 * Also finally covers Python's for-else / while-else in full, which the
 * while-loop container deliberately deferred here.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Exiting Early with break",
      idea: "Stop the loop the moment the answer is known, instead of scanning the rest.",
      steps: [
        "Identify the condition that means no further iterations are needed.",
        "Test that condition inside the loop body.",
        "Execute break when it holds, which leaves the loop immediately.",
        "Note that the remaining body statements, the update, and the condition check are all skipped.",
        "Continue with the first statement after the loop, using whatever the loop recorded before exiting.",
      ],
      code: [
        {
          language: "cpp",
...
while d * d <= n:
    ...
    {
      name: "Skipping an Iteration with continue",
      idea: "Abandon the current iteration and move to the next — but know where 'the next' begins.",
      steps: [
        "Identify the items the body should not process.",
        "Test for them at the top of the body, before any real work.",
        "Execute continue, which skips the rest of the body for this iteration only.",
        "In a for loop, control passes to the update and then the condition.",
        "In a while loop, control passes directly to the condition, skipping any update below the continue.",
        "Confirm the loop can still make progress, since a skipped update means it never will.",
      ],
      code: [
        {
          language: "cpp",
          code: `// SAFE in a for loop — the update lives in the header
for (int i = 0; i < 5; i++) {
...
System.out.println(sum);   // 12`,
        ...
        {
          language: "python",
          code: `# SAFE in a for loop — there is no update to skip
for i in range(5):
    if i == 2:
        continue
    print(i, end=" ")
...
print(total)   # 12`,
    ...
    {
      name: "Escaping Nested Loops",
      idea: "Get out of more than one loop level, which break alone cannot do.",
      steps: [
        "Recognise that break exits only the innermost loop containing it.",
        "Decide how far out you actually need to go.",
        "In Java, label the outer loop and break to that label.",
        "In C++ or Python, set a flag in the inner loop and test it immediately after, or use goto in C++.",
        "Prefer extracting the nested loops into a function and returning, which exits every level at once in all three languages.",
      ],
      code: [
        {
          language: "cpp",
          code: `vector<vector<int>> grid = {{1, 2}, {3, 4}, {5, 6}};
int target = 4;
...
print(find(grid, target))   # (1, 1)`,
  ...
  examples: [
    {
      input: "arr = [4, 8, 15, 16, 23, 42]; search for 15 with break",
      output: "Index 2, after three comparisons instead of six",
      walkthrough: [
        "The first iteration compares 4 against 15, which does not match, so the body ends normally.",
        "The second compares 8, which also does not match.",
        "The third compares 15, which matches, so the index is recorded.",
        "break executes, leaving the loop immediately without running the update or re-checking the condition.",
        "The remaining three elements are never examined, because the answer is already known.",
        "Execution resumes at the statement after the loop with the recorded index available.",
      ],
      why: "The clearest justification for break: once the answer exists, further iterations are pure waste.",
    },
    {
      input: "int i = 0; while (i < 5) { if (i == 2) continue; print(i); i++; }",
      output: "0 1 then the program hangs forever",
      walkthrough: [
        "With i at 0 the condition passes, the guard is false, 0 prints, and i becomes 1.",
        "With i at 1 the condition passes, the guard is false, 1 prints, and i becomes 2.",
        "With i at 2 the condition passes and the guard is now true, so continue executes.",
        "continue jumps directly to the condition check, skipping both the print and the increment below it.",
        "The condition is re-checked with i still equal to 2, which passes, so the body runs again.",
        "The guard is true again, continue fires again, and nothing in the loop can ever change i.",
      ],
      why: "The continue trap in its exact form. Nothing in the syntax warns you, and the program simply stops responding.",
    },
    {
      input: "for (int i = 0; i < 5; i++) { if (i == 2) continue; print(i); }",
      output: "0 1 3 4, terminating normally",
      walkthrough: [
        "With i at 0 and 1 the guard is false, so both values print and the update advances i each time.",
        "With i at 2 the guard is true and continue executes, skipping the print.",
        "Control passes to the update rather than straight to the condition, because the update lives in the header.",
        "i becomes 3, the condition is checked, and the loop proceeds normally.",
        "3 and 4 print, then i reaches 5, the condition fails, and the loop exits.",
        "The same continue statement that hung the while loop is harmless here purely because of where the update sits.",
      ],
      why: "Placed beside the previous example, it isolates the cause exactly: nothing about continue changed, only the location of the update.",
    },
    {
      input: "Nested loops over a grid, with break in the inner loop when the target is found",
      output: "The inner loop exits, but the outer loop continues over every remaining row",
      walkthrough: [
        "The outer loop begins its first row and the inner loop scans that row's values.",
        "When the target is found, break executes and leaves the inner loop.",
        "Control resumes at the first statement after the inner loop, still inside the outer loop's body.",
        "The outer loop's update runs and its condition is checked, so the next row begins.",
        "The inner loop scans that row too, even though the answer was already found.",
        "Every remaining row is scanned, which is exactly the waste the break was meant to prevent.",
      ],
      why: "break binding to the innermost loop is stated in every reference and still surprises people, because the intent reads as leave the search.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why does my while loop hang when I add a continue?",
      answer:
    ...
    {
      question: "Does break exit all the loops it is inside?",
      answer:
    ...
    {
      question: "What is the cleanest way to break out of two loops?",
      answer:
    ...
    {
      question: "What does Python's for-else actually do?",
      answer:
    ...
    {
      question: "Should I use break or return to leave a loop?",
      answer:
    ...
    {
      question: "Is using continue considered bad style?",
      answer:
    ...
    {
      question: "Does a break inside a switch also break the surrounding loop?",
      answer:
    ...
    {
      question: "Where exactly does continue jump to?",
      answer:
  ...
  relatedIds: ["for-loop", "while-loop", "do-while-loop", "switch-case", "nested-loops"],
};
