...
/**
 * Subtopic 21 of Basics (Medium #5). Both subtopic 16 and 18 point forward here.
 *
 * Third construct in this module that one of the three languages simply does not
 * have — after switch (no Python equivalent) and do-while (no Python
 * equivalent). Python does NOT support overloading: a second def with the same
 * name silently REPLACES the first. No error, no warning. That silence is worse
 * than an error and is the highest-value thing in this container.
 *
 * The other load-bearing fact: the return type is not part of the signature.
 * Students reach for it constantly because it feels like it should count.
 *
 * Also worth carrying over from subtopic 19: in C++, by-value and by-reference
 * parameters do NOT distinguish overloads, which surprises people who have just
 * learned that the two behave very differently.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Overloading by Parameter List",
      idea: "Define several functions sharing a name, distinguished by how many parameters they take or what types those are.",
      steps: [
        "Confirm the versions are the same operation applied to different inputs, rather than different operations.",
        "Write each version with the same name and a different parameter list.",
        "Vary the count, the types, or the order of types — anything else will not distinguish them.",
        "Do not rely on the return type or the parameter names, since neither is part of the signature.",
        "Call the shared name with the arguments you have, and the compiler selects the matching version.",
      ],
      code: [
        {
          language: "cpp",
...
int main() {
    ...
    // error: method add(int,int) is already defined
        ...
        {
          language: "python",
          code: `# Python does NOT support overloading. A second def REPLACES the first.
...
print(add(2, 3, 4))        # 9`,
    ...
    {
      name: "How the Compiler Chooses",
      idea: "Follow the resolution order from exact match to conversion, and recognise when two candidates tie.",
      steps: [
        "Collect every function sharing the called name.",
        "Discard any whose parameter count cannot match the arguments supplied.",
        "Prefer a candidate whose parameter types match the arguments exactly.",
        "If none matches exactly, prefer a promotion of a smaller type to a larger one.",
        "Otherwise accept a standard conversion, such as an integer to a floating-point value.",
        "If no candidate is viable, or two are equally good, the compiler reports an error rather than guessing.",
      ],
      code: [
        {
          language: "cpp",
...
//         // both candidates accept exactly one argument`,
    ...
    // error: method f(int) is already defined
        ...
        {
          language: "python",
          code: `# There is no resolution step in Python — the name holds one function.
...
print(area(2.0))    # 12.56636`,
    ...
    {
      name: "Python's Alternatives in Practice",
      idea: "Pick the substitute matching what actually varies between the versions.",
      steps: [
        "Determine whether the versions differ by argument count, by argument type, or by both.",
        "If only the count varies, use default arguments — the simplest and most readable option.",
        "If the count varies widely or unpredictably, accept *args and branch on its length.",
        "If the type varies and there are only two or three cases, branch with isinstance inside one function.",
        "If the type varies across several cases, use functools.singledispatch to keep the implementations separate.",
        "Never define two functions with the same name expecting both to survive.",
      ],
      code: [
        {
          language: "python",
...
print(summarise(3.5))       # unsupported: float`,
        ...
        {
          language: "cpp",
          code: `// For comparison — C++ expresses the same intents natively
...
// The compiler routes each call, with no branching written by hand.`,
        ...
        {
          language: "java",
          code: `// For comparison — Java leans on overloading for both roles
...
public class Alternatives {
  ...
  examples: [
    {
      input: "add(int, int), add(double, double) and add(int, int, int) defined; call add(2, 3)",
      output: "The two-integer version runs",
      walkthrough: [
        "The compiler collects all three functions named add.",
        "The three-parameter version is discarded immediately, since only two arguments were supplied.",
        "Two candidates remain, both accepting two parameters.",
        "The integer version matches the argument types exactly, with no conversion required.",
        "The double version would require converting both arguments, which ranks below an exact match.",
        "The integer version is selected, and this decision is made entirely at compile time.",
      ],
      why: "Shows resolution as a filter followed by a ranking, which is what makes ambiguity errors understandable rather than arbitrary.",
    },
    {
      input: "void f(int) and void f(double) defined; call f(5L) with a long argument",
      output: "Compile error: the call is ambiguous",
      walkthrough: [
        "Both candidates accept one argument, so neither is discarded on count.",
        "Neither matches a long exactly, so no candidate wins outright.",
        "Converting a long to an int is a standard conversion.",
        "Converting a long to a double is also a standard conversion, at the same rank.",
        "Two candidates are equally good, so the compiler has no basis to choose between them.",
        "It reports an ambiguity rather than picking one, and the fix is a cast at the call site or an exact overload.",
      ],
      why: "Ambiguity is a compile error rather than a silent choice, which is the behaviour you want and the reason overload sets should stay small.",
    },
    {
      input: "Two Python functions both named add, defined one after the other",
      output: "The second replaces the first, with no error at definition",
      walkthrough: [
        "The first def executes and binds the name add to a function object.",
        "The second def executes and binds the same name to a different function object.",
        "This is a rebinding, exactly like assigning to a variable twice — the first object is discarded.",
        "Nothing is reported, because redefining a name is entirely legal Python.",
        "Calling add with the original argument count now raises TypeError, since only the newer function exists.",
        "The failure surfaces at a call site that previously worked, far from the definition that caused it.",
      ],
      why: "The silence is the danger. Merging two modules that each define the same function name breaks calls with no warning anywhere.",
    },
    {
      input: "Attempting to overload on return type alone: int f() and double f()",
      output: "Compile error in both C++ and Java",
      walkthrough: [
        "The compiler chooses an overload using only the call site's arguments.",
        "Both candidates take no arguments, so the argument list distinguishes nothing.",
        "A call may legally discard its return value, so there is not always a context to select from.",
        "Even when the result is used, resolving by expected type would quickly become ambiguous.",
        "Both languages therefore reject the declarations outright rather than defining rules for it.",
        "Giving the functions different names is the correct fix, since they genuinely produce different things.",
      ],
      why: "A rule students reach for constantly, and the reason it cannot work is more useful than memorising that it does not.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why can't I overload on the return type?",
      answer:
    ...
    {
      question: "Does Python support function overloading?",
      answer:
    ...
    {
      question: "What actually distinguishes one overload from another?",
      answer:
    ...
    {
      question: "How does the compiler decide which overload to call?",
      answer:
    ...
    {
      question: "What causes an ambiguous overload error?",
      answer:
    ...
    {
      question: "Should I use overloading or default arguments?",
      answer:
    ...
    {
      question: "When should I not overload?",
      answer:
