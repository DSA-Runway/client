...
<!-- @approach -->
### Iterative Digit-Removal Loop

<!-- @idea -->
Strip the last digit off N with integer division, counting the strips until N reaches zero.

<!-- @steps -->
1. Guard the input: zero has one digit, so return 1 immediately.
2. Take the absolute value if negatives are possible.
3. Initialise a counter to zero.
4. While N is greater than zero, increment the counter.
5. Remove the last digit by dividing N by 10 with integer division.
6. When N reaches zero the condition fails, and the counter holds the digit count.
...
<!-- @idea -->
Express the same division as a function calling itself, one digit per call.

<!-- @steps -->
1. Define the base case: a number below 10 has exactly one digit.
2. Otherwise, remove the last digit by dividing by 10.
3. Return one plus the digit count of that smaller number.
4. Each call reduces the number by one digit, so the recursion is guaranteed to reach the base case.
5. Note that every pending call occupies a stack frame until the base case returns.
...
<!-- @code cpp -->
```cpp
int countDigitsRec(long long n) {
    if (n < 0) n = -n;
...
<!-- @code java -->
```java
static int countDigitsRec(long n) {
    if (n < 0) n = -n;
...
<!-- @idea -->
Compute floor(log10(N)) + 1 directly, with no iteration at all.

<!-- @steps -->
1. Guard zero and negatives, since the logarithm is undefined for both.
2. Take the base-10 logarithm of N.
3. Discard the fractional part by casting to an integer.
4. Add one to account for exact powers of ten.
5. Confirm N stays below roughly 10^15, above which floating-point rounding makes the result unreliable.
...
<!-- @code java -->
```java
static int countDigitsLog(long n) {
    if (n == 0) return 1;
    if (n < 0) n = -n;
...
- 15: If the standard library avoids log10 for exactly this task, that is a strong signal.
...
<!-- @idea -->
Convert the number to text and take its length.

<!-- @steps -->
1. Take the absolute value, so the minus sign is not counted as a digit.
2. Convert the integer to its string representation.
3. Return the length of that string.
4. Note that no zero guard is needed, since converting zero already gives one character.
...
<!-- @code java -->
```java
static int countDigitsStr(long n) {
...
<!-- @approach -->
### Lookup Table - Constant Time and Exact

<!-- @idea -->
Compare N against the precomputed boundaries of each digit range, avoiding both iteration over digits and floating point.

<!-- @steps -->
1. Take the absolute value if negatives are possible.
2. Compare N against 9, then 99, then 999, and so on.
3. Return the position of the first boundary that N does not exceed.
4. Since a 64-bit integer has at most 19 digits, the comparison chain has a fixed maximum length.
5. No floating-point value is involved at any point, so no rounding can occur.
...
<!-- @code java -->
```java
// This is essentially what the JDK does internally.
// Long.toString calls stringSize, which compares against a table
...
<!-- @code python -->
```python
# Python integers are unbounded, so a fixed table only covers a range.
...
<!-- @why -->
The core trace. Every other approach is a different route to this same count, so the mechanism is worth having concrete.

<!-- @walkthrough -->
1. The counter starts at 0 and 12345 is greater than zero.
2. Increment to 1, and 12345 / 10 gives 1234, discarding the 5.
3. Increment to 2, and 1234 / 10 gives 123.
4. Increment to 3, and 123 / 10 gives 12.
5. Increment to 4, and 12 / 10 gives 1.
6. Increment to 5, and 1 / 10 gives 0 because integer division discards the fraction.
7. The condition fails and the counter holds 5, matching the five digits.
...
<!-- @why -->
Confirms the final single digit still costs one full iteration, which is exactly where an off-by-one would appear.

<!-- @walkthrough -->
1. The counter starts at 0 and 7789 is greater than zero.
2. Increment to 1, and 7789 / 10 gives 778.
3. Increment to 2, and 778 / 10 gives 77.
4. Increment to 3, and 77 / 10 gives 7.
5. Increment to 4, and 7 / 10 gives 0, since 0.7 truncates to 0.
6. The loop ends with the counter at 4.

<!-- @example -->

<!-- @input -->
N = 0, run through all five approaches

<!-- @output -->
Correct answer is 1. Two approaches fail without a guard.

<!-- @why -->
The same input distinguishes all five approaches, which makes it the clearest single test of whether an implementation is complete.

<!-- @walkthrough -->
1. The iterative loop tests N greater than zero, which fails immediately, so it returns 0 — wrong without a guard.
2. The recursion uses n < 10 as its base case, which 0 satisfies, so it returns 1 correctly with no extra code.
3. The logarithm approach is worst: log10 of zero is undefined, giving negative infinity in C++ and Java and raising ValueError in Python.
4. String conversion returns 1 naturally, since converting zero produces the single character 0.
5. The lookup table returns 1 naturally, since 0 is not greater than 9 and the first comparison matches.
6. So the choice of base case or boundary decides whether zero needs handling at all.

<!-- @example -->

<!-- @input -->
N = 999999999999999 (fifteen nines)

<!-- @output -->
Four approaches return 15. The logarithm formula returns 16.

<!-- @why -->
Measured in both python3 and g++ on this machine. The value fits in a 64-bit integer, so this is a reachable input rather than a hypothetical.

<!-- @walkthrough -->
1. The true value of log10 for this number is 14.999999999999999996 and a fraction more, just below 15.
2. At that magnitude the gaps between representable doubles are wide enough that no double exists between the true value and 15.0.
3. The library therefore returns exactly 15.0 as the nearest representable value.
4. Casting keeps 15 and adding one produces 16, when the correct answer is 15.
5. The iterative loop performs fifteen divisions and returns 15, touching no floating-point value.
6. The lookup table finds that the number is not greater than the fifteenth boundary and returns 15, also with no floating point.
...
<!-- @doubt -->
### Why divide by 10 rather than take the modulo?
...
<!-- @doubt -->
### Is the O(1) logarithm formula better than the O(log N) loop?
...
<!-- @doubt -->
### Why exactly does the logarithm formula fail?
...
<!-- @doubt -->
### How can an approach be constant time without using a logarithm?
...
<!-- @doubt -->
### Why does recursion use more memory than the loop?
...
<!-- @doubt -->
### How do I handle zero and negative numbers?
...
<!-- @doubt -->
### Why not just use len(str(n)) and move on?
