...
<!-- @approach -->
### Declaring Values of Each Type

<!-- @idea -->
Create one value of each core type — and see where static and dynamic typing diverge.

<!-- @steps -->
1. Choose the type that matches the kind of value you need to store.
2. In C++ and Java, write the type name before the variable name.
3. In Python, write only the name and let the interpreter infer the type from the value.
4. Assign the value, matching the literal form to the type — quotes for text, a decimal point for floating point.
5. The type is now fixed in C++ and Java, and free to change on reassignment in Python.
...
<!-- @approach -->
### Inspecting Size, Range, and Type

<!-- @idea -->
Ask the language directly how big a type is and what it can hold, instead of memorising tables.

<!-- @steps -->
1. Pick the type you want to inspect.
2. In C++, use sizeof to get its size in bytes and the numeric_limits template for its bounds.
3. In Java, read the MAX_VALUE and MIN_VALUE constants on the matching wrapper class.
4. In Python, use type() to see what a value currently is, since the type lives on the value rather than the name.
5. Confirm the limit by comparing it against the largest value your problem could produce.
...
<!-- @idea -->
Work out the largest value your answer can reach, then pick a type that holds it — the habit that prevents overflow bugs.

<!-- @steps -->
1. Read the problem constraints and find the largest input values.
2. Work out the biggest number the computation could produce, not just the biggest input.
3. Compare that against the type's maximum: roughly 2 × 10⁹ for a 32-bit int, roughly 9 × 10¹⁸ for a 64-bit integer.
4. If it exceeds the int limit, declare the value as long long in C++ or long in Java.
5. In Python, skip this step entirely — integers resize themselves.
6. Watch intermediate results too: a product can overflow even when the final answer would have fit.
...
<!-- @input -->
int x = 5; then attempt x = "hello";

<!-- @output -->
C++ and Java: compile error. Python: works fine.

<!-- @why -->
The cleanest demonstration of the static/dynamic divide, which is the main concept of this subtopic.

<!-- @walkthrough -->
1. In C++ and Java the name x was declared as int, binding it to that type permanently.
2. Assigning text to it violates that binding, so the compiler rejects the program before it runs.
3. In Python the name x was never given a type — it simply referred to the integer 5.
4. Reassigning makes x refer to a string instead, and the old integer is discarded.
5. The same code is an error in one language and ordinary in another, purely because of static versus dynamic typing.

<!-- @example -->

<!-- @input -->
int a = 100000, b = 100000; compute a * b as an int

<!-- @output -->
-1794967296 instead of 10000000000

<!-- @why -->
The overflow bug in its most common form. Every step of the arithmetic is right; only the type was wrong.

<!-- @walkthrough -->
1. The true product is 10,000,000,000.
2. A 4-byte int can only hold up to 2,147,483,647.
3. The result exceeds that, so the extra high bits are discarded.
4. What remains is reinterpreted as a signed value, which lands on a negative number.
5. No warning or error is produced — the program runs and reports a confidently wrong answer.

<!-- @example -->

<!-- @input -->
Compute 0.1 + 0.2 and compare it to 0.3

<!-- @output -->
0.30000000000000004, and the equality check is false

<!-- @why -->
Shows that a type's limits are about precision as well as size, and explains a bug that looks like the computer doing arithmetic wrong.

<!-- @walkthrough -->
1. 0.1 and 0.2 cannot be represented exactly in binary floating point, the same way 1/3 cannot be written exactly in decimal.
2. Each is stored as the closest representable value, slightly off from the true number.
3. Adding the two approximations accumulates both small errors.
4. The result is very close to 0.3 but not identical to the stored form of 0.3.
5. Comparing with == returns false, so equality on floating point is unreliable by nature.
...
<!-- @doubt -->
### When should I use int and when should I use long long?
...
<!-- @doubt -->
### Why does 7 / 2 give 3 instead of 3.5?
...
<!-- @doubt -->
### Python doesn't make me declare types. Does it not have them?
...
<!-- @doubt -->
### float or double — which should I use?
...
<!-- @doubt -->
### Does char store a letter or a number?
...
<!-- @doubt -->
### Do I need to memorise all these sizes and ranges?
