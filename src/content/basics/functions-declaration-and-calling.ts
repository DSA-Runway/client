...
/**
 * Subtopic 16 of Basics — the last Easy one, and the first construct that is
 * about structuring code rather than controlling it.
 *
 * Scope discipline matters here. Four Medium subtopics own the deeper material:
 * Function Parameters and Return Values (17), Pass by Value vs Pass by Reference
 * (18), Variable Scope and Lifetime (19), and Function Overloading (20). So this
 * container covers what a function IS, how to define it, how to call it, and
 * what return does — and points forward rather than duplicating.
 *
 * The genuinely interesting three-way divergence is declaration ORDER, and it is
 * usually taught wrong. GFG says Python functions "must be defined before they
 * are called", which is true but hides the mechanism: in Python the constraint is
 * on EXECUTION order, not file order, which is why mutual calls between functions
 * work regardless of who is written first.
 */
  ...
  approaches: [
    {
      ...
      idea: "Give a block a name, then run it by that name from anywhere.",
      steps: [
        "Choose a name describing what the block does, preferably a verb.",
        "Write the function with its return type, name, parameter list and body.",
        "Place it where the language allows it to be seen by the caller.",
        "Call it by name with parentheses, supplying any required arguments.",
        "Execution leaves the caller, runs the body, and resumes at the exact point of the call.",
      ],
      code: [
        {
          language: "cpp",
...
print(result)        # None`,
    ...
    {
      name: "Returning a Value, or Returning Nothing",
      idea: "Send a result back to the caller — and know that return also ends the function on the spot.",
      steps: [
        "Decide whether the function produces a value or performs an action.",
        "If it produces a value, declare the matching return type and return that value.",
        "If it performs an action, declare it as returning nothing.",
        "Use return with no value to exit such a function early.",
        "Ensure every path through a value-returning function reaches a return statement.",
      ],
      code: [
        {
          language: "cpp",
...
int bad(int x) {
    ...
    // no return here — the caller receives garbage
        ...
        {
          language: "java",
          code: `// return ends the method immediately
...
print(low, high)   # 1 7`,
    ...
    {
      name: "Declaration Order and Where Functions Can Live",
      idea: "Satisfy each language's rule about when a function must be visible to its caller.",
      steps: [
        "Determine whether the language resolves names by file position or by whole-unit analysis.",
        "In C++, define the function above its first call, or declare a prototype near the top.",
        "In Java, place the method anywhere inside the class, and mark it static if main calls it directly.",
        "In Python, ensure the def statement has executed before the call actually runs, not merely before it is written.",
        "Verify by checking that mutually calling functions still resolve correctly.",
      ],
      code: [
        {
          language: "cpp",
...
// }`,
        ...
        {
          language: "python",
          code: `# THE ERROR — the def has not executed yet
...
print(is_even(4))   # True`,
  ...
  examples: [
    {
      input: "int total = add(2, 3) * 10;  where add returns a + b",
      output: "50",
      walkthrough: [
        "Execution reaches the call and pauses, recording exactly where to resume.",
        "The values 2 and 3 are handed to the function's parameters.",
        "Control transfers into the function body, where a + b evaluates to 5.",
        "return sends 5 back and ends the function immediately.",
        "Control resumes at the precise position of the call, not at the next line.",
        "The returned 5 takes the call's place in the expression, so 5 times 10 is computed.",
        "total is assigned 50.",
      ],
      why: "Shows a call is an expression that resumes mid-statement, which is the detail that makes the return-address mechanism necessary.",
    },
    {
      input: "A function that returns the first negative value, given [3, 7, -2, -8]",
      output: "-2, with the last element never examined",
      walkthrough: [
        "The loop compares 3, which is not negative, so the body ends normally.",
        "It compares 7, which is also not negative.",
        "It compares -2, which is negative, so return executes.",
        "return ends the loop and the entire function in one step, sending -2 back.",
        "The final element -8 is never examined, because the function is no longer running.",
        "A break would have left only the loop, and the code after it would still have run.",
      ],
      why: "Distinguishes return from break — return exits every enclosing loop at once, which is why it is the cleanest escape from nesting.",
    },
    {
      input: "A value-returning function with a path that reaches no return statement",
      output: "C++: undefined behaviour. Java: compile error. Python: None.",
      walkthrough: [
        "In C++ the function ends without a return, so the caller reads whatever happens to be in the return location.",
        "The program compiles, often with only a warning, and produces an unpredictable value.",
        "In Java the compiler analyses every path and proves at least one reaches no return.",
        "It refuses to compile, reporting a missing return statement before the program can ever run.",
        "In Python execution simply falls off the end of the function body.",
        "The function returns None, which flows into the caller with no error and may fail much later somewhere unrelated.",
      ],
      why: "The same mistake produces a runtime unknown, a build failure, and a silent None — so the habit that protects you differs by language.",
    },
    {
      input: "Calling a function that is written below the call site",
      output: "C++: compile error without a prototype. Java: fine. Python: depends on when the call runs.",
      walkthrough: [
        "C++ compiles top to bottom, so at the call site the name has not been seen and the compiler reports it as undeclared.",
        "Adding a prototype above the call resolves it, since the compiler now knows the signature.",
        "Java reads the entire class before resolving any name, so a method defined below main is found without difficulty.",
        "In Python a def is a statement that creates the name when it executes.",
        "A call written at the top level before that def has run raises NameError, because the name does not exist yet.",
        "A call written inside another function is only looked up when that function actually runs, by which point the def has executed and it succeeds.",
      ],
      why: "The precise rule differs from the usual summary: Python constrains execution order rather than file order, which is why mutual recursion works there without any forward declaration.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "What is the difference between declaring and defining a function?",
      answer:
    ...
    {
      question: "Why does C++ complain about a function I clearly defined further down?",
      answer:
    ...
    {
      question: "What does return actually do?",
      answer:
    ...
    {
      question: "What happens if I forget to return a value?",
      answer:
    ...
    {
      question: "Does Python have void functions?",
      answer:
    ...
    {
      question: "Why does Java say my method cannot be referenced from a static context?",
      answer:
    ...
    {
      question: "Do Python functions have to be written above the code that calls them?",
      answer:
    ...
    {
      question: "How do I know when to split code into a function?",
      answer:
