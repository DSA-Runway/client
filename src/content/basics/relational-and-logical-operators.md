...
<!-- @idea -->
Ask a yes-or-no question about two values and receive a boolean.

<!-- @steps -->
1. Choose the two values to compare.
2. Pick the operator matching the question you are asking.
3. The comparison evaluates to true or false.
4. For decimals, compare the difference against a small tolerance rather than testing exact equality.
5. For text and objects, use the language's content-comparison method rather than assuming the operator compares contents.
...
<!-- @idea -->
Build a compound condition from several simple ones.

<!-- @steps -->
1. Write each simple condition separately and confirm it produces a boolean.
2. Join them with AND when every condition must hold.
3. Join them with OR when any one of them is enough.
4. Apply NOT to invert a condition.
5. Add parentheses whenever AND and OR appear in the same expression.
...
<!-- @idea -->
Order conditions so a check that could fail is protected by one that runs first.

<!-- @steps -->
1. Identify the condition that could fail or crash — an array access, a division, a call on something possibly absent.
2. Identify the condition that makes it safe — a bounds check, a non-zero check, a presence check.
3. Place the safety check on the left of the AND.
4. Place the risky condition on the right, where it only runs if the guard passed.
5. For OR, place the cheap or decisive condition on the left so the expensive one is often skipped.
...
<!-- @input -->
age = 20, hasID = true; evaluate age >= 18 && hasID

<!-- @output -->
true

<!-- @why -->
Shows the left-to-right order explicitly, which is the foundation the short-circuit behaviour is built on.

<!-- @walkthrough -->
1. The left condition age >= 18 is evaluated first, comparing 20 against 18.
2. 20 is greater than 18, so the left side produces true.
3. Because the left side is true, the result is not yet decided and evaluation continues.
4. The right operand hasID is already a boolean holding true.
5. AND requires both sides to be true, and both are, so the whole expression is true.

<!-- @example -->

<!-- @input -->
arr has 3 elements, i = 3; evaluate i < n && arr[i] == target

<!-- @output -->
false, with no crash

<!-- @why -->
The bounds-guard idiom in the exact situation it exists for, and the reason condition order is a correctness concern rather than style.

<!-- @walkthrough -->
1. The left condition i < n compares 3 against 3.
2. 3 is not less than 3, so the left side produces false.
3. With AND, a false left operand decides the whole expression regardless of the right side.
4. Evaluation stops immediately and arr[3] is never read.
5. Reversing the order would read arr[3] first, which is out of bounds and crashes before any check runs.

<!-- @example -->

<!-- @input -->
x = 20; evaluate 1 <= x <= 10 in C++, Java, and Python

<!-- @output -->
C++: true (wrong). Java: compile error. Python: False (correct).

<!-- @why -->
One line, three different outcomes. It is the clearest case of a habit from one language producing a silent bug in another.

<!-- @walkthrough -->
1. In C++, 1 <= x is evaluated first and produces true because 20 is greater than 1.
2. That boolean converts to the integer 1, and the expression becomes 1 <= 10.
3. That is true, so the condition passes even though 20 is far outside the range — and it would pass for every value of x.
4. In Java the same first step produces a boolean, and comparing a boolean against an int is illegal, so compilation fails.
5. In Python the chain is read mathematically as both 1 <= x and x <= 10, and since 20 exceeds 10 it correctly evaluates to False.
...
<!-- @doubt -->
### Why doesn't == work for comparing strings in Java?
...
<!-- @doubt -->
### What is short-circuit evaluation and why should I care?
...
<!-- @doubt -->
### Why is 1 <= x <= 10 wrong in C++ when it works in Python?
...
<!-- @doubt -->
### How do I correctly negate a compound condition?
...
<!-- @doubt -->
### What does an if condition actually receive?
