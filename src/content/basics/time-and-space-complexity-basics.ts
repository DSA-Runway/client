...
/**
 * Subtopic 28 of Basics (Hard #1). The payoff container for the whole module —
 * a dozen earlier subtopics produced measured numbers, and this one turns them
 * into the framework that explains them.
 *
 * DELIBERATE DESIGN: every complexity claim here is anchored to a figure already
 * measured earlier in this curriculum, not to a hypothetical. Count Digits gave
 * a loop bounded at 19. GCD gave 44 steps for the hardest pair under two
 * billion. Prime Check gave 100,001 operations dropping to 315. Nested Loops
 * gave the polynomial-versus-exponential correction. The abstraction lands
 * better when the student has already seen the evidence.
 *
 * SOURCES
 * - GeeksforGeeks, "Knowing the complexity in competitive programming" — the
 *   10^8 operations-per-second heuristic and the constraint-to-complexity chart.
 * - GeeksforGeeks, "How an Online Judge Works and How to Avoid TLE".
 *
 * COMPUTED ON THIS MACHINE rather than recalled: the growth table, and the
 * largest n each complexity class can handle inside a 10^8 budget —
 * O(n!) n<=11, O(2^n) n<=26, O(n^3) n<=464, O(n^2) n<=10,000,
 * O(n log n) n<=4,523,071, O(n) n<=100,000,000. Solved by binary search on
 * each cost function rather than copied from a chart.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Counting Operations as a Function of Input Size",
      idea: "Express the number of steps in terms of n, then name the shape of that expression.",
      steps: [
        "Identify what n represents — the array length, the value of a number, the number of nodes.",
        "Count how many times the innermost operation executes, as an expression in n.",
        "Check how that expression responds when n doubles.",
        "Name the growth: unchanged is constant, doubled is linear, quadrupled is quadratic.",
        "Write it in Big O notation, which describes the growth rather than the exact count.",
      ],
      code: [
        {
          language: "cpp",
          code: `// O(1) — constant. The work does not depend on n at all.
...
# The syntax is identical and the complexity is not.`,
    ...
    {
      name: "Dropping Constants and Lower-Order Terms",
      idea: "Reduce an operation count to its dominant shape, since that is what survives as n grows.",
      steps: [
        "Write the exact operation count as an expression in n.",
        "Remove any multiplicative constants, since they do not affect how the cost responds to a change in n.",
        "Remove any terms that grow more slowly than the largest one.",
        "The remaining term is the complexity.",
        "Remember separately that the discarded constant is still real work, even though Big O ignores it.",
      ],
      code: [
        {
          language: "cpp",
          code: `// Exact count: 3n operations.  Complexity: O(n)
for (int i = 0; i < n; i++) {
...
# The approximation gets better as n grows, which is the regime it is for.`,
          annotations: {
            22: "The reason the rule is principled rather than lazy.",
            27: "Honest about where it breaks down: Big O is a statement about large n.",
          },
        },
      ],
    },
    {
      name: "Reading Complexity from Code",
      idea: "Apply four structural rules to derive the complexity without counting individual operations.",
      steps: [
        "Identify each loop and how many times it runs relative to n.",
        "Multiply the counts of nested loops, since the inner one restarts for every outer iteration.",
        "Add the counts of sequential blocks, then keep only the largest.",
        "Recognise a logarithm wherever a value is divided or multiplied by a constant each iteration.",
        "Account for recursion by multiplying the number of calls by the work done in each.",
        "Check whether the inner loop's bound depends on the outer counter, which changes the total.",
      ],
      code: [
        {
          language: "cpp",
          code: `// RULE 1 — sequential blocks ADD, then take the maximum
void f1(int n) {
...
//   Prime Check tests up to sqrt(n)        -> O(sqrt n), 315 ops vs 100,001`,
          annotations: {
            17: "The counter multiplies rather than increments, so it reaches n in log2(n) steps.",
            31: "A triangular loop looks cheaper and is — by a factor of two, which Big O discards.",
          },
        },
        {
          language: "java",
          code: `// Sequential — add, then take the maximum
...
static int fib(int n) {
    ...
    // This is genuinely exponential, unlike any depth of nested loops.`,
        ...
        {
          language: "python",
          code: `# Sequential — add, take the maximum
def f1(n):
...
# This is the single most valuable complexity fact in Python.`,
    ...
    {
      name: "Choosing an Algorithm from the Constraints",
      idea: "Read the input limits in the problem statement and rule out every complexity that cannot finish.",
      steps: [
        "Find the maximum value of n stated in the constraints.",
        "Assume roughly 100 million elementary operations per second.",
        "Compute what your intended approach would cost at that maximum n.",
        "Compare that against the budget, and reject the approach if it exceeds it.",
        "Work backwards from the limit to the complexity class you need.",
        "Only then start writing code.",
      ],
      code: [
        {
          language: "cpp",
          code: `// Budget: about 10^8 operations per second.
...
// the problem actually constrains.`,
        ...
        {
          language: "python",
          code: `# n <= 10^5, so a nested scan is 10^10 operations — rejected.
...
def total_fast(arr):
  ...
  examples: [
    {
      input: "Two loops over n, once nested and once in sequence",
      output: "Nested is O(n²). Sequential is O(n).",
      walkthrough: [
        "In the nested version the inner loop restarts completely for every iteration of the outer one.",
        "That gives n multiplied by n executions of the body, which is n squared.",
        "In the sequential version each loop runs to completion before the next begins.",
        "That gives n plus n executions, which is 2n.",
        "Dropping the constant leaves O(n), so the two structures differ by an entire complexity class.",
        "At n equal to one million that is a million operations against a trillion — instant against never.",
      ],
      why: "The single most common misreading. The visual difference between the two is one level of indentation, and the cost difference is a factor of a million at realistic input sizes.",
    },
    {
      input: "Prime checking to n-1 versus stopping at √n, on n = 100003",
      output: "100,001 operations against 315 — the same answer",
      walkthrough: [
        "The linear scan tests every candidate divisor from 2 up to 100002.",
        "Since the number is prime none divides evenly, so there is no early exit and all 100,001 tests run.",
        "The square-root version stops once the candidate squared exceeds the number, which is at 316.",
        "That is 315 operations, and it reaches an identical conclusion.",
        "Everything beyond the square root could only find the larger partner of a pair already tested.",
        "The measured reduction is 317-fold, and it came from changing one loop bound.",
      ],
      why: "Measured in the Prime Check subtopic rather than assumed here. It shows a complexity improvement translating into a concrete count on a real input.",
    },
    {
      input: "The 6k ± 1 refinement against the plain square-root loop, on n = 1000000007",
      output: "10,540 operations against 31,621 — both O(√n)",
      walkthrough: [
        "The plain loop tests every candidate up to the square root, roughly 31,623 of them.",
        "The refined version eliminates all multiples of 2 and 3 first, which is two thirds of every integer.",
        "It then steps by 6, testing two candidates per stop rather than six.",
        "The measured counts are 31,621 against 10,540, a ratio of exactly 3.0.",
        "Both are O of the square root of n, so Big O treats them as equivalent.",
        "The three-fold saving is real work that the notation deliberately discards.",
      ],
      why: "The clearest demonstration of Big O's limitation. It is answering how cost grows, not how much cost there is, and both facts matter when a solution sits near a time limit.",
    },
    {
      input: "A problem stating n ≤ 10⁵, and an idea that requires nested loops",
      output: "10¹⁰ operations — about 100 seconds. Rejected before writing any code.",
      walkthrough: [
        "The constraint gives a maximum n of 100,000.",
        "A nested loop over the same data performs n squared operations, which is 10 to the 10th.",
        "At roughly 100 million operations per second that is about 100 seconds.",
        "Typical time limits are 1 to 2 seconds, so the approach cannot pass regardless of how it is written.",
        "Working backwards, a budget of 10 to the 8th at n equal to 10 to the 5th permits about 1,000 operations per element.",
        "That points at O(n log n) or O(n), which is what a sort or a hash set provides.",
      ],
      why: "The practical payoff of the entire subtopic — the algorithm is chosen from the constraints before a line is written, rather than discovered after a timeout.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why not just measure how long the code takes?",
      answer:
    ...
    {
      question: "Why do we drop constants? Isn't 3n genuinely slower than n?",
      answer:
    ...
    {
      question: "What is the difference between polynomial and exponential?",
      answer:
    ...
    {
      question: "How do I know which complexity a problem needs?",
      answer:
    ...
    {
      question: "Should I quote the best, average, or worst case?",
      answer:
    ...
    {
      question: "Does recursion cost more than a loop?",
      answer:
    ...
    {
      question: "Is a better complexity class always the better solution?",
      answer:
    ...
    {
      question: "Does the 10 to the 8th operations rule hold for Python?",
      answer:
