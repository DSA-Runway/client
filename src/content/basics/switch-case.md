...
<!-- @idea -->
Branch on one variable's exact value, with a break ending each case and a default catching the rest.

<!-- @steps -->
1. Confirm the branching compares a single variable against fixed values, not ranges or compound conditions.
2. Write the switch with that variable as its expression.
3. Add one case label per value, each followed by the statements for that value.
4. End each case with a break so execution leaves the switch instead of continuing downward.
5. Add a default at the end to handle values no case matched.
...
<!-- @code python -->
```python
# Python has no switch statement. From 3.10, match/case is the closest form.
day = 3
...
<!-- @approach -->
### Fall-Through: Bug and Tool

<!-- @idea -->
Understand that a case is an entry point, so execution continues downward until a break — then use that deliberately.

<!-- @steps -->
1. Recognise that matching a case only decides where execution begins.
2. Trace what runs after the matched block when no break follows it.
3. Add a break at the end of every case whose code should not continue into the next.
4. To group values that share a body, stack their case labels with no statements between them.
5. Place the shared statements after the last stacked label, ending with a single break.
...
<!-- @code python -->
```python
# Fall-through does not exist in Python. match/case cannot produce the bug.
day = 1
...
<!-- @approach -->
### Multi-Way Branching in Python

<!-- @idea -->
Choose among Python's three real options, since the language has no switch statement.

<!-- @steps -->
1. Determine whether the branching is a value-to-value mapping or genuinely runs different logic.
2. For a value-to-value mapping, build a dictionary and look the value up.
3. Supply a default to the lookup so a missing key does not raise an error.
4. For different logic per branch on Python 3.10 or later, use match/case.
5. For anything involving ranges, or when the Python version may be older, use an if/elif ladder.
...
<!-- @code cpp -->
```cpp
// For comparison: the same three intents in C++
...
<!-- @code java -->
```java
// For comparison: the same three intents in Java
...
<!-- @input -->
day = 3 in a switch with cases 1, 2, 3 and a default, each ending in break

<!-- @output -->
Wednesday

<!-- @why -->
Shows the jump model explicitly: unlike a ladder, the earlier cases were never tested, only bypassed.

<!-- @walkthrough -->
1. The expression day is evaluated once, producing 3.
2. Control jumps directly to the label case 3 — cases 1 and 2 are not compared or examined at all.
3. The statements under case 3 run, printing Wednesday.
4. The break is reached, which exits the switch immediately.
5. The default block is skipped, and execution continues after the closing brace.

<!-- @example -->

<!-- @input -->
day = 1 in a switch whose cases have no break statements

<!-- @output -->
Monday, then Tuesday, then Wednesday — all three

<!-- @why -->
The most common switch bug, and it produces extra output rather than an error — so it is often mistaken for a problem elsewhere.

<!-- @walkthrough -->
1. The expression evaluates to 1 and control jumps to case 1.
2. Monday is printed.
3. With no break, execution simply continues to the next line, which happens to be under case 2.
4. The case 2 label is not a barrier — it is only an entry point, and execution is already inside the switch.
5. Tuesday is printed, then execution continues into case 3 and prints Wednesday.
6. The switch ends only because there are no more statements, not because anything stopped it.

<!-- @example -->

<!-- @input -->
c = 'e' in a switch with cases 'a', 'e', 'i', 'o', 'u' stacked above one shared block

<!-- @output -->
vowel

<!-- @why -->
The same fall-through that caused the previous bug, used deliberately — which is why the behaviour exists rather than being a design flaw.

<!-- @walkthrough -->
1. The expression evaluates to the character e and control jumps to case 'e'.
2. There are no statements under that label, so execution falls straight through to the next.
3. It continues past case 'i', case 'o' and case 'u', all of which are also empty.
4. It reaches the shared block and prints vowel.
5. The break exits the switch before the default block can run.
...
<!-- @doubt -->
### Why do all my cases run instead of just the matching one?
...
<!-- @doubt -->
### When should I use switch instead of an else-if ladder?
...
<!-- @doubt -->
### Can I use a switch for ranges like score >= 90?
...
<!-- @doubt -->
### Is fall-through ever intentional?
...
<!-- @doubt -->
### Why can't I declare a variable inside a case in C++?
...
<!-- @doubt -->
### What is Java's arrow syntax, and should I use it?
