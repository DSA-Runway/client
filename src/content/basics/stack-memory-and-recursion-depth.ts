...
/**
 * Subtopic 29 of Basics (Hard #2). The memory counterpart to the complexity
 * subtopic: where the O(n) space of a recursive algorithm physically lives, and
 * what happens when it runs out.
 *
 * SOURCES
 * - GeeksforGeeks, "Function Call Stack in C" — frames, and what each holds.
 * - GeeksforGeeks, "Python | Handling recursion limit" — setrecursionlimit.
 * - GeeksforGeeks, "Tail Recursion" and "Tail Call Optimisation in C" — the
 *   frame-reuse optimisation and why it needs the call to be the last statement.
 *
 * ONE CORRECTION TO THE SOURCE: GFG states Python's recursion limit is "usually
 * set to a small value (approx, 10^4)". Measured on this machine with Python 3,
 * sys.getrecursionlimit() returns 1000, not 10,000 — and f(998) succeeds while
 * f(5000) raises. The order-of-magnitude difference matters when estimating
 * whether a recursion will fit, so the measured figure is used here.
 *
 * MEASURED ON THIS MACHINE, not quoted:
 * - ulimit -s reports 8176 KB, roughly an 8 MB stack.
 * - Recursion with tiny frames (~48 bytes) survived past depth 160,000.
 * - Recursion with a 1 KB local array crashed between depth 7,000 and 8,000 —
 *   which is 8 MB divided by 1 KB, confirming stack size / frame size = depth.
 * - THE HEADLINE: the same recursion is safe at 160,000 or fatal at 7,000
 *   depending only on how much the frame holds. A 20x difference from adding one
 *   local array. That is the non-obvious result and it drives the whole subtopic.
 * - Python raises a catchable RecursionError; C++ segfaults with no diagnostic.
 * - sys.setrecursionlimit(30000) then f(20000) succeeds — verified.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "How the Call Stack Works",
      idea: "Follow a frame from push to pop, and see what it holds while it exists.",
      steps: [
        "A call pushes a frame holding the return address, the parameters and the local variables.",
        "The parameters are initialised from the arguments as part of building the frame.",
        "The function body runs, using only its own frame's locals.",
        "A nested call pushes another frame above it, leaving the caller's frame intact underneath.",
        "Returning pops the frame, discarding every local in it at once and restoring the caller's position.",
        "Only the frames on the current call chain exist simultaneously, which is the depth.",
      ],
      code: [
        {
          language: "cpp",
...
# inspect.stack() is expensive — use it to understand, not in real code.`,
          annotations: {
            ...
            21: "Building the full stack listing on every call is far slower than the work being measured.",
          },
        },
      ],
    },
    {
      name: "When Recursion Runs Out of Stack",
      idea: "Find the depth limit, understand what sets it, and recognise how each language reports hitting it.",
      steps: [
        "Determine the stack size available to the program, which is fixed before it starts.",
        "Estimate the size of one frame from the parameters and local variables it holds.",
        "Divide the stack size by the frame size to approximate the maximum depth.",
        "Compare that against the deepest recursion the problem's constraints permit.",
        "Reduce the frame size by moving large locals out of the recursive function if the margin is thin.",
        "Recognise the failure mode: a catchable exception in Python and Java, and a silent crash in C++.",
      ],
      code: [
        {
          language: "cpp",
          code: `// MEASURED ON THIS MACHINE: ulimit -s reports 8176 KB, about 8 MB.
...
//   a frame with a 1 KB array -> roughly 8,000 levels`,
          annotations: {
            ...
            23: "The worst reporting of the three languages, which makes estimating the depth in advance more important here.",
          },
        },
        {
          language: "java",
          code: `// Java throws StackOverflowError — technically catchable, and catching
...
# when estimating whether a recursion will fit.`,
    ...
    {
      name: "Converting Recursion to Iteration",
      idea: "Remove the stack cost by carrying the state yourself, either in loop variables or in an explicit stack.",
      steps: [
        "Determine whether the recursive call is in tail position, meaning nothing happens after it returns.",
        "If it is, the parameters carry all the state, so rewrite the call as reassigning those parameters inside a loop.",
        "If it is not, identify what must be remembered across the call and push it onto a stack you manage.",
        "Replace the recursion with a loop that pops from that stack until it is empty.",
        "Note that the depth is now bounded by heap memory rather than by the fixed call stack.",
        "Prefer the iterative form when the depth scales with the input rather than with something bounded.",
      ],
      code: [
        {
          language: "cpp",
          code: `// TAIL RECURSIVE — the call is the last thing the function does
...
// a far higher ceiling, at the cost of writing the bookkeeping yourself.`,
        ...
        {
          language: "java",
          code: `// Java performs NO tail-call optimisation, so writing tail-recursive
...
#   Degenerate tree (every node has one child): n levels      -> CONVERT`,
  ...
  examples: [
    {
      input: "The same recursion with a few bytes of locals, and with a 1 KB local array",
      output: "Past depth 160,000 versus between 7,000 and 8,000",
      walkthrough: [
        "The stack on this machine is 8,176 KB, roughly 8 MB, reported by ulimit -s.",
        "The first version declares almost nothing, giving a frame of around 48 bytes.",
        "Dividing 8 MB by 48 bytes predicts a depth in the low hundreds of thousands, and the measurement passed 160,000.",
        "The second version declares a 1 KB array as a local, so every frame carries an extra kilobyte.",
        "Dividing 8 MB by 1 KB predicts about 8,000 frames, and the measurement crashed between 7,000 and 8,000.",
        "Nothing about the algorithm changed — only what each frame holds — and the safe depth fell by a factor of twenty.",
      ],
      why: "The non-obvious result of the subtopic. Frame size is invisible in the code and decides whether a recursion survives, which is why large locals do not belong in recursive functions.",
    },
    {
      input: "A function called one million times in a loop, versus recursing one million levels deep",
      output: "The loop is completely safe. The recursion crashes.",
      walkthrough: [
        "In the loop, each call pushes a frame, runs, and pops before the next call begins.",
        "At any instant only one frame exists beyond the caller's, so the stack never grows.",
        "A million calls therefore cost no more stack than a single call does.",
        "In the recursion, each call makes another before returning, so no frame is popped until the base case is reached.",
        "A million frames must coexist, which at even a hundred bytes each is a hundred megabytes against an eight megabyte limit.",
        "The stack cost is the maximum depth, never the total number of calls.",
      ],
      why: "The distinction that determines whether recursion is safe. It reframes the question from how often a function is called to how many calls are open at once.",
    },
    {
      input: "Exceeding the recursion limit in Python, Java, and C++",
      output: "Python: catchable RecursionError. Java: StackOverflowError. C++: silent segmentation fault.",
      walkthrough: [
        "Python enforces its own counter before the real stack is touched, defaulting to 1,000 on this machine.",
        "Measured: a recursion of depth 998 succeeds and one of depth 5,000 raises RecursionError, which is an ordinary catchable exception.",
        "Raising the ceiling with setrecursionlimit to 30,000 then allowed a 20,000-deep recursion to complete, also verified.",
        "Java throws StackOverflowError when the real stack is exhausted, which can be caught to report the depth though catching it to continue is unwise.",
        "C++ provides nothing: the process receives a segmentation fault and dies with no exception, message or indication of cause.",
        "So diagnosability decreases from Python to Java to C++, while the underlying limit is the same physical constraint.",
      ],
      why: "Knowing how the failure will present decides how much you must reason about depth in advance. In C++ there is no runtime signal to fall back on.",
    },
    {
      input: "A recursion with no reachable base case",
      output: "Stack overflow within milliseconds",
      walkthrough: [
        "Each call immediately makes another without any condition stopping it.",
        "No frame is ever popped, so the stack grows without bound.",
        "At a few dozen bytes per frame, eight megabytes is consumed in a fraction of a second.",
        "The failure is therefore almost instant rather than gradual.",
        "The same happens with a base case that exists but is never reached, such as decrementing by two toward zero from an odd number.",
        "A stack overflow that appears immediately almost always means non-termination rather than genuinely deep recursion.",
      ],
      why: "The timing is a useful diagnostic. An immediate overflow points at a broken base case, while one that takes a moment points at a depth problem.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "What is actually stored in a stack frame?",
      answer:
    ...
    {
      question: "How deep can my recursion go?",
      answer:
    ...
    {
      question: "Why did adding one local array make my recursion crash?",
      answer:
    ...
    {
      question: "Is calling a function a million times dangerous?",
      answer:
    ...
    {
      question: "What is tail recursion and can I rely on it?",
      answer:
    ...
    {
      question: "When should I convert recursion to a loop?",
      answer:
    ...
    {
      question: "How do the three languages report a stack overflow?",
      answer:
    ...
    {
      question: "Should I just increase the stack size?",
      answer:
