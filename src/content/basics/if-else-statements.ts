...
/**
 * Subtopic 8 of Basics — the first control-flow lesson. Everything from
 * data-types through relational operators was groundwork for the condition this
 * statement consumes.
 *
 * Scope note: chaining many conditions is subtopic 9 (Else-If Ladder), so this
 * container covers if, if-else, nesting, and the conditional expression, and
 * hands the multi-branch case forward rather than duplicating it.
 *
 * The bug given the most weight is optional braces in C++/Java — it is silent,
 * it appears when code is *edited* rather than written, and Python's indentation
 * model makes it impossible, which is a genuinely useful contrast.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Running Code Conditionally with if",
      idea: "Guard a block so it executes only when a condition holds.",
      steps: [
        "Write the condition as an expression that evaluates to a boolean.",
        "Attach a block containing the statements that should run when it is true.",
        "At runtime the condition is evaluated once.",
        "If it is true, the block runs and execution continues after it.",
        "If it is false, the block is skipped entirely and execution continues after it.",
      ],
      code: [
        {
          language: "cpp",
...
if names:
    ...
    {
      name: "Choosing Between Two Paths with if-else",
      idea: "Provide an alternative block so exactly one of two paths always runs.",
      steps: [
        "Write the condition and the block to run when it is true.",
        "Attach an else with the block to run when it is false.",
        "At runtime the condition is evaluated once.",
        "Whichever block matches runs, and the other is skipped completely.",
        "Execution rejoins after the else block, regardless of which path was taken.",
      ],
      code: [
        {
          language: "cpp",
...
if logged_in and is_admin:
    ...
    {
      name: "The Conditional Expression",
      idea: "Pick between two values in a single expression, rather than two blocks.",
      steps: [
        "Confirm both branches produce a value for the same purpose, rather than performing different actions.",
        "Write the condition and the two candidate values in the language's order.",
        "The condition is evaluated once.",
        "Only the matching value is evaluated and produced; the other is not.",
        "Assign or use the resulting value like any other expression.",
      ],
      code: [
        {
          language: "cpp",
...
print(max(a, b))   # 9`,
  ...
  examples: [
    {
      input: "n = 7; run if (n % 2 == 0) print Even else print Odd",
      output: "Odd",
      walkthrough: [
        "The condition is evaluated first: n % 2 computes the remainder of 7 divided by 2, which is 1.",
        "That result is compared against 0, and 1 does not equal 0, so the condition is false.",
        "The if block is skipped entirely — Even is never printed.",
        "Control moves to the else block, which prints Odd.",
        "Execution rejoins after the else and continues with the next statement.",
      ],
      why: "The smallest complete if-else, and it reuses the modulo operator from the arithmetic lesson to show conditions are built from what came before.",
    },
    {
      input: "a = 3, b = 9, c = 5; find the largest using nested if",
      output: "b is largest",
      walkthrough: [
        "The outer condition a > b compares 3 against 9, which is false.",
        "The outer if block is skipped without evaluating anything inside it.",
        "Control enters the else block, where a second question is asked.",
        "The inner condition b > c compares 9 against 5, which is true.",
        "The inner if block runs and prints that b is largest.",
        "Only two comparisons were made in total, because the skipped branch was never entered.",
      ],
      why: "Shows that nesting is not just indentation — an entire subtree of conditions goes unevaluated when its parent branch is not taken.",
    },
    {
      input: "x = -5; if (x > 0) print A; print B;  with no braces in C++",
      output: "B is printed even though the condition was false",
      walkthrough: [
        "The condition x > 0 compares -5 against 0 and evaluates to false.",
        "Without braces, the if controls exactly one statement — the print of A.",
        "That statement is skipped, so A is not printed.",
        "The print of B was never part of the if at all; it is an ordinary next line.",
        "It runs unconditionally, producing output that contradicts what the indentation suggested.",
        "Wrapping both lines in braces attaches them both to the condition and fixes it.",
      ],
      why: "The bug appears when code is edited rather than written, so it survives the review that would have caught it on day one.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Do I need braces if my if statement has only one line?",
      answer:
    ...
    {
      question: "Why does the line after my if always run, even when the condition is false?",
      answer:
    ...
    {
      question: "What does if (x) mean when x is a number and not a comparison?",
      answer:
    ...
    {
      question: "When should I use if-else instead of two separate ifs?",
      answer:
    ...
    {
      question: "Why is if (x > 0); legal? It does nothing.",
      answer:
    ...
    {
      question: "When should I use a ternary instead of if-else?",
      answer:
    ...
    {
      question: "Why is Python's conditional expression written backwards?",
      answer:
    ...
    {
      question: "How deeply should I nest if statements?",
      answer:
