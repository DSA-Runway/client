...
/**
 * The very first subtopic a student sees. Two jobs: explain what programming
 * actually is, and set up the language choice they make at the end of Basics —
 * which is why the pipeline visualization runs all three languages side by side.
 */
  ...
  approaches: [
    {
      ...
      idea: "Write text into a file, translate it, and run it — the same three moves in every language.",
      steps: [
        "Write the source code in a plain text file with the right extension: .cpp, .java, or .py.",
        "Save the file. In Java the filename must match the class name exactly.",
        "Translate it — compile for C++ and Java, or hand it straight to the interpreter for Python.",
        "Run the result and read the output.",
        "If an error appears, read the line number it names, fix that line, and repeat.",
      ],
      code: [
        {
          language: "cpp",
...
int main() {
    ...
    {
      name: "How Code Becomes Output",
      idea: "The commands that run the translation, and what each one produces along the way.",
      steps: [
        "The source file is handed to a translator — a compiler or an interpreter.",
        "The translator checks the syntax. If a rule is broken, it reports the line and stops.",
        "C++ produces a machine-code executable; Java produces a .class bytecode file; Python produces nothing on disk and executes directly.",
        "The result runs: the operating system runs the C++ executable, the JVM runs the Java bytecode, the interpreter runs the Python source.",
        "Output appears in the terminal.",
      ],
      code: [
        {
          language: "cpp",
          code: `g++ hello.cpp -o hello    # compile: source -> executable
...
# Output: Hello, World!`,
        ...
        {
          language: "java",
          code: `javac Hello.java    # compile: source -> Hello.class (bytecode)
...
# Output: Hello, World!`,
        ...
        {
          language: "python",
          code: `python hello.py    # interpret and execute in one step
...
# Output: Hello, World!`,
  ...
  examples: [
    {
      input: 'A file hello.py containing: print("Hello, World!")',
      output: "Hello, World!",
      walkthrough: [
        "The interpreter opens the file and reads the first line.",
        "It recognises print as a built-in instruction to display something.",
        'It evaluates the value inside the parentheses — the text "Hello, World!".',
        "It sends that text to the terminal and moves to a new line.",
        "There are no further lines, so the program ends.",
      ],
      why: "The shortest complete program that exists. It proves the whole pipeline — file, translator, execution, output — works before any real logic is involved.",
    },
    {
      input: "A program that adds 5 and 3 and prints the result",
      output: "8",
      walkthrough: [
        "The program stores 5 in a named box, and 3 in another.",
        "It computes the sum of the two boxes, producing 8.",
        "It stores 8 in a third box.",
        "It sends the contents of that box to the screen.",
        "The program ends.",
      ],
      why: "Introduces the input-process-output shape that every program follows, no matter how large.",
    },
    {
      input: 'A C++ file missing a semicolon: cout << "Hi"',
      output: "A compile error naming the line, and no program is produced.",
      walkthrough: [
        "The compiler reads the line and reaches the end without finding a semicolon.",
        "It cannot tell where this statement was meant to end, so it reports an error at that line.",
        "Compilation stops. No executable file is created.",
        "Nothing runs — there is no partial output.",
        "Adding the semicolon and compiling again produces a working program.",
      ],
      why: "Shows that a compiled language catches the mistake before execution, which is exactly the difference from an interpreted language.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Which language should I choose for DSA?",
      answer:
    ...
    {
      question: "Python is slower. Will that be a problem for DSA?",
      answer:
    ...
    {
      question: "What's the practical difference between a compiler and an interpreter?",
      answer:
    ...
    {
      question: "Why does Java make me write a class just to print one line?",
      answer:
    ...
    {
      question: "Do I need to memorise all this syntax?",
      answer:
  ...
  relatedIds: ["input-and-output", "data-types", "variables-and-constants"],
};
