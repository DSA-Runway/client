...
<!-- @idea -->
Read the last digit off N and append it to a growing result, shifting the result left by one place each time.

<!-- @steps -->
1. Initialise the result accumulator to zero.
2. In Python, record the sign and take the absolute value first, since the loop does not terminate for negatives there.
3. While N is not yet exhausted, read its last digit with modulo 10.
4. Multiply the accumulator by 10 to open a units slot, then add that digit.
5. Remove the digit from N with integer division by 10.
6. When N reaches zero, the accumulator holds the reversed number; reapply the sign if one was recorded.
...
<!-- @idea -->
Express the same construction as a function calling itself, carrying the accumulator through the calls.

<!-- @steps -->
1. Define a helper taking both the remaining number and the accumulator built so far.
2. The base case is a remaining number of zero, at which point the accumulator is the answer.
3. Otherwise read the last digit, append it to the accumulator, and recurse on the number with that digit removed.
4. Each call reduces the number by one digit, so the base case is always reached.
5. Note that every pending call holds a stack frame, so the memory grows with the digit count.
...
<!-- @code cpp -->
```cpp
// Accumulator-passing style — the accumulator carries the partial result down
...
<!-- @code java -->
```java
// Java has no default arguments, so overloading supplies the initial accumulator
...
- 14: The infinite loop becomes a stack overflow in the recursive form, which at least fails loudly.

<!-- @approach -->
### String Reversal

<!-- @idea -->
Convert the number to text, reverse the characters, and convert back.

<!-- @steps -->
1. Record the sign and take the absolute value, so the minus character is not reversed into the middle.
2. Convert the number to its string representation.
3. Reverse the characters of that string.
4. Convert the reversed string back to an integer, which discards any leading zeros automatically.
5. Reapply the sign to the result.
...
<!-- @annotations -->
- 3: The most-negative-value trap, avoided by widening before taking the absolute value.
- 4: StringBuilder has a built-in reverse; String itself does not.

<!-- @approach -->
### Overflow-Safe Iterative

<!-- @idea -->
Test whether the next multiplication will overflow before performing it, rather than checking the result afterwards.

<!-- @steps -->
1. Read the last digit and remove it from N, before touching the accumulator.
2. Compare the accumulator against the type's maximum divided by 10 — a comparison that cannot itself overflow.
3. If the accumulator already exceeds that, the next multiplication would overflow, so report failure.
4. If it equals that exactly, compare the incoming digit against the maximum's own last digit.
5. Mirror both checks against the minimum for negative accumulators.
6. Only once both checks pass, multiply by 10 and add the digit.
...
- 15: By this line the multiplication is guaranteed to fit, so no undefined behaviour can occur.

<!-- @code java -->
```java
static int reverseSafe(int x) {
    int rev = 0;
    while (x != 0) {
        int digit = x % 10;
        x /= 10;
...
<!-- @code python -->
```python
# Python integers are unbounded, so arithmetic overflow cannot occur.
...
<!-- @why -->
Shows the two roles clearly: multiplying by ten opens a slot, and the modulo supplies what goes in it.

<!-- @walkthrough -->
1. The accumulator starts at 0 and N is 12345.
2. Read the last digit with 12345 % 10, giving 5. The accumulator becomes 0 times 10 plus 5, which is 5, and N becomes 1234.
3. Read 4. The accumulator shifts left to 50 and gains the 4, giving 54, and N becomes 123.
4. Read 3. The accumulator becomes 543 and N becomes 12.
5. Read 2. The accumulator becomes 5432 and N becomes 1.
6. Read 1. The accumulator becomes 54321 and N becomes 0, since 1 divided by 10 truncates to zero.
7. The condition fails and the accumulator holds 54321.
...
<!-- @why -->
Looks like a bug the first time it appears. Stating it explicitly prevents a student from adding a broken workaround.

<!-- @walkthrough -->
1. Read 0. The accumulator becomes 0 times 10 plus 0, which is still 0, and N becomes 120.
2. Read 0 again. The accumulator is still 0, and N becomes 12.
3. Read 2. The accumulator becomes 2 and N becomes 1.
4. Read 1. The accumulator becomes 21 and N becomes 0.
5. The result is 21 rather than 0021, because leading zeros are not part of a number's value.
6. The two zeros were never lost — they simply have no representation in an integer once nothing follows them.

<!-- @example -->

<!-- @input -->
N = 1534236469 with a 32-bit int accumulator

<!-- @output -->
1056389759 — silently wrong. The true reverse is 9646324351.

<!-- @why -->
The overflow lesson from data-types arriving in a real algorithm, with a reachable input rather than a contrived one.

<!-- @walkthrough -->
1. The input is a perfectly valid int, well under the maximum of 2147483647.
2. The loop proceeds normally until the accumulator reaches 964632435 with one digit still to add.
3. The next step needs 964632435 times 10 plus 1, which is 9646324351.
4. That value needs more than 31 bits, so the high bits are discarded and what remains is reinterpreted.
5. The function returns 1056389759, which was measured with g++ on this machine.
6. No error or warning is produced at any point, so the wrong answer flows onward silently.

<!-- @example -->

<!-- @input -->
N = -123 in Python, using the same loop that works in C++ and Java

<!-- @output -->
An infinite loop — the value gets stuck at -1 forever

<!-- @why -->
Traced on this machine. The clearest possible payoff of the negative-division divergence taught in Arithmetic Operators — the same algorithm is correct in two languages and non-terminating in the third.

<!-- @walkthrough -->
1. Reading -123 % 10 in Python gives 7, not -3, because the remainder takes the divisor's sign.
2. Dividing -123 // 10 gives -13, because flooring rounds toward negative infinity rather than toward zero.
3. The next steps give -2, then -1, with the accumulator collecting wrong digits along the way.
4. At -1 the division gives -1 // 10, which floors -0.1 down to -1 rather than truncating it to 0.
5. So N stays at -1 on every subsequent iteration and the condition testing against zero never fails.
6. The identical code in C++ and Java terminates correctly, because both truncate toward zero and give the remainder the dividend's sign.
...
<!-- @doubt -->
### Why do we multiply the accumulator by 10?
...
<!-- @doubt -->
### Why does 1200 reverse to 21 instead of 0021?
...
<!-- @doubt -->
### Why can reversing a valid number overflow?
...
<!-- @doubt -->
### Why can't I just check whether the result looks wrong after multiplying?
...
<!-- @doubt -->
### Why does the overflow guard compare the digit against 7?
...
<!-- @doubt -->
### Why does my Python version hang on negative numbers?
...
<!-- @doubt -->
### Do negatives need special handling in C++ and Java?
