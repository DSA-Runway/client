...
/**
 * Subtopic 19 of Basics (Medium #3). The mutation half of the function
 * interface, deliberately separated from subtopic 18's "how values get in".
 *
 * This is the most misunderstood topic in the module, and the confusion is
 * almost always the same one: people see a list change inside a function and
 * conclude the language passes by reference. It does not. Java and Python both
 * pass a COPY OF THE REFERENCE — which makes mutation visible and reassignment
 * invisible, and that asymmetry is the entire tell.
 *
 * The decisive test is one line: can you write swap(a, b) for two integers?
 * C++ can. Java and Python cannot, at all, by any means. That single question
 * settles the argument faster than any amount of prose.
 *
 * This is also the third lesson converging on one model — variables-and-constants
 * introduced boxes-versus-labels, for-each-loop showed mutating-versus-rebinding,
 * and this closes it. Those links are made explicit rather than left implicit.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Passing by Value",
      idea: "The function gets a copy, so writing to the parameter cannot affect the caller.",
      steps: [
        "The argument's value is copied into the parameter when the call is made.",
        "The parameter is a separate variable that exists only for the duration of the call.",
        "Writing to it modifies the copy alone.",
        "When the function returns, the copy is discarded.",
        "The caller's variable holds exactly what it held before the call.",
      ],
      code: [
        {
          language: "cpp",
...
int main() {
    ...
    public static void main(String[] args) {
        ...
        {
          language: "python",
          code: `def increment(x):        # x is a name bound to the same integer
...
print(p, q)              # 2 1`,
    ...
    {
      name: "Passing by Reference in C++",
      idea: "Make the parameter an alias for the caller's variable, so the function can modify it.",
      steps: [
        "Add an ampersand to the parameter type to declare it as a reference.",
        "No copy is made — the parameter becomes another name for the caller's variable.",
        "Writing to the parameter writes directly to the caller's variable.",
        "Add const when the goal is only to avoid a copy, not to allow modification.",
        "Choose by value for small types, const reference for large ones, and plain reference only when modification is intended.",
      ],
      code: [
        {
          language: "cpp",
...
int main() {
    ...
    return 0;
        ...
        {
          language: "java",
          code: `// Java has NO reference parameters. These are the substitutes.
...
public static void main(String[] args) {
    ...
    System.out.println(arr[0] + " " + arr[1]);   // 2 1
        ...
        {
          language: "python",
          code: `# Python has no reference parameters either. Same substitutes.
...
print(arr)            # [2, 1]`,
    ...
    {
      name: "What Java and Python Actually Do",
      idea: "Recognise that a copied reference still points at the same object, which makes mutation visible and reassignment invisible.",
      steps: [
        "Identify what value the variable actually holds — a number, or a reference to an object.",
        "The call copies that value into the parameter, whichever it is.",
        "If it was a reference, both the caller and the parameter now point at the same object.",
        "Mutating that object through the parameter reaches the shared object, so the caller sees the change.",
        "Reassigning the parameter points only the copy elsewhere, so the caller sees nothing.",
        "Use that asymmetry as the test: true pass by reference would make both visible.",
      ],
      code: [
        {
          language: "python",
          code: `# MUTATION reaches the caller — one shared object
def add_item(lst):
    lst.append(99)
...
int main() {
  ...
  examples: [
    {
      input: "A function that tries to swap two integer variables belonging to the caller",
      output: "Works in C++ with reference parameters. Impossible in Java and Python.",
      walkthrough: [
        "In C++, declaring the parameters with an ampersand makes them aliases for the caller's variables.",
        "The three-step swap writes directly to those variables, so the caller's values are exchanged.",
        "In Java, both parameters are copies of the caller's numbers, so the swap happens inside the function and is discarded.",
        "No alternative signature exists, because Java has no reference parameters anywhere in the language.",
        "In Python, the parameters are names bound to the same integer objects, and integers are immutable.",
        "Rebinding those names inside the function cannot affect the caller's names, so the swap is equally impossible.",
      ],
      why: "The decisive test. Whether a language can express this settles whether it has pass by reference, without any argument about terminology.",
    },
    {
      input: "def add_item(lst): lst.append(99)  called with arr = [1, 2]",
      output: "arr becomes [1, 2, 99] — the caller's list did change",
      walkthrough: [
        "The variable arr holds a reference to a list object, not the list itself.",
        "The call copies that reference into the parameter, so two references now exist.",
        "Both references point at the same single list object in memory.",
        "Calling append travels along the parameter's reference and modifies that shared object.",
        "There is only one list, so the caller sees the change through its own reference.",
        "This is not pass by reference — it is a copied reference reaching a shared object.",
      ],
      why: "The observation that convinces people a language passes by reference, shown with the mechanism that actually explains it.",
    },
    {
      input: "def replace(lst): lst = [9, 9]  called with arr = [1, 2]",
      output: "arr is still [1, 2] — nothing changed",
      walkthrough: [
        "The call copies the reference into the parameter, exactly as before.",
        "Assigning a new list to the parameter creates a new object and points the parameter at it.",
        "Only the parameter's copy of the reference moved.",
        "The caller's reference still points at the original list, which was never touched.",
        "When the function returns, the parameter and the new list are discarded.",
        "Under true pass by reference the caller's variable would have been repointed too.",
      ],
      why: "Placed beside the previous example it isolates the asymmetry, which is the actual proof that neither language passes by reference.",
    },
    {
      input: "Passing a vector of 10,000 strings to a C++ function by value versus by const reference",
      output: "By value copies every string; by const reference copies nothing",
      walkthrough: [
        "By value, the parameter is a brand new vector constructed from the argument.",
        "Constructing it copies all 10,000 elements, and each string copies its own characters.",
        "That work happens on every call, even when the function only reads the data.",
        "By const reference, the parameter is an alias and no copying occurs at all.",
        "The const qualifier makes any attempt to modify it a compile error, so the caller's data is still safe.",
        "The result is the performance of a reference with the safety guarantee of a value.",
      ],
      why: "The practical reason const reference dominates real C++ code, and the one place the value-versus-reference choice is about speed rather than semantics.",
    },
  ],
  ...
  commonDoubts: [
    ...
    {
      question: "If Java is pass by value, why did my list change inside the function?",
      answer:
    ...
    {
      question: "Can I write a swap function in Java or Python?",
      answer:
    ...
    {
      question: "What is the difference between mutating and reassigning a parameter?",
      answer:
    ...
    {
      question: "When should I use const reference in C++?",
      answer:
    ...
    {
      question: "Why can't I change an integer through a parameter in Python?",
      answer:
    ...
    {
      question: "Should I write functions that modify their arguments?",
      answer:
    ...
    {
      question: "What is call by sharing?",
      answer:
