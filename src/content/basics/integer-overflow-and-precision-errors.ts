...
/**
 * Subtopic 30 of Basics (Hard #3) — the final container, and the capstone.
 *
 * Four earlier subtopics point forward to this one, and by now the module has
 * produced a genuine catalogue of MEASURED failures. This container's job is to
 * turn those scattered incidents into one framework, so the closing section is a
 * synthesis rather than new material.
 *
 * MEASURED EARLIER IN THIS MODULE (all reproduced with g++ / python3 here):
 * - data-types:        100000 * 100000 -> -1794967296
 * - count-digits:      log10(999999999999999) returns exactly 15.0, so the
 *                      formula reports 16 digits instead of 15
 * - reverse-a-number:  1534236469 reverses to 9646324351; a naive int gives
 *                      1056389759
 * - lcm:               50000 * 60000 overflows to -129496; divide-first gives
 *                      the correct 300000
 * - prime-check:       46341*46341 wraps to -2147479015, so "i*i <= n" is true
 *                      forever and the loop never terminates
 * - palindrome-number: a full-range scan found the naive form NOT observably
 *                      wrong, which is the nuance that keeps the module honest
 *
 * MEASURED FOR THIS CONTAINER:
 ...
 *   (below 0.3, while the sum lands above it — which is exactly why they differ)
 * - 0.1 added ten times gives 0.9999999999999999, not 1.0
 * - 123456789012345 through a 32-bit float loses 909,177; through a double, zero
 * - float(2^53 + 1) == float(2^53) is True — doubles stop counting consecutive
 *   integers at 9,007,199,254,740,992
 * - int 16777217 through a float returns 16777216 (24-bit mantissa)
 * - -INT_MIN and abs(INT_MIN) are both still INT_MIN
 * - INT_MAX + 1 wraps to INT_MIN, and INT_MIN - 1 wraps to INT_MAX
 * - INT_MAX - a as a pre-check cannot itself overflow
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Integer Overflow: Detect Before, Not After",
      idea: "Rearrange the check so the test itself cannot overflow, or widen the type before the operation runs.",
      steps: [
        "Determine the largest value the computation can reach, considering intermediate results and not only the final answer.",
        "Compare that against the type's maximum of roughly 2.1 billion for 32 bits or 9.2 quintillion for 64 bits.",
        "If a wider type is available and the answer fits, widen an operand before the operation rather than casting the result.",
        "If the type is fixed, rewrite the check by dividing or subtracting from the limit, so the test itself cannot overflow.",
        "Guard the most-negative value separately wherever an absolute value or negation is taken.",
        "Decide what to do on detection: return a sentinel, throw, or use a wider type.",
      ],
      code: [
        {
          language: "cpp",
...
int main() {
    ...
    public static void main(String[] args) {
        ...
        {
          language: "python",
          code: `# Python integers are UNBOUNDED — arithmetic overflow does not exist.
...
# integers were holding perfectly.`,
    ...
    {
      name: "Floating-Point Representation and Why Equality Fails",
      idea: "Understand that a decimal fraction is stored as the nearest representable binary value, so two different roundings rarely land on the same result.",
      steps: [
        "Recognise that binary fractions represent only sums of one half, one quarter, one eighth and so on.",
        "Note that one tenth is not such a sum, so it cannot be stored exactly at any precision.",
        "The stored value is therefore the nearest representable neighbour, which may be slightly above or below.",
        "Arithmetic on two approximations produces a result that is itself approximate.",
        "That result need not equal the approximation of the mathematically correct answer.",
        "Conclude that testing floating-point equality asks whether two independent roundings coincided, which is not the question you meant.",
      ],
      code: [
        {
          language: "python",
...
# integer is the usual answer.`,
          annotations: {
            ...
            30: "Decimal takes strings rather than floats, because passing 0.1 would already have lost the precision.",
          },
        },
        {
          language: "cpp",
...
int main() {
    ...
    return 0;
          ...
          annotations: {
            ...
            22: "The single most useful sentence about widening conversions, and it is easy to get backwards.",
            26: "The root cause of the Count Digits log10 failure, stated as a general limit.",
          },
        },
        {
          language: "java",
          code: `public class Precision {
    public static void main(String[] args) {
    ...
    {
      name: "Working Safely with Floating Point",
      idea: "Replace equality with a tolerance, avoid accumulating error, and never trust a float-derived integer at a boundary.",
      steps: [
        "Replace any equality test between floating-point values with a comparison of their difference against a small tolerance.",
        "Choose the tolerance for the magnitudes involved, using a relative tolerance when the values vary widely.",
        "Avoid summing many floating-point values where an integer accumulation would work instead.",
        "Where a decimal quantity must be exact, store it as an integer in a smaller unit.",
        "Never derive an integer from a floating-point computation at a boundary, since the rounding may cross it.",
        "Prefer double over float in every case where memory is not the binding constraint.",
      ],
      code: [
        {
          language: "cpp",
...
# assume any integer derived from it is exact at a boundary.`,
          annotations: {
            6: "math.isclose is the right default in Python — relative by design, with an absolute option for comparisons against zero.",
  ...
  examples: [
    {
      input: "The five overflow failures measured across this module",
      output: "Every one silent — a wrong number, and in one case a program that never terminates",
      walkthrough: [
        "Data Types: 100000 times 100000 is 10 billion, which exceeded the int range and returned negative 1,794,967,296.",
        "Reverse a Number: 1534236469 is a valid int whose reverse is 9,646,324,351, and a naive int accumulator returned 1,056,389,759.",
        "LCM: 50000 times 60000 is 3 billion, which overflowed and produced negative 129,496 for an answer that was actually 300,000 and fitted easily.",
        "Prime Check: at candidate 46341 the expression i times i wrapped to negative 2,147,479,015, and since a negative value is always below n the loop never terminated.",
        "Count Digits: log10 of fifteen nines returned exactly 15.0 rather than a value just below it, so the formula reported sixteen digits instead of fifteen.",
        "In none of these cases did the program stop, warn, or indicate that anything had gone wrong.",
      ],
      why: "Seeing all five together is what makes the pattern visible. Each looked like an isolated quirk in its own subtopic, and together they are one property of fixed-width arithmetic.",
    },
    {
      input: "What 0.1, 0.2 and 0.3 are actually stored as",
      output: "Three values that are all slightly wrong, and wrong in different directions",
      walkthrough: [
        "0.1 is stored as 0.1000000000000000055511151231257827021181583404541015625, slightly above one tenth.",
        "0.2 is stored as 0.200000000000000011102230246251565404236316680908203125, also slightly above.",
        "0.3 is stored as 0.299999999999999988897769753748434595763683319091796875, which is slightly BELOW three tenths.",
        "Adding the first two produces a value slightly above three tenths, because both inputs rounded upward.",
        "The stored 0.3 rounded downward, so the sum and the constant are genuinely different numbers.",
        "The equality test therefore returns false, and the difference is 5.551115123125783 times ten to the minus seventeen.",
      ],
      why: "The usual explanation stops at floating point is imprecise. Seeing the exact stored values, and that the roundings went in opposite directions, explains why this specific comparison fails rather than merely that it might.",
    },
    {
      input: "Converting 123456789012345 through a float and through a double",
      output: "The float loses 909,177. The double loses nothing.",
      walkthrough: [
        "A 32-bit float has a 24-bit mantissa, so it represents integers exactly only up to 2 to the 24th, which is 16,777,216.",
        "The value 123456789012345 is far beyond that, so it is rounded to the nearest representable float.",
        "Converting back gives 123456788103168, an error of 909,177.",
        "A 64-bit double has a 53-bit mantissa, exact up to 2 to the 53rd, and this value is below that.",
        "Converting through a double returns the original exactly.",
        "Crucially, long long to float is a widening conversion that requires no cast at all — the compiler treats it as safe.",
      ],
      why: "Widening protects range and not precision. A conversion the language considers safe enough to perform silently can still discard nearly a million, which is the strongest argument for preferring double over float.",
    },
    {
      input: "Whether a double can distinguish 2⁵³ from 2⁵³ + 1",
      output: "No — they compare equal",
      walkthrough: [
        "A double has 53 bits of mantissa, so it can represent every integer exactly up to 2 to the 53rd.",
        "That value is 9,007,199,254,740,992.",
        "Adding one requires a 54th bit of precision, which does not exist.",
        "The result rounds back to 2 to the 53rd, so the two values become the same double.",
        "Measured: the comparison returns true, meaning the two are genuinely indistinguishable.",
        "This is the underlying reason the Count Digits logarithm failed — at that magnitude the gaps between representable doubles are wide enough to swallow the difference between a value and the integer above it.",
      ],
      why: "It connects the abstract mantissa width to a failure the student already saw, turning the log10 result from a curiosity into an instance of a general limit.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why doesn't the program crash when an integer overflows?",
      answer:
    ...
    {
      question: "Why can't I check for overflow after doing the arithmetic?",
      answer:
    ...
    {
      question: "Why is abs of the most negative integer still negative?",
      answer:
    ...
    {
      question: "How should I compare floating-point values?",
      answer:
    ...
    {
      question: "Should I use float or double?",
      answer:
    ...
    {
      question: "Why do the log10 and square-root failures in this module happen?",
      answer:
    ...
    {
      question: "Does any of this apply to Python?",
      answer:
