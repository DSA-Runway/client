...
/**
 * Subtopic 14 of Basics. Completes the loop group.
 *
 * The lesson worth the most weight: THE LOOP VARIABLE IS A COPY. Both GFG pages
 * state this outright — Java's says "the loop variable holds a copy of the value,
 * not the actual array element" — and it is the source of the one bug everybody
 * writes, which is attempting to modify a collection through a for-each and
 * finding nothing changed.
 *
 * C++ gives you an opt-out via auto&. Java does not for primitives. Python's
 * version is a third mechanism entirely: the name is rebound to the object, so
 * rebinding does nothing but MUTATING the object does — which connects straight
 * back to the boxes-versus-labels model from variables-and-constants.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Iterating Values Directly",
      idea: "Visit every element in order when you need the values and not their positions.",
      steps: [
        "Confirm you need the elements themselves rather than their indices.",
        "Declare a loop variable of the element's type.",
        "Name the collection to iterate.",
        "Each iteration assigns the next element to the loop variable.",
        "Use that variable inside the body; the loop stops automatically after the last element.",
      ],
      code: [
        {
          language: "cpp",
...
int main() {
    ...
    return 0;
        ...
        {
          language: "java",
          code: `public class ForEachBasics {
...
print()`,
    ...
    {
      name: "Modifying Elements: Copy vs Reference",
      idea: "Understand why writing to the loop variable usually changes nothing, and what each language offers instead.",
      steps: [
        "Recognise that the loop variable is assigned a copy of each element by default.",
        "Writing to that copy changes the copy and leaves the collection untouched.",
        "In C++, declare the loop variable as a reference to write through to the element.",
        "In Java, use an indexed loop, since primitives have no reference form.",
        "In Python, distinguish rebinding the name, which does nothing, from calling a method that mutates the object, which does.",
      ],
      code: [
        {
          language: "cpp",
...
print(matrix)              # unchanged by that loop`,
    ...
    {
      name: "When You Need the Index or the Position",
      idea: "Fall back to an indexed loop, or ask for the index explicitly, when the position matters.",
      steps: [
        "Decide whether the body needs the element's position or only its value.",
        "If the position is needed, use an indexed loop in C++ or Java.",
        "In Python, use enumerate to receive the index and the value together.",
        "For backwards iteration or a partial range, use an indexed loop in every language.",
        "To walk two collections in step, index both or use a pairing helper.",
      ],
      code: [
        {
          language: "cpp",
...
print()`,
  ...
  examples: [
    {
      input: "arr = [3, 7, 1, 9]; for (int value : arr) sum += value;",
      output: "20",
      walkthrough: [
        "The loop takes the first element, 3, and assigns it to value.",
        "The body adds 3 to sum, making it 3.",
        "The next element, 7, is assigned to value and added, making sum 10.",
        "The next element, 1, brings sum to 11, and the last element, 9, brings it to 20.",
        "There are no more elements, so the loop ends without any bound being checked by the programmer.",
        "No index was ever created, so no off-by-one was possible.",
      ],
      why: "The read-only case, which is what for-each is for and where it is strictly better than an indexed loop.",
    },
    {
      input: "arr = [3, 7, 1, 9]; for (int value : arr) value *= 2;",
      output: "arr is still [3, 7, 1, 9] — nothing changed",
      walkthrough: [
        "The first element, 3, is copied into the loop variable value.",
        "The body doubles value to 6, which modifies the copy and not the array slot it came from.",
        "The iteration ends and that copy is discarded.",
        "The next element is copied into value, doubled, and discarded in exactly the same way.",
        "After all four iterations the array holds the values it started with.",
        "No error is reported at any point — the code did precisely what it said, which was to modify a copy.",
      ],
      why: "The universal for-each bug. It fails silently in all three languages, which is why the copy semantics have to be understood rather than memorised.",
    },
    {
      input: "The same doubling written with C++'s reference form: for (int& value : arr)",
      output: "arr becomes [6, 14, 2, 18]",
      walkthrough: [
        "The ampersand makes value a reference, meaning it is an alias for the element rather than a copy of it.",
        "Assigning to value writes through that alias directly into the array slot.",
        "The first element becomes 6, the second 14, the third 2 and the fourth 18.",
        "Nothing is copied and nothing is discarded at the end of each iteration.",
        "Java has no equivalent for primitives, so the same fix there requires an indexed loop.",
        "Python has no equivalent either, since assignment rebinds a name rather than writing to a location.",
      ],
      why: "Shows the fix is a genuine language feature in C++ and simply unavailable in the other two, which decides how the same task is written in each.",
    },
    {
      input: "matrix = [[1, 2], [3, 4]]; for row in matrix: row.append(0)",
      output: "matrix becomes [[1, 2, 0], [3, 4, 0]] — this one does change",
      walkthrough: [
        "The loop binds the name row to the first inner list object.",
        "Calling append is a method call on that object, which reaches the real list rather than a copy.",
        "The inner list gains a 0, and the change is visible through matrix because there is only one list.",
        "The same happens for the second inner list.",
        "Writing row = [9, 9] instead would rebind the local name and leave matrix untouched.",
        "So mutating works and reassigning does not — the difference is whether you reach through the label or move it.",
      ],
      why: "The exception that makes the rule make sense, and it is the boxes-versus-labels model from the variables lesson applied to iteration.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "I changed the loop variable but the array is unchanged. Why?",
      answer:
    ...
    {
      question: "When should I use a for-each instead of an indexed loop?",
      answer:
    ...
    {
      question: "What does the ampersand do in C++'s range-based for?",
      answer:
    ...
    {
      question: "Why does row.append() change my Python list when row = [] doesn't?",
      answer:
    ...
    {
      question: "Can I get the index in a for-each loop?",
      answer:
    ...
    {
      question: "Why can't I remove items from a collection while looping over it?",
      answer:
    ...
    {
      question: "Can a for-each iterate backwards?",
      answer:
    ...
    {
      question: "Is Python's for loop a for-each?",
      answer:
  ...
  relatedIds: ["for-loop", "while-loop", "variables-and-constants", "break-and-continue"],
};
