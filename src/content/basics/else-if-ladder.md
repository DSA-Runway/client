...
<!-- @approach -->
### Building an else-if Ladder

<!-- @idea -->
Chain conditions so exactly one branch runs, with an optional catch-all at the end.

<!-- @steps -->
1. Write the first condition and the block that should run when it holds.
2. Attach each further condition with else if, so it is only tested when all previous ones failed.
3. Order the conditions so the most specific is tested first.
4. Add a final else for the case where nothing matched, if every input should produce a result.
5. At runtime, evaluation stops at the first true condition and skips every remaining test.
...
<!-- @approach -->
### Ordering Overlapping Conditions

<!-- @idea -->
When conditions overlap, put the most specific first — otherwise the broad one swallows everything.

<!-- @steps -->
1. Identify whether the conditions overlap, meaning one input could satisfy more than one of them.
2. If they do, determine which condition is the most restrictive.
3. Place that condition first, and order the rest from most to least specific.
4. With >= thresholds this means descending order; with <= it means ascending.
5. Verify by tracing an input that should hit the last rung and confirming no earlier condition catches it.
...
- 12: The program compiles and runs. Only the answer is wrong.
...
<!-- @approach -->
### The Default Branch and Uncovered Gaps

<!-- @idea -->
Decide explicitly what happens when nothing matches, instead of letting it fall through silently.

<!-- @steps -->
1. List every input the ladder is expected to handle.
2. Check whether the conditions together cover all of them.
3. If every input should produce a result, add a final else as the catch-all.
4. If falling through is genuinely correct, leave the else off deliberately rather than by omission.
5. When the ladder assigns a variable, confirm that variable holds something sensible on the fall-through path.
...
<!-- @input -->
score = 85 in a descending grade ladder: >=90 A, >=80 B, >=70 C, else F

<!-- @output -->
B

<!-- @why -->
Shows first-match-wins concretely: the third condition would also have been true, and was never even tested.

<!-- @walkthrough -->
1. The first condition score >= 90 is evaluated: 85 is not at least 90, so it is false.
2. Evaluation moves to the second rung and tests score >= 80.
3. 85 is at least 80, so this condition is true and its block runs, assigning B.
4. The remaining conditions are skipped entirely — score >= 70 is never evaluated at all.
5. The final else is also skipped, and execution continues after the whole ladder.
6. Exactly two comparisons were performed to reach the answer.

<!-- @example -->

<!-- @input -->
score = 95 in an ascending ladder: >=70 C, >=80 B, >=90 A

<!-- @output -->
C — and the A and B branches are unreachable for every input

<!-- @why -->
The order bug in full: it is not an edge case that fails, it is every case, and there is no error message pointing at it.

<!-- @walkthrough -->
1. The first condition score >= 70 is evaluated: 95 is at least 70, so it is true.
2. Grade is assigned C, and the ladder stops — no further condition is tested.
3. The intended answer was A, but the branch that would have produced it was never reached.
4. The problem is not this one input. Any score of 80 or more satisfies score >= 70 first.
5. So the B branch runs only for scores that failed score >= 70 yet pass score >= 80, which is impossible.
6. Both later branches are dead code, and the compiler reports nothing because each condition is individually valid.

<!-- @example -->

<!-- @input -->
score = 45 in a ladder with no final else

<!-- @output -->
grade keeps whatever value it already held

<!-- @why -->
Falling through is silent by design. Making it visible is the point of writing the final else deliberately.

<!-- @walkthrough -->
1. The condition score >= 90 is false for 45.
2. The condition score >= 80 is false.
3. The condition score >= 70 is false.
4. There is no else, so the ladder ends without running any block.
5. Grade is never assigned by the ladder and still holds its previous value.
6. In C++ an uninitialised grade would show leftover memory, and in Python an unassigned name would raise NameError instead.
...
<!-- @doubt -->
### My grade ladder gives everyone the same grade. What is wrong?
...
<!-- @doubt -->
### Do I need to write both bounds, like score >= 80 && score < 90?
...
<!-- @doubt -->
### What is the difference between else-if and several separate ifs?
...
<!-- @doubt -->
### How many branches before I should switch to a switch statement?
...
<!-- @doubt -->
### Can I have an else without any else-if in between?
