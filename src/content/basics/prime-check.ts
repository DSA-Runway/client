...
/**
 * Subtopic 27 of Basics (Medium #11) — the last Medium one. Completes the
 * number-theory trio: GCD -> LCM -> Prime Check. Unlike the first two this one
 * stands alone, and its lesson is the square-root bound.
 *
 * SOURCES
 * - GeeksforGeeks, "Introduction to Primality Test and School Method" — the
 *   naive O(n) scan and the square-root bound, with the reasoning that the
 *   smallest factor above one cannot exceed sqrt(n).
 * - GeeksforGeeks, "Check for Prime Number" — the 6k +/- 1 refinement, described
 *   there as three times faster than testing every value up to sqrt(n).
 * - GeeksforGeeks, "Sieve of Eratosthenes" — O(n log log n) precompute for
 *   answering many queries.
 *
 * MEASURED ON THIS MACHINE (g++ -O2), operations instrumented:
 *
 * 1. isPrime(100003): naive takes 100,001 modulo operations, the sqrt bound
 *    takes 315. A 317x reduction on a six-digit input.
 *
 * 2. isPrime(1000000007): sqrt bound 31,621 operations, 6k +/- 1 10,540. Ratio
 *    3.0, which confirms GFG's "three times faster" claim exactly.
 *
 * 3. THE i*i OVERFLOW IS REAL. sqrt(INT_MAX) is 46340.95, so the loop reaches
 *    i = 46341, and 46341*46341 is 2,147,488,281 — past INT_MAX. As an int it
 *    wraps to -2,147,479,015, which is NEGATIVE, so "i*i <= n" stays true
 *    forever and the loop never terminates. Verified numerically.
 *
 * 4. The 6k +/- 1 version agrees with the sqrt version on 0..199,999 with zero
 *    mismatches, plus spot checks including 2147483647 — which is itself prime,
 *    being the Mersenne prime 2^31 - 1.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Trial Division to n-1",
      idea: "Test every integer from 2 up to n-1 as a possible divisor.",
      steps: [
        "Return false immediately for any value below 2, since primes are defined as greater than 1.",
        "Start a candidate divisor at 2.",
        "Test whether the candidate divides n exactly using modulo.",
        "If it does, n has a divisor other than 1 and itself, so return false.",
        "Otherwise advance the candidate and repeat while it stays below n.",
        "If no candidate divided evenly, n is prime.",
      ],
      code: [
        {
          language: "cpp",
...
int main() {
    ...
    return 0;
        ...
        {
          language: "java",
          code: `static boolean isPrimeNaive(int n) {
    if (n < 2) return false;
    for (int i = 2; i < n; i++) {
        if (n % i == 0) return false;
    }
...
// work and is still O(n). The square root is the real bound.`,
          annotations: {
            13: "Worth naming, because n/2 feels like an optimisation and does not change the complexity class at all.",
          },
        },
        {
          language: "python",
...
# a billion modulo operations to confirm one number is prime.`,
    ...
    {
      name: "Square-Root Bound",
      idea: "Stop at the square root, since divisors come in pairs and one member of every pair sits at or below it.",
      steps: [
        "Return false for any value below 2.",
        "Start a candidate divisor at 2.",
        "Continue while the candidate squared does not exceed n.",
        "Test whether the candidate divides n exactly.",
        "If it does, return false, since a divisor other than 1 and n has been found.",
        "If the loop completes without finding one, no divisor at or below the square root exists, so none exists at all and n is prime.",
      ],
      code: [
        {
          language: "cpp",
...
print(is_prime_range(1000000007))   # True`,
    ...
    {
      name: "6k ± 1 Refinement",
      idea: "Handle 2 and 3 directly, then test only candidates of the form 6k − 1 and 6k + 1, since no other form can be prime.",
      steps: [
        "Return false for any value below 2, and true for 2 and 3 directly.",
        "Return false if the number is divisible by 2 or by 3.",
        "Start the candidate at 5, which is the first number of the form 6k − 1.",
        "At each stop test both the candidate and the candidate plus 2, covering 6k − 1 and 6k + 1.",
        "Advance the candidate by 6 and continue while its square does not exceed n.",
        "If no candidate divided evenly, n is prime.",
      ],
      code: [
        {
          language: "cpp",
          code: `bool isPrime6k(int n) {
    if (n < 2)  return false;
...
// from 0 to 199,999, with zero mismatches.`,
        ...
        {
          language: "java",
          code: `static boolean isPrime6k(int n) {
    if (n < 2)  return false;
    if (n <= 3) return true;
...
//   leaving only    6k+1 and 6k+5, and 6k+5 is 6(k+1)-1`,
          annotations: {
            5: "The i += 6 step is the whole optimisation, and it is only sound because of the guard two lines above.",
          },
        },
        {
          language: "python",
          code: `def is_prime_6k(n):
    if n < 2:
        return False
    if n <= 3:
        return True
...
# which matters most in Python where each operation is comparatively slow.`,
    ...
    },
    {
      ...
      idea: "Precompute primality for every number up to a limit by marking multiples, then answer each query instantly.",
      steps: [
        "Create a boolean array covering 0 to the limit, initially assuming everything is prime.",
        "Mark 0 and 1 as not prime.",
        "For each candidate whose square does not exceed the limit, check whether it is still marked prime.",
        "If it is, it is prime, so mark all its multiples as composite.",
        "Begin marking at the candidate squared, since every smaller multiple already has a smaller prime factor and was marked earlier.",
        "After building, any query is a single array lookup.",
      ],
      code: [
        {
          language: "cpp",
...
int main() {
    ...
    return 0;
          ...
          annotations: {
            ...
            25: "The known count of primes below one million, useful as a correctness check on any sieve implementation.",
          },
        },
        {
          language: "java",
          code: `static boolean[] buildSieve(int limit) {
...
# because the marking happens inside the interpreter's C code.`,
  ...
  examples: [
    {
      input: "n = 100003, naive scan versus the square-root bound",
      output: "Both return prime. Naive takes 100,001 operations, the square-root bound takes 315.",
      walkthrough: [
        "The naive loop tests every candidate from 2 up to 100002, performing one modulo each.",
        "Since the number is prime, none divides evenly and there is no early exit, so all 100,001 tests run.",
        "The square-root version stops once the candidate squared exceeds the number.",
        "The square root of 100003 is about 316.2, so the loop tests only up to 316.",
        "That is 315 operations, and the conclusion is identical.",
        "Everything beyond 316 could only have found the larger partner of a pair whose smaller member was already tested.",
      ],
      why: "The measured 317-fold reduction comes from changing one loop bound, which makes the square-root argument concrete rather than theoretical.",
    },
    {
      input: "n = 36, showing why the bound is exactly the square root",
      output: "The divisor pairs mirror around 6, which is √36",
      walkthrough: [
        "The divisors of 36 are 1, 2, 3, 4, 6, 9, 12, 18 and 36.",
        "They pair up as 1 with 36, 2 with 18, 3 with 12, 4 with 9, and 6 with itself.",
        "In every pair, one member is at most 6 and the other is at least 6.",
        "6 is the square root of 36, and it is the exact point where the pairing turns around.",
        "So testing 2, 3, 4, 5 and 6 covers the smaller member of every possible pair.",
        "Testing 9, 12, 18 or 36 could only rediscover partners already found, which is why the loop can stop.",
      ],
      why: "A perfect square makes the mirroring visible, since the middle pair is a single value sitting exactly at the bound.",
    },
    {
      input: "isPrime(2147483647) written with a plain i * i <= n condition",
      output: "Infinite loop — the program hangs",
      walkthrough: [
        "The square root of 2147483647 is about 46340.95, so the loop must reach 46341 before its condition fails.",
        "At that point it computes 46341 multiplied by 46341, which is 2,147,488,281.",
        "That value exceeds the maximum int of 2,147,483,647, so it wraps.",
        "The wrapped result is -2,147,479,015, which is negative.",
        "A negative value is always less than or equal to n, so the condition remains true.",
        "The candidate keeps increasing, the product keeps wrapping, and the loop never terminates.",
        "Casting to a wider type before the multiplication, or computing the square root once beforehand, fixes it.",
      ],
      why: "Verified numerically. The input is genuinely prime, so there is no early exit to hide the bug, and it is exactly the value a test suite would use to probe the boundary.",
    },
    {
      input: "isPrime(1000000007), square-root bound versus 6k ± 1",
      output: "Both return prime. 31,621 operations against 10,540 — a ratio of 3.0.",
      walkthrough: [
        "The square root of 1000000007 is about 31,623, so the plain loop tests roughly that many candidates.",
        "The 6k form first eliminates every even number and every multiple of 3, which is two thirds of all integers.",
        "It then advances in steps of 6, testing two candidates at each stop rather than six.",
        "That gives roughly a third as many modulo operations over the same range.",
        "Measured: 31,621 operations against 10,540, a ratio of exactly 3.0.",
        "Both are O of the square root of n — only the constant factor changed.",
      ],
      why: "Confirms the standard three-times-faster claim by measurement, and reinforces that a constant-factor gain is real without being a complexity improvement.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "Why can we stop at the square root?",
      answer:
    ...
    {
      question: "Why write i * i <= n instead of i <= sqrt(n)?",
      answer:
    ...
    {
      question: "Why does my prime check hang on large numbers?",
      answer:
    ...
    {
      question: "Why does 6k ± 1 work, and how much does it help?",
      answer:
    ...
    {
      question: "When should I use a sieve instead?",
      answer:
    ...
    {
      question: "Why does the sieve start marking at i squared?",
      answer:
    ...
    {
      question: "Is stopping at n / 2 a useful optimisation?",
      answer:
