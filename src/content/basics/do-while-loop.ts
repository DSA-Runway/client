...
/**
 * Subtopic 13 of Basics. Completes the loop trio.
 *
 * One idea defines it: the condition moves from before the body to after it, so
 * the body is guaranteed to run at least once. Everything else follows —
 * including why the canonical use is input validation, where the value the
 * condition tests does not exist until the first iteration has produced it.
 *
 * Second language gap after switch-case: Python has no do-while. PEP 315
 * proposed one and was rejected, so the honest answer is the while True plus
 * break idiom rather than a translation.
 *
 * Also worth being straight with students: do-while is genuinely the least-used
 * of the three loops, and almost absent from DSA. Overselling it would be wrong.
 */
...
`.trim(),
  ...
  approaches: [
    {
      name: "Writing a do-while Loop",
      idea: "Put the body before the condition so it always runs at least once.",
      steps: [
        "Declare any variable the condition will test before the loop, so it stays in scope.",
        "Write the body inside a do block.",
        "Write the condition in a while clause after the closing brace.",
        "Terminate the whole statement with a semicolon.",
        "At runtime the body runs first, then the condition is checked.",
        "If the condition is true, control returns to the top of the body; if false, execution continues after the loop.",
      ],
      code: [
        {
          language: "cpp",
...
public class DoWhileBasics {
    public static void main(String[] args) {
        ...
        {
          language: "python",
          code: `# Python has no do-while. The equivalent uses while True with a break.
...
while True:
    ...
    {
      name: "Input Validation: The Canonical Use",
      idea: "Ask, then judge — because the value being judged does not exist until you have asked.",
      steps: [
        "Declare the variable that will hold the input, before the loop.",
        "Inside the body, prompt for and read the value.",
        "Optionally report why a previous attempt was rejected.",
        "Write the loop condition to be true while the value is still unacceptable.",
        "The loop repeats until a valid value arrives, then exits with that value in scope.",
      ],
      code: [
        {
          language: "cpp",
...
do {
    ...
    choice = sc.nextInt();
        ...
        {
          language: "python",
          code: `# The same intent with while True and break
while True:
...
while True:
    ...
    {
      name: "Emulating do-while in Python",
      idea: "Choose between the two standard workarounds, and know why one is preferred.",
      steps: [
        "Write while True so the loop is unconditional at the header.",
        "Place the body's work first, exactly as a do block would.",
        "Evaluate the stopping test after that work, where the do-while condition would sit.",
        "Break when the loop should end, remembering the test is the negation of a do-while condition.",
        "Prefer this over a flag variable, which needs an extra name and an extra assignment to say the same thing.",
      ],
      code: [
        {
          language: "python",
          code: `# METHOD 1 — while True with break (preferred)
total = 0
while True:
    n = int(input())
    total += n
...
while True:
    ...
    if i >= 5:
        ...
        {
          language: "cpp",
          code: `// For comparison — the construct Python is emulating
int total = 0;
int n;
do {
    cin >> n;
...
while (true) {
  ...
  examples: [
    {
      input: "i = 10; do { print(\"ran\"); } while (i < 5);  compared with while (i < 5) { print(\"ran\"); }",
      output: "The do-while prints once. The while prints nothing.",
      walkthrough: [
        "In the do-while, control enters the body immediately without evaluating anything.",
        "The body prints, producing output before any condition has been considered.",
        "The condition 10 < 5 is then evaluated and is false.",
        "The loop exits, having run its body exactly once.",
        "In the while version, the condition 10 < 5 is evaluated first and is false.",
        "The body is skipped entirely and nothing is printed.",
        "Identical condition, identical body, different output — because of where the check sits.",
      ],
      why: "The single defining difference between the two loops, isolated to the one input where it is visible.",
    },
    {
      input: "Age validation with inputs -5, then 150, then 42",
      output: "Two rejections, then Accepted: 42",
      walkthrough: [
        "The body runs and reads -5, which is the first value that exists to be judged.",
        "The condition tests whether -5 is outside the range 1 to 120, which it is, so the loop repeats.",
        "The body runs again and reads 150, which is also outside the range, so the loop repeats.",
        "The body runs a third time and reads 42.",
        "The condition finds 42 within range, so it is false and the loop exits.",
        "42 is still in scope after the loop because the variable was declared before it.",
      ],
      why: "The canonical use, and it shows why entry-controlled checking is impossible here — there was no value to test until the body had run.",
    },
    {
      input: "The same validation written with a while loop instead",
      output: "Works correctly, but the prompt-and-read logic appears twice",
      walkthrough: [
        "The value must be read once before the loop, so the condition has something to test.",
        "The while condition then checks whether that value is invalid.",
        "If it is, the body reads again — repeating the same prompt-and-read code already written above.",
        "The loop exits once a valid value arrives, giving the correct result.",
        "The cost is duplication: the same logic exists in two places and must be kept in step.",
        "A later change to the prompt or the read must be made twice, and missing one is a real and common bug.",
      ],
      why: "Removing this duplication is the whole practical argument for do-while, so seeing the duplicated version is what justifies the construct.",
    },
    {
      input: "do { int x; cin >> x; } while (x != 0);",
      output: "Compile error — x is not in scope in the condition",
      walkthrough: [
        "The variable x is declared inside the do block.",
        "Its scope begins at that declaration and ends at the block's closing brace.",
        "The while clause sits after that closing brace, so the block has already ended.",
        "At that point the name x no longer refers to anything, and the compiler rejects the code.",
        "Moving the declaration above the do block gives x a scope that covers both the body and the condition.",
        "This is caught at compile time, so it costs a moment rather than a debugging session.",
      ],
      why: "The one structural gotcha unique to do-while, arising directly from the condition sitting outside the block it depends on.",
    },
  ],
  ...
  commonDoubts: [
    {
      question: "When would I actually use a do-while?",
      answer:
    ...
    {
      question: "Why does a do-while need a semicolon when other loops don't?",
      answer:
    ...
    {
      question: "Does Python have a do-while loop?",
      answer:
    ...
    {
      question: "Why not just copy the body once before a while loop?",
      answer:
    ...
    {
      question: "Is do-while ever strictly required, or always optional?",
      answer:
    ...
    {
      question: "Why can't I use a variable declared inside the do block in the condition?",
      answer:
    ...
    {
      question: "Which of the three loops should I default to?",
      answer:
    ...
    {
      question: "Do I need do-while for DSA problems?",
      answer:
  ...
  relatedIds: ["while-loop", "for-loop", "break-and-continue"],
};
